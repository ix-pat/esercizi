...
`r i2 <- i2+1;item()` (**Punti 3**) Quante famiglie hanno una spesa superiore al 75-esimo percentile?
...
`r i2 <- i2+1;item()` (**Punti 14**) Quanto vale $P\Big((A\cap B)\cup C\Big)$?
...
`r i2 <- i2+1;item()` (**Punti 3**) Si estrae 5 volte da $X\sim N(102,1.5)$, posto $A=\{X>100\}$, quale è la probabilità che $A$ si avveri 3 volte su 5?
...
`r i2 <- i2+1;item()` (**Punti 2**) Se $F$ è la funzione di ripartizione della VA $X$, posto $a < b$ due numeri qualunque, a cosa equivale
$$
F(b)-F(a) \qquad?
$$
...
`r i2 <- i2+1;item()` (**Punti 14**) Ogni giorno un impianto di produzione confeziona 1000 lotti. In media il 10 di questi lotti sono fallati, con una deviazione standard pari a 0.3. Dopo 300 giorni di produzione qual è la probabilità che il numero di lotti fallati sia maggiore di 3100?
...
`r i2 <- i2+1;item()` (**Punti 2**) Definire lo _Standard Error_ di uno stimatore.

`r i2 <- i2+1;item()` (**Punti 3**) Definire gli errori di primo e di secondo tipo di un test statistico.

`r i2 <- i2+1;item()` (**Punti 3**) In un test statistico, per quali valori di $p_\text{value}$ si tende a rifiutare $H_0$?
...
(**Punti 14**) Il Supermercato $S$, della catena $C$, ha monitorato gli accessi al suo interno per una settimana.
Qui di seguito il numero di accessi per giorno della settimana di $S$ e la percentuale di accessi nella catena $C$.


```{r}

g <- c( "Lun" , "Mart" , "Merc" , "Giov" , "Ven","Tot")
...
S <- c(s,sum(s))
N <- c("25\\%","10\\%","10\\%","10\\%","45\\%","100\\%")
D <- data.frame(rbind(S,N),row.names = c("Supermercato S","Catena C"))
names(D) <- g
kable(D,booktabs=T,escape = F,linesep="") %>%
  kable_styling(full_width = F, latex_options = "HOLD_position")

ns <- c(n/100*sum(s),sum(s))
ch <- (s-ns[1:5])^2/ns[1:5]
ch <- c(ch,sum(ch))
D1 <- data.frame(rbind(S,N,ns,ch),row.names = c("Supermercato S","Catena C","$n_j^*$","$\\chi^2$"))
names(D1) <- g
# sum(((n/100*sum(s)-s)^2)/n/100*sum(s))
# qchisq(.99,4)
...
kable(D1,booktabs=T,escape = F,linesep="") %>%
  kable_styling(full_width = F, latex_options = "HOLD_position")
...
Il chi quadro osservato è `r sum(ch)` è minore di $\chi^2_{4;0.05}$ `r chi`, e quindi **non rifiuto** $H_0$.
:::
...
`r i2 <- i2+1;item()` (**Punti 2**) Interpretare i parametri di regressione $\hat\beta_0$ e $\hat\beta_1$.

`r i2 <- i2+1;item()` (**Punti 2**) Se $W=-10\times Y$, quanto varrà $r_{XW}$, coefficiente di correlazione tra $X$ e $W$?
...
`r i2 <- i2+1;item()` (**Punti 3**) Quante famiglie hanno una spesa inferiore al 25-esimo percentile?
...
`r i2 <- i2+1;item()` (**Punti 2**) La spesa media è pari a $\bar x=`r mean(samp)`$, che forma ci dobbiamo aspettare dell'istogramma di densità?

`r i2 <- i2+1;item()` (**Punti 2**) La varianza della spesa è pari a $Var=`r var(samp)`$.
...
`r i2 <- i2+1;item()` (**Punti 14**) Quanto vale $P(A\cup B)$?
...
`r i2 <- i2+1;item()` (**Punti 3**) Si estrae 6 volte da $X\sim \text{Pois}(1.5)$, posto $A=\{X<2\}$, quale è la probabilità che $A$ si avveri 3 volte su 6?
...
`r i2 <- i2+1;item()` (**Punti 2**) Se $X$ è una VC con valore atteso $E(X)=0.5$ e $V(X)=1.2$, posto $Y=X^2$ è vero che $E(Y)=E^2(X)$?
...
`r i2 <- i2+1;item()` (**Punti 14**) Si lancia un dado perfetto 100 volte. Qual è la probabilità che la proporzione di volte che si osserva la faccia sei (⚅) sia maggiore di 0.2?
...
`r i2 <- i2+1;item()` (**Punti 3**) Sia $\hat \mu$ lo stimatore di massima verosimiglianza di $\mu$ del modello Normale.
\begin{eqnarray*}
...
`r i2 <- i2+1;item()` (**Punti 2**) Siano $h_1$ e $h_2$ due stimatori per $\theta$, cosa significa dire che $h_1$ è più efficiente di $h_2$?

`r i2 <- i2+1;item()` (**Punti 3**) Definire la significatività e la potenza di un test statistico.

`r i2 <- i2+1;item()` (**Punti 3**) Se in un test statistico il $p_\text{value}>0.1$ possiamo rifiutare $H_0$?
...
`r i2 <- i2+1;item()` (**Punti 12**) In un'indagine sull'opinione sul reddito di cittadinanza sono stati intervistate 140 persone che vivono al nord e 170 che vivono al sud: 60 su 140 che vivono al nord sono favorevoli al reddito di cittadinanza mentre 95 su 170 che vivono al sud sono favorevoli. 
...
`r i2 <- i2+1;item()` (**Punti 2**) Calcolare e discutere il $p$-value del test precedente.
...
`r i2 <- i2+1;item()` (**Punti 14**) Stimare la previsione per $x=1.0$ nel modello di regressione dove $Y$ viene spiegata da $X$.
...
`r i2 <- i2+1;item()` (**Punti 2**) Cosa sono i _punti di leva_ e cosa gli _outliers_?

`r i2 <- i2+1;item()` (**Punti 2**) Se $W=10\times Y$, posto
...
`r i2 <- i2+1;item()` (**Punti 3**) Qual è la percentuale di famiglie con spesa superiore al 25-esimo percentile?
...
`r i2 <- i2+1;item()` (**Punti 14**) Calcolare $P(W< 2)$.
...
`r i2 <- i2+1;item()` (**Punti 3**) Posto $V=2+5\cdot W$, ricavare valore atteso e varianza di $V$.
...
`r i2 <- i2+1;item()` (**Punti 2**) Se $A$ e $B$ sono due eventi diversi dal vuoto, è possibile che $P(A)+P(B)>1$?
...
`r i2 <- i2+1;item()` (**Punti 14**) Un'urna contiene 3 palline bianche, 2 nere e 5 blu. Si estrae 200 volte con reimmissione.
...
`r i2 <- i2+1;item()` (**Punti 3**) Sia $\hat \lambda$ lo stimatore di massima verosimiglianza di $\lambda$ del modello Poisson.
\begin{eqnarray*}
  \hat\lambda &=&  \frac 1n\sum_{i=1}^nx_i\\
\end{eqnarray*}
...
`r i2 <- i2+1;item()` (**Punti 2**) Siano $h$ uno stimatore per $\theta$, cosa significa dire che $h$ è asintoticamente corretto?

`r i2 <- i2+1;item()` (**Punti 3**) Definire la probabilità di significatività.

`r i2 <- i2+1;item()` (**Punti 3**) Se in un test statistico $0.01 < p_\text{value} <0.05$ cosa possiamo concludere?
...
(**Punti 14**) In un'indagine sull'opinione sul reddito sono stati rilevati i redditi di 140 persone che vivono al nord e quelli di 170 che vivono al sud: il reddito medio di chi vive al nord è di 32.2 mila euro annui con una SD pari a 2.4 mila euro annui, mentre il
...
a <- "N"
b <- "S"
cat(ttest_2c_et(mu1 = mu1,mu2 = mu2,s1h = s1h,s2h=s2h,n1 = n1,n2 = n2,h1 = h1,alpha = alpha,a = a,b = b))
```
:::
...
`r i2 <- i2+1;item()` (**Punti 14**) Stimare la previsione per $x=1.4$ nel modello di regressione dove $Y$ viene spiegata da $X$.
...
`r i2 <- i2+1;item()` (**Punti 3**) Calcolare e discutere $R^2$.
...
`r i2 <- i2+1;item()` (**Punti 2**) Cos'è un _punto influente_?

`r i2 <- i2+1;item()` (**Punti 2**) Se $W=10+ Y$, posto
$$w_i=\beta_0'+\beta_1'x_ì +\epsilon_i'$$
...
hj <- dat2$hj
```
la classe modale è la classe $`r `$ essendo la classe con densità $h_{`r pmod`}=`r hj[pmod]`$ maggiore.
:::


`r i2 <- i2+1;item()` (**Punti 3**) Quante imprese hanno una spesa compresa tra 15 mila euro e il 75-esimo percentile?
...
`r i2 <- i2+1;item()` (**Punti 2**)  La spesa media è pari a $\bar x=`r mean(samp)`$ mila euro, 
...
`r i2 <- i2+1;item()` (**Punti 2**) La spesa media in investimenti green su `r n` aziende modenesi è pari a $\bar x=`r mean(samp)`$ mila euro, mentre la spesa media  in investimenti green della provincia di Reggio, su un campione di 150 aziende è pari a $12.22$ mila euro.
...
`r i2 <- i2+1;item()` (**Punti 14**) Quanto vale $P\Big((A\cup B)\cup C\Big)$?
...
`r i2 <- i2+1;item()` (**Punti 3**) Sia $X\sim N(10,1)$, posto $A=\{X>8\}$. Si estrae ripetutamente da $X$  e si finisce quando $A$ si avvera **2** volte. Calcolare la probabilità di finire dopo 6 estrazioni.
...
`r i2 <- i2+1;item()` (**Punti 2**) Siano $A$ e $B$ due eventi diversi dal vuoto. È vero che se $A$ e $B$ sono 
...
`r i2 <- i2+1;item()` (**Punti 2**) Se $F$ è la funzione di ripartizione della VA $X$, 
...
`r i2 <- i2+1;item()` (**Punti 3**) Sia $\hat \lambda$ lo stimatore di massima verosimiglianza di $\lambda$ del modello di Poisson: $\hat\lambda =  \frac 1n\sum_{i=1}^nx_i$
Scrivere lo Standard Error di $\hat \lambda$.

`r i2 <- i2+1;item()` (**Punti 3**) Se $h$ è uno stimatore per $\theta$ tale che
$E(h)\ne \theta$ e che $\lim_ {n\to +\infty}E(h)=\theta$ di quale proprietà gode $h$?

`r i2 <- i2+1;item()` (**Punti 3**) Definire gli errori di primo e di secondo tipo di un test statistico e le relative probabilità.

`r i2 <- i2+1;item()` (**Punti 3**) In un confronto tra due campioni viene messo a test
$$
\begin{cases}
H_0:\sigma_A=\sigma_B\\
H_1:\sigma_A\ne \sigma_B
...
`r item(); i2 <- i2+1 `(**Punti 7**) Costruire un Intervallo di Confidenza al 95% per la media di popolazione $\mu$.
...
`r item(); i2 <- i2+1 `(**Punti 7**) La stessa domanda è stata posta ad un secondo campione di $n_S=38$ consumatori privati,scelti a caso tra i cittadini del comune di Sassuolo. 
...
et <- F
um <- ""
cat(ttest_2c_om(mu1 = mu1,mu2 = mu2,s1h = s1h,s2h = s2h,n1 = n1,n2 = n2,h1 = h1,alpha = alpha))
```
:::
...
`r i2 <- i2+1;item()` (**Punti 14**) Calcolare il residuo del quarto dato nel modello di regressione dove $Y$ viene spiegata da $X$.
...
`r i2 <- i2+1;item()` (**Punti 3**) Scrivere la scomposizione della varianza del modello di regressione 
...
`r i2 <- i2+1;item()` (**Punti 3**) Il parametro di regressione $\hat\beta_0$, in questo caso, è interpretabile?

`r i2 <- i2+1;item()` (**Punti 2**) Una previsione per $x=40$ è attendibile? Perché?

`r i2 <- i2+1;item()` (**Punti 2**) Se $W=5+ Y$, posto $w_i=\beta_0'+\beta_1'x_ì +\epsilon_i'$
il modello in cui $W$ viene spiegata da $X$, quanto varranno $\beta_0'$ e $\beta_1'$?
...
`r i2 <- i2+1;item()` (**Punti 14**) Disegnare l'istogramma delle densità percentuali.
...
`r i2 <- i2+1;item()` (**Punti 3**) Quante aziende hanno una spesa compresa tra il 25-esimo percentile e 15 mila euro?
...
`r i2 <- i2+1;item()` (**Punti 2**) Che relazione dobbiamo attenderci tra media, mediana e moda?
...
`r i2 <- i2+1;item()` (**Punti 14**) Si consideri l'evento $E$="più di due veicoli impegnino la rotatoria". Calcolare $P(E)$.
...
`r i2 <- i2+1;item()` (**Punti 3**) Si osserva la rotatoria per $n=6$ minuti. Qual è
...
`r i2 <- i2+1;item()` (**Punti 2**) Se $X\sim N(0,2)$ e $X\sim N(1,1.2)$, è vero che
$$
X-Y\sim N(-1,0.8) \qquad ?
$$


`r i2 <- i2+1;item()` (**Punti 2**) Se $X$ è una VC con supporto {0,1,2} e $Y$ è una VC con supporto {2,3,4,5}.
...
`r i2 <- i2+1;item()` (**Punti 3**) Se in un test statistico che utilizza la statistica test _t_ con 10 gradi di libertà $t_\text{obs}=1.4$, il $p_\text{value}$ sarà maggiore o minore di 0.05? Perché?
...
`r i2 <- i2+1;item()` (**Punti 4**) Costruire un intervallo di confidenza la 95% per $\pi$ la quota di persone del quartiere _Q_ favorevole alle corsie preferenziali
...
`r i2 <- i2+1;item()` (**Punti 10**) Un'indagine molto più ampia condotta su tutta la città ha mostrato che
la percentuale di favorevoli alle corsie preferenziali è del 55%. Testare al 5% l'ipotesi che nel quartiere _Q_ la quota di favorevoli sia uguale a quella cittadina contro l'alternativa che sia maggiore.
...
`r i2 <- i2+1;item()` (**Punti 14**) Stimare la previsione per $x=16$ nel modello di regressione dove $Y$ viene spiegata da $X$.
...
`r i2 <- i2+1;item()` (**Punti 2**) Cosa sono i _punti influenti_?

`r i2 <- i2+1;item()` (**Punti 2**) Se $W=2\times Y$ e $V=X+3$, posto $w_i=\beta_0'+\beta_1'v_ì +\epsilon_i'$
...
`r i2 <- i2+1;item()` (**Punti 3**) Qual è la percentuale di aziende che hanno una spesa 
...
`r i2 <- i2+1;item()` (**Punti 14**) Qual è la probabilità di vincere?
...
`r i2 <- i2+1;item()` (**Punti 3**) Si ripete il gioco di sopra finché non si vince due volte. Qual è la probabilità di finire alla quarta giocata?
...
`r i2 <- i2+1;item()` (**Punti 2**) Se $X\sim \text{Pois}(2)$ e $Y\sim\text{Pois}(1)$, è vero che
$$
X-Y\sim\text{Pois}(1)\qquad ?
$$


`r i2 <- i2+1;item()` (**Punti 2**) Se $X$ è una VC con supporto {0,1,2} e $X$ è una VC con supporto {-2,-1,0}.
...
(**Punti 14**) Il supermercato _S_ vende, in media ogni giorno, 102.3 kg di pasta, con una deviazione standard di 10.2. Dopo 60 giorni di apertura, qual è la probabilità che il totale di pasta venduta sia maggiore di 6700 kg?
...
`r i2 <- i2+1;item()` (**Punti 3**) Se uno stimatore $h$ per $\theta$ è tale che $\lim_{n\to\infty}V(h)=0$, di quali proprietà gode $h$?

`r i2 <- i2+1;item()` (**Punti 3**) Un intervallo di confidenza per $\theta$ al 95% è più ampio o meno ampio di uno al 99%? Perché?

`r i2 <- i2+1;item()` (**Punti 3**) Se in un test statistico che utilizza la statistica test _t_ con 10 gradi di libertà $t_\text{obs}=14$, il $p_\text{value}$ sarà maggiore o minore di 0.05? Perché?
...
`r i2 <- i2+1;item()` (**Punti 4**) Costruire un intervallo di confidenza la 99% per $\mu$ il punteggio medio che le persone del quartiere _Q_ esprimono riguardo alle corsie preferenziali.
...
`r i2 <- i2+1;item()` (**Punti 14**) Questi sono alcuni dei dati osservati
...
`r i2 <- i2+1;item()` (**Punti 3**) Ricavare numericamente la scomposizione della varianza del modello di regressione sopra stimato.
...
`r i2 <- i2+1;item()` (**Punti 2**) Che differenza c'è tra _interpolazione_ e _estrapolazione_?

`r i2 <- i2+1;item()` (**Punti 2**) Cosa significa che $r$ è un numero puro?
...
`r i2 <- i2+1;item()` (**Punti 3**) Quante imprese hanno un bilancio compreso tra $-4$ mila 
euro e zero.
...
`r i2 <- i2+1;item()` (**Punti 2**)  La media è risultata essere $\bar x=`r mean(samp)`$; che
...
`r i2 <- i2+1;item()` (**Punti 2**) Siano $x_1,...,x_n$, $n$ numeri, $n$ dispari. 
...
`r i2 <- i2+1;item()` (**Punti 14**) Calcolare $P(A\cup B)$.
...
`r i2 <- i2+1;item()` (**Punti 3**) Posto $Z=X-Y$, Calcolare la probabilità che $P(Z>1|Z\le 2)$. 
...
`r i2 <- i2+1;item()` (**Punti 2**) Siano $A$ e $B$ due eventi diversi dal vuoto.
...
`r i2 <- i2+1;item()` (**Punti 2**) Sia $X\sim\text{Pois}(\lambda=1)$, disegnare la
...
(**Punti 14**) Un'urna contiene tre palline, una Rossa, una Blue e una Nera. Si vince
...
`r i2 <- i2+1;item()` (**Punti 3**) Definire gli errori di primo e di secondo 
...
(**Punti 14**) In uno studio sule preferenze di genere è stato chiesto ad un campione di 150 persone,
...
`r i2 <- i2+1;item()` (**Punti 14**) Calcolare il residuo del comune B nel modello di regressione dove $Y$ viene spiegata da $X$.
...
`r i2 <- i2+1;item()` (**Punti 3**) Interpretare il parametro di regressione $\hat\beta_1$.

`r i2 <- i2+1;item()` (**Punti 2**) Descrivere la differenza tra punti di leva e punti influenti.

`r i2 <- i2+1;item()` (**Punti 2**) Gli stimatori $\hat\beta_0$ e $\hat\beta_1$ dei minimi 
...
`r i2 <- i2+1;item()` (**Punti 3**) Quante aziende hanno un bilancio compreso tra il 25-esimo 
...
`r i2 <- i2+1;item()` (**Punti 2**) Siano $x_1,...,x_n$, $n$ numeri. Si consideri la funzione:
$$g(x)=(x_1-x)^2+...+(x_n-x)^2.$$
Per quale valore di $x$, $g(x)$ è minima?
...
`r i2 <- i2+1;item()` (**Punti 14**) Si consideri l'evento $E$="più di due veicoli 
...
`r i2 <- i2+1;item()` (**Punti 3**) Calcolare
$$
...
`r i2 <- i2+1;item()` (**Punti 2**) Siano $X\sim N(0,1)$ e $Y\sim N(0,1)$, $X$ e $Y$ indipendenti.
Come si distribuisce $X^2+Y^2$?


`r i2 <- i2+1;item()` (**Punti 2**) Se $X$ è una VC con supporto {2,3,4,5} e $Y$ è una VC con supporto 
{1,2,6}.
Qual è il supporto di $X-Y$?
...
(**Punti 14**) Un'urna contiene 4 palline numerate da 1 a 4. 
...
`r i2 <- i2+1;item()` (**Punti 3**) Sia $\hat\sigma^2$ lo stimatore di massima verosimiglianza
di $\sigma^2$. In virtù di quale proprietà $\hat \sigma$ è lo stimatore di
massima verosimiglianza per $\sigma$?

`r i2 <- i2+1;item()` (**Punti 3**) Siano $T_1$ e $T_2$ due test per lo stesso sistema di ipotesi,
...
`r i2 <- i2+1;item()` (**Punti 3**) Se in un test statistico bilaterale che utilizza 
...
`r item(); i2 <- i2+1 `(**Punti 4**) Costruire un Intervallo di Confidenza al 95% per la media di popolazione $\mu$.


:::{.sol data-latex=""}
$$
S=\sqrt{\frac{n}{n-1}}\hat \sigma=`r sqrt(35/34)*2.1` 
$$


\begin{eqnarray*}
  \left[~~12.4\pm t_{34;.005}{\frac{`r sqrt(35/34)*2.1`}{\sqrt{35}}}~~\right] &=&  
  \left[~~12.4\pm `r qt(.995,34)`\times`r 2.1/sqrt(34)`~~\right]\\
  &=&\left[~~`r 12.4-qt(.995,34)*2.1/sqrt(34)`; `r 12.4+qt(.995,34)*2.1/sqrt(34)`~~\right]
\end{eqnarray*}  
:::

`r item(); i2 <- i2+1 `(**Punti 10**) La stessa domanda è stata posta ad un secondo campione di $n_F=38$ consumatori privati,scelti a caso tra i cittadini del comune di Ferrara. 
...
h1 <- "<"
um <- ""
cat(ttest_2c_et(mu1 = mu1,mu2 = mu2,s1h = s1h,s2h = s2h,n1 = n1,n2 = n2,h1 = h1,alpha = alpha))
```
:::
...
`r i2 <- i2+1;item()` (**Punti 14**) Stimare la previsione per $x=1.6$ nel modello di regressione dove $Y$ viene spiegata da $X$.
...
`r i2 <- i2+1;item()` (**Punti 3**) Calcolare numericamente $RSS$:
$$
...
`r i2 <- i2+1;item()` (**Punti 3**) Gli stimatori $\hat\beta_0$ e $\hat\beta_1$ sono consistenti? 
Perché?

`r i2 <- i2+1;item()` (**Punti 2**) Se in un modello di regressione con 11 dati, 
il residuo studentizzato del dato $i$ è $\tilde \epsilon_i=1.23$, cosa possiamo concludere?

`r i2 <- i2+1;item()` (**Punti 2**) Sia $\hat\beta_1$ lo stimatore dei minimi quadrati per $\beta_1$.
...
`r i2 <- i2+1;item()` (**Punti 3**) Quante aziende hanno un bilancio compreso tra $-1$ e il 75-esimo 
percentile?
...
`r i2 <- i2+1;item()` (**Punti 2**) Sapendo che $\sigma_X=`r sc (samp)`$ la Standard Deviation di $X$
...
`r i2 <- i2+1;item()` (**Punti 3**) Si ripete il gioco di sopra finché non si vince tre volte. Qual è la probabilità di finire alla quinta giocata?
...
`r i2 <- i2+1;item()` (**Punti 2**) Se $X$ è una VC con supporto {1,2,3} e $Y$ è una VC con 
supporto {1,3,5}.
Qual è il supporto di $X\times Y$?
...
(**Punti 14**) Il supermercato _S_ accoglie, in media ogni giorno, 3.242 mila persone, con una
...
`r i2 <- i2+1;item()` (**Punti 3**) Sia $\hat\theta$ lo stimatore di massima verosimiglianza 
per $\theta$. Qual è la distribuzione asintotica di $\hat\theta$?

`r i2 <- i2+1;item()` (**Punti 3**) Un intervallo di confidenza per $\theta$ al 90% è più ampio o meno ampio di uno al 95%? Perché?

`r i2 <- i2+1;item()` (**Punti 3**) Se in un test statistico bilaterale che utilizza la statistica test _z_ , risulta $z_\text{obs}=14$, il $p_\text{value}$ sarà maggiore o minore di 0.01? Perché?
...
`r i2 <- i2+1;item()` (**Punti 4**) Costruire un intervallo di confidenza la 95% per $\pi$ la quota di tassisti favorevoli al decreto liberalizzazione
...
`r i2 <- i2+1;item()` (**Punti 10**) La stessa domanda è stata posta ad un secondo 
...
`r i2 <- i2+1;item()` (**Punti 3**) Calcolare la percentuale di varianza di $Y$ spiegata dal modello.
...
`r i2 <- i2+1;item()` (**Punti 2**) Se in un modello di regressione con 15 dati, 
il residuo studentizzato del dato $i$ è $\tilde \epsilon_i=12.3$, cosa possiamo concludere?


`r i2 <- i2+1;item()` (**Punti 2**) Sia $\hat\beta_0$ lo stimatore dei minimi quadrati per $\beta_0$.
