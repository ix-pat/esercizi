...
`r punti_p()` Siano $\hat\theta$ uno stimatore per $\theta$, cosa significa dire che $\hat\theta$ è asintoticamente corretto?
...
`r punti_p()` Se $\hat\theta$ è uno stimatore per $\theta$ tale che
...
`r punti_p()` Se uno stimatore $\hat\theta$ per $\theta$ è tale che $\lim_{n\to\infty}V(\hat\theta)=0$, di quali proprietà gode $\hat\theta$?
