...
`r i2 <- i2+1;item()` Disegnare l'istogramma di densità percentuale.
...
`r p=0.65; X<-dat2`
`r i2 <- i2+1;item()`   calcolare il valore approssimato del percentile  $`r p*100`$-esimo, e tracciarlo nell'istogramma.
...
`r i2 <- i2+1;item()` Qual è il numero di individui con `r nomex` superiore a 5.2?
...
`r i2 <- i2+1;item()`   Analizzare la relazione tra media, mediana e moda alla luce del istogramma di densità.
...
`r i1 <- i1+1; i2 <- 0`   Sono stati analizzati `r n` individui per investigare su `r nomex`. È riportata qui di seguito la distribuzione in classi espressa in frequenza assoluta.
...
`r i2 <- i2+1;item()`  Calcolare il numero di individui maggiori del 75-esimo percentile, $x_{0.75}$
...
`r i2 <- i2+1;item()`   calcolare il valore approssimato della media aritmetica  $\bar x$ e della varianza $\sigma^2$.
...
`r i1 <- i1+1; i2 <- 0`   Sono stati analizzati `r n` individui per investigare su `r nomex`. Sono noti i percentili
...
`r i2 <- i2+1;item()`   Calcolare le frequenze assolute
...
`r i2 <- i2+1;item()`   Calcolare la percentuale approssimata di individui con `r nomex` inferiore a 45 
...
`r i2 <- i2+1;item()`   Calcolare la percentuale approssimata di individui con `r nomex` compresa tra 45 e il 90-esimo percentile, $x_{0.90}$.
...
`r i2 <- i2+1;item()` Qual è l'intervallo con il maggior numero di imprese?

:::{.sol data-latex=""}
L'intervallo $[10;\ 19]$.
:::

`r i2 <- i2+1;item()` Qual è il numero di imprese che hanno addetti
nella classe $[0,\, 9]$?
...
`r i2 <- i2+1;item()` In quale classe si trova il 15$^{o}$ percentile?

:::{.sol data-latex=""}
Il 15$^{o}$ percentile si trova nella classe $[0;\, 9]$.
:::

`r i2 <- i2+1;item()` Qual è l'intervallo che contiene la mediana?
...
`r i2 <- i2+1;item()` In quale classe si trova il 75$^{o}$ percentile?
...
`r i2 <- i2+1;item()` Quale relazione ci si deve attendere
...
`r i2 <- i2+1;item()` Determinare il valore approssimato della mediana,
...
`r i2 <- i2+1;item()` Definizione formale di percentile.
...
`r i2 <- i2+1;item()` Qual è la percentuale di autovetture comprese nella seconda classe?
...
`r i2 <- i2+1;item()` In quale classe cade la mediana?
...
`r i2 <- i2+1;item()` Qual è la "classe modale" ?
...
`r i2 <- i2+1;item()` Giulio ha una macchina con cilindrata pari a 625cc. Indicare la risposta corretta:
...
`r i2 <- i2+1;item()` Disegnare l'istogramma (delle densità relative) della
...
`r i2 <- i2+1;item()` Disegnare l'istogramma (delle densità percentuali) della
distribuzione della cilindrata delle auto in circolazione.
...
`r i2 <- i2+1;item()`  il campo di variazione,
`r i2 <- i2+1;item()` la mediana,
`r i2 <- i2+1;item()`  la media sapendo che la somma è pari a 187.64 per l'impianto A e 227.07 per l'impianto B.
...
`r i2 <- i2+1;item()`  $\,$il primo quartile,
`r i2 <- i2+1;item()`  $\,$il terzo quartile,
`r i2 <- i2+1;item()`  $\,$la differenza interquartile.
...
`r i2 <- i2+1;item()` Calcolare la varianza, sapendo che
$\sum_{i=1}^{20} x_{A;\, i}^{2} = 2064.08$ e
$\sum_{i=1}^{20} x_{B;\, i}^{2} = 3077.31$. 
`r i2 <- i2+1;item()` Calcolare la deviazione standard.
...
`r i2 <- i2+1;item()`   La distribuzione è asimmetrica?
Se sì, di quale tipo di asimmetria si tratta?
`r i2 <- i2+1;item()`  Dalle risposte date, emergono differenze tra i due impianti?
