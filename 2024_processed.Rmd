...
`r item()` `r punti_p()` Quante famiglie spendono meno di 5 mila euro all'anno? 
...
`r item()` `r punti_p()` Che relazione dobbiamo aspettarci tra media,  mediana e moda? 
...
`r item()` `r punti_p()` Sia lancia una seconda moneta perfetta 3 volte. Sia $Y$ la variabile casuale che conta il numero di Teste su 3 lanci. Calcolare la probabilità che $X+Y=3$.
...
`r item()` `r punti_p()` Se $P(A)=0.4$ e $P(B)=0.8$, $A$ e $B$ possono essere incompatibili? Perché?

:::{.sol data-latex=""}
Se $A$ e $B$ fossero incompatibili, allora $A\cap B=\emptyset$ allora $P(A\cup B)=P(A)+P(B)$, ma essendo $0.4+0.8=1.2>1$ allora $A\cap B\neq\emptyset$
:::

`r item()` `r punti_p()` Cosa significa che la funzione di ripartizione è continua a destra?
...
`r item()` `r punti_p()` Definire lo Standard Error di uno stimatore.
...
`r item()` `r punti_p()` Se in un modello di regressione $r=-1$ cosa significa?

`r item()` `r punti_p()` Se in un modello di regressione $r=0.55$, $\hat\sigma_X=0.9$ e $\hat\beta_1=1.5$, quanto varrà
$\hat\sigma_Y$, la standard deviation di $Y$?

`r item()` `r punti_p()` Testare l'ipotesi che $\beta_1$ sia uguale a zero, contro l'alternativa che sia diverso per diversi livelli di significatività e dare una valutazione approssimativa del $p$-value (ad esempio il $p$-value è minore di 0.001, compreso tra 0.05 e tra 0.01, ecc.).
...
`r item()` `r punti_p()` Quante famiglie investono più di 4 mila euro all'anno? 
...
`r item()` `r punti_p()` La spesa media è pari a $`r mean(samp)`$, mentre la varianza è pari a $`r s2c(samp)`$.
...
`r item()` `r punti_p()` Sia $Y$ numero settimanale di pratiche inevase dall'ufficio comunale, si assume $Y\sim\text{Pois}(2)$ e si assume $X$ indipendente da $Y$. Calcolare la probabilità che $X+Y=5$.
...
`r item()` `r punti_p()` Se $P(A)=0.4$ e $P(B)=0.4$, $A$ e $B$ possono essere incompatibili? Perché?

`r item()` `r punti_p()` Sia $X$ una variabile casuale e sia $F$ la sa funzione di ripartizione. Siano $a$ e $b$ due numeri reali, $a<b$, esprimere
\[
P(a<X\leq b)
\]
in termini di $F$.
...
`r item()` `r punti_p()` Che differenza c'è tra lo Standard Error di uno stimatore e la Deviazione Standard del campione?
...
`r item()` `r punti_p()` Se in un modello di regressione $r<0$ è possibile che $\hat\beta_1>0$? Perché?

`r item()` `r punti_p()` Testare l'ipotesi che $\beta_0$ sia uguale a 35, contro l'alternativa che sia minore per diversi livelli di significatività e dare una valutazione approssimativa del $p$-value (ad esempio il $p$-value è minore di 0.0005, compreso tra 0.05 e  0.01, ecc.).
...
`r item()` `r punti_p()` Quante famiglie spendono tra 4 mila e 10 mila euro l'anno? 
...
`r item()` `r punti_p()` Sia $Y\sim N(3,1)$, $X$ e $Y$ indipendenti, posto $W=X-Y$, calcolare $P(W<0)$.
...
`r item()` `r punti_p()` Se $P(A)=0.4$ e $P(B)=0.6$, in che relazione sono $A$ e $B$?


`r item()` `r punti_p()` Sia $X$ una variabile casuale e sia $F$ la sua funzione di ripartizione. Cosa significa che $F$ è una funzione crescente?
...
`r item()` `r punti_p()` Che differenza c'è tra lo Standard Error di uno stimatore e la Deviazione Standard di popolazione?

`r item()` `r punti_p()` Definire gli errori di primo e di secondo tipo.
...
`r item()` `r punti_p()` Un biologo sta studiando il numero di cellule di una certa specie osservate in campioni di terreno. Ha raccolto dati sul numero di cellule in `r total` campioni. I dati osservati sono riportati nella tabella seguente. L'obiettivo è determinare se i dati seguono una distribuzione di Poisson.
...
`r item()` `r punti_p()` Calcolare e discutere il $p$-value del test precedente.
...
`r item()` `r punti_p()` Dare un'interpretazione dei parametri di regressione stimati.

`r item()` `r punti_p()` Definire gli outliers.

`r item()` `r punti_p()` Se in un modello di regressione $r=0$ cosa significa?

`r item()` `r punti_p()` Se in un modello di regressione $r=0.55$, $\hat\sigma_Y=0.9$ e $\hat\beta_1=1.5$, quanto varrà
...
`r item()` `r punti_p()` Sia $X$ la VC del punto precedente e sia $Y\sim N(0,1)$, $X$ ed $Y$ indipendenti. Considerato $A=\{X\leq 2\}$, $B=\{Y<1\}$, calcolare $P(A\cup B)$.
...
`r item()` `r punti_p()` Se $A$ e $B$, sono due eventi tali che $P(B|A)=1$, determinare $A\cap B$.

:::{.sol data-latex=""}
Se $P(B|A)=P(A\cap B)/P(A)$ e $P(B|A)=P(A\cap B)/P(A)=1$ se e solo se $P(A\cap B)=P(A)$ e quindi $A\cap B = A$.
:::

`r item()` `r punti_p()` Sia $X$ una Bernoulli di parametro $\pi$, $X\sim\text{Ber}(\pi)$ e sia $F$ la sua funzione di ripartizione. Se $F(0)=0.6$, quanto vale $\pi$?
...
`r item()` `r punti_p()` Se $\hat\theta$ è lo stimatore di massima verosimiglianza 
...
`r item()` `r punti_p()` perché la previsione per $x=`r round(mx)`$ è più affidabile di quella per $x=`r round(10*mx)`$.

`r item()` `r punti_p()` Se in un modello di regressione $r=-1$ cosa significa?

`r item()` `r punti_p()` Se in un modello di regressione $r=0.75$, $\hat\sigma_X=0.8$ e $\hat\beta_1=1.2$, calcolare
...
`r item()` `r punti_p()` Testare l'ipotesi che $\beta_1$ sia uguale a zero, contro l'alternativa che sia diverso per $\alpha=0.1,0.05,0.01,0.001$ e dare una valutazione approssimativa del $p_\text{value}$ (ad esempio il $p_\text{value}$ è minore di 0.001, compreso tra 0.05 e tra 0.01, ecc.).
...
`r item()` `r punti_p()` Quante famiglie spendono più del 25-esimo percentile $x_{0.25}$? 
...
`r item()` `r punti_p()` La media è pari a $\bar x=`r p(mean(samp),2)`$, senza disegnare l'istogramma, che forma distributiva dobbiamo aspettarci?

`r item()` `r punti_p()` `r round_all(2)`La spesa media della regione Emilia-Romagna, calcolata su `r n_ER` famiglie è pari $\bar x_{ER}=`r x_ER`$, quella della Lombardia, calcolata su `r n_L` famiglie è pari $\bar x_{L}=`r x_L`$, mentre quella del Veneto,  calcolata su `r n_V` famiglie è pari $\bar x_{V}=`r x_V`$. Qual è la spesa media complessiva delle tre regioni?
...
`r item()` `r punti_p()` Siano $X\sim\text{Pois}(1.2)$ e $Y\sim\text{Binom}(n=2,\pi=0.5)$, $X$ e $Y$ indipendenti, posto $A=\{X\geq 2\}$, $B=\{Y=0\}$, calcolare $P(A\cup B)$.
...
`r item()` `r punti_p()` Se $A$ e $B$ sono due eventi diversi da $\emptyset$, se
$P(A|B)=0$ in che relazione sono $A$ e $B$?


`r item()` `r punti_p()` Sia $X$ una variabile casuale con supporto $S_X=c\{-1,0,2\}$ e con funzione di probabilità
$$
f(x)=\begin{cases}
\frac 25, &\text{ se $x=-1$}\\
...
`r item()` `r punti_p()` Cosa significa che lo stimatore di massima verosimiglianza è invariante alle trasformazioni monotone invertibili?

`r item()` `r punti_p()` Siano $T_1$ e $T_2$ due test per lo stesso sistema di ipotesi con la stessa significatività $\alpha$, sia $\beta_1=0.23$ la probabilità di errore di secondo tipo del test $T_1$ e $\beta_2=0.18$ la probabilità di errore di secondo tipo del test $T_2$. Quale dei due test è più potente?

`r item()` `r punti_p()`  In un'indagine sull'opinione sul reddito di inclusione sono stati intervistate 150 persone che vivono al nord e 180 che vivono al sud: 60 su 150 che vivono al nord sono favorevoli al reddito di cittadinanza mentre 95 su 180 che vivono al sud sono favorevoli. 
Messo a test
...
`r item()` `r punti_p()` Se in un modello di regressione $r^2=1$ cosa significa?

`r item()` `r punti_p()` Se in un modello di regressione $r<0$ che segno avrà $\hat\beta_1$? Perché?

`r item()` `r punti_p()` Testare l'ipotesi che $\beta_0$ sia uguale a 16.4, contro l'alternativa che sia minore per $\alpha=0.1,0.05,0.01,0.001$ e dare una valutazione approssimativa del $p_\text{value}$ (ad esempio il $p_\text{value}$ è minore di 0.001, compreso tra 0.05 e tra 0.01, ecc.).
...
`r item()` `r punti_p()` Qual è la percentuale di famiglie spendono più del 55-esimo percentile $x_{0.55}$? 
...
`r item()` `r punti_p()` , posto $W=X-Y$, calcolare $P(W<1|W>-1)$.
...
`r item()` `r punti_p()` Se $P(A)=0.4$ e $P(B)=0.7$, è possibile che $A\cap B=\emptyset$? Perché?
...
`r item()` `r punti_p()` Sia $X\sim\text{Binom}(n=2,\pi=0.5)$ e sia $F$ la sua funzione di ripartizione. Disegnare $F(x)$ nell'intervallo $-1\leq x\leq 3$
...
`r item()` `r punti_p()` Definire i punti di leva e indicare una misura per misurarli.

`r item()` `r punti_p()` Se in un modello di regressione $r^2=0$ cosa significa?

`r item()` `r punti_p()` Se in un modello di regressione $r=0.55$, $\hat\sigma_Y=0.9$ e $\hat\beta_1=1.5$,  calcolare
$\hat\alpha_1$, la stima del coefficiente angolare del modello
