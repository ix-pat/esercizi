...
`r i2 <- i2+1;item()`  Calcolare la probabilità di ottenere due biglietti
con il numero 1.
...
`r i2 <- i2+1;item()` La probabilità che almeno uno dei biglietti sia
...
`r i2 <- i2+1;item()`  La probabilità che la somma dei numeri riportati sui
...
`r i2 <- i2+1;item()`  Qual è la probabilità di trovare la combinazione giusta
...
`r i2 <- i2+1;item()`    Quali sono le coppie di palline che formano l'unione
...
`r i2 <- i2+1;item()`    Quali sono le coppie di palline che formano l'intersezione
...
`r i2 <- i2+1;item()`    Qual è il complementare di C?

:::{.sol data-latex=""}
$\overline{C}=B$
:::

`r i2 <- i2+1;item()`    Calcolare la probabilità di C condizionata all'evento A, $P(C|A).$
...
`r i2 <- i2+1;item()`    Calcolare P($\overline{B}$).

:::{.sol data-latex=""}
P($\overline{B}$)= 1 - P(B)=1/2
:::

`r i2 <- i2+1;item()`    Calcolare  la probabilità che si verifichi B,
...
`r i2 <- i2+1;item()`    I due eventi sono incompatibili?
...
`r i2 <- i2+1;item()`    Qual è la probabilità che lo studente
...
`r i2 <- i2+1;item()`    Qual è la probabilità che lo studente superi
...
`r i2 <- i2+1;item()`    Calcolare la probabilità che Giulio non debba pagare
la multa (evento $C$).
...
`r i2 <- i2+1;item()`    Calcolare la probabilità che Giulio in tre giorni
...
`r i2 <- i2+1;item()`    Costruire il supporto.
...
`r i2 <- i2+1;item()`    Determinare la probabilità di un
punto del supporto.
...
`r i2 <- i2+1;item()`    Qual è la probabilità che la somma dei punteggi sia 9?
...
`r i2 <- i2+1;item()`    Qual è la probabilità che la somma ($S$)
...
`r i2 <- i2+1;item()`    Calcolare la probabilità che la SOMMA ($S$)
...
`r i2 <- i2+1;item()`    Ricavare la distribuzione di probabilità di $X=A_{1}+B_{1}$.
...
`r i2 <- i2+1;item()`    Calcolare la probabilità dell'evento $\{A_{1}>B_{1}\}$.
...
`r i2 <- i2+1;item()`    Si ripete l'esperimento 10 volte reinserendo i biglietti
...
`r i2 <- 1;item() ` Calcolare la probabilità di estrarre una pallina Rossa
...
`r i2 <- i2 +1;item() ` Calcolare la probabilità di estrarre una pallina timbrata con $\mbox{A}$
...
`r i2 <- i2 +1;item() ` Gli eventi $A$ ed $R$ sono indipendenti?
...
`r i2 <- i2 +1;item() ` Calcolare la probabilità di estrarre una pallina verde
...
`r i2 <- i2 +1;item() ` Gli eventi $V$ ed $R$ sono indipendenti?
...
`r i2 <- i2 +1;item() ` Gli eventi $V$ ed $A$ sono indipendenti?
...
`r i2 <- i2 +1;item() ` Calcolare $P(N)$ (probabilità di estrarre nera) e $P(B)$ (probabilità di estrarre una pallina timbrata con B)
...
`r i2 <- i2 +1;item() ` Gli eventi $B$ ed $N$ sono indipendenti?
...
`r i2 <- i2 +1;item() ` Gli eventi $C$ (estrarre una pallina con C) ed $N$ sono indipendenti?
...
`r i2 <- i2 +1;item() ` Gli eventi $B$ ed $N$ sono incompatibili?
...
`r i2 <- i2+1;item()` Sapendo che è uscita una pallina marcata con A, qual è la probabilità che all'inizio del gioco sia stata estratta la pallina Rossa?
...
`r i2 <- +1;item()` si estrae 3 volte __con__ reintroduzione.
...
`r i2 <- i2+1;item()` si estrae 3 volte __senza__ reintroduzione.
Calcolare la probabilità di aver 3 colori diversi.
...
`r i2 <- i2+1;item()` Calcolare la probabilità che dall'urna $A$ sia uscito $\mbox{+1}$, __dato__ che
la somma fa 1
