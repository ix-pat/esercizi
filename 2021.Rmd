...
`r i2 <- i2+1;item()` (**Punti 3**) Qual è il numero di imprese con utile negativo?
...
`r i2 <- i2+1;item()` (**Punti 2**) (**Punti 2**) L'utile medio è pari a $\bar x=`r mean(samp)`$, e la sua standard deviation
...
`r i2 <- i2+1;item()` (**Punti 14**) Calcolare la probabilità che le persone **totali** ($C_1+...+C_4$) in fila al supermercato a mezzogiorno, siano almeno due.
...
`r i2 <- i2+1;item()` (**Punti 3**) Qual è la probabilità di avere esattamente _due_ casse su _quattro_ senza fila?

::: {.sol data-latex=""}
Posto `r p <- dpois(0,0.5)`
$$
\pi = P(C_i=0)=\frac {0.5}{0!}e^{-0.5}=`r p`
$$
la VC $X$ che conta il numero di casse con zero persone in fila su 4
$$
X\sim\text{Binom}(4,`r p`)
$$
e quindi 
$$
P(X=2)={4\choose 2}`r p`^2(1-`r p`)^{4-2}=`r dbinom(2,4,p)`
$$
:::

`r i2 <- i2+1;item()` (**Punti 3**) Quando due eventi $A$ e $B$ si dicono _indipendenti_ e quando _incompatibili_?
...
`r i2 <- i2+1;item()` (**Punti 2**) Se $X_1\sim N(2,1)$, $X_2\sim N(1,1)$ e $X_3\sim N(1,1)$, 
...
`r i2 <- i2+1;item()` (**Punti 3**) Sia $h$ uno stimatore per theta, tale che
$$
...
`r i2 <- i2+1;item()` (**Punti 3**) Si sono osservati due gruppi di dati quantitativi e si è osservato, $\hat\mu_1=10.2$ e $\hat\mu_2=15.6$. Posto a test
$$
\begin{cases}
H_0:\mu_1=\mu_2\\
H_1:\mu_1\ne \mu_2
...
Si consideri il modello di regressione dove $Y$ viene spiegata da $X$
  ...
  `r i2 <- i2+1;item()` (**Punti 14**) Prevedere il voto nel test di scienze per uno studente che ha ottenuto 6 nel test di matematica.
...
:::
  ...
  `r i2 <- i2+1;item()` (**Punti 3**) Calcolare la percentuale di varianza spiegata dal modello.
...
:::
  ...
  `r i2 <- i2+1;item()` (**Punti 3**) Discutere il qq-plot dei residui
```{r,fig.asp=1}
...
:::
 ...
 `r i2 <- i2+1;item()` (**Punti 2**) Cosa vuol dire che $r$ è un numero puro?
...
`r i2 <- i2+1;item()` (**Punti 14**) Calcolare il valore approssimato del 25-esimo percentile.
...
`r i2 <- i2+1;item()` (**Punti 3**) Analizzando l'istogramma, individuare il valore della media aritmetica e della mediana.
...
`r i2 <- i2+1;item()` (**Punti 2**) Qual è la percentuale di dati compresa tra il 25-esimo e il 75-esimo percentile?
...
X_L=X_E-X_U.
$$
`r i2 <- i2+1;item()` (**Punti 14**) Calcolare la probabilità che la variazione di livello sia negativa ($X_L<0$).
...
`r i2 <- i2+1;item()` (**Punti 3**) Nell'ipotesi di indipendenza tra i giorni, calcolare la probabilità di avere esattamente _due_ giorni su _cinque_ con livello negativo.  `r p <- pnorm(0,mm,sqrt(ss))`
...
$$
X\sim\text{Binom}(5,`r p`)
$$
e quindi 
$$
P(X=2)={5\choose 2}`r p`^2(1-`r p`)^{5-2}=`r dbinom(2,5,p)`
$$
:::


`r i2 <- i2+1;item()` (**Punti 3**) Se $P(A)=0.6$ e $P(B)=0.8$, $A$ e $B$ possono essere incompatibili?
...
`r i2 <- i2+1;item()` (**Punti 2**) Se $X\sim N(\mu_X,\sigma^2_X)$ come si distribuisce 
...
`r i2 <- i2+1;item()` (**Punti 3**) Siano $X_1,...,X_n$ $n$ VC IID, replicazioni di $X\sim \mathscr{L}(\theta)$ e sia $\hat\theta$ lo stimatore di massima verosimiglianza per $\theta$, 
$\hat\theta$ è corretto?
...
Sotto ipotesi di omogeneità, testare all'`r ac`% l'ipotesi che il reddito medio femminile nel comune $`r a`$ sia uguale a quello del comune $`r b`$, contro l'alternativa che siano **diversi**.

::: {.sol data-latex=""}
```{r,results='asis'}
cat(ttest_2c_om(mu1 = mu1,mu2 = mu2,s1h = s1h,s2h = s2h,n1 = n1,n2 = n2,h1 = h1,alpha = alpha,a = a,b = b))
```
:::
...
Si consideri il modello di regressione dove $Y$ viene spiegata da $X$
  ...
  `r i2 <- i2+1;item()` (**Punti 14**) Si osservino le prime 5 coppie di dati
```{r}
...
:::
  ...
  `r i2 <- i2+1;item()` (**Punti 3**) Discutere il diagramma dei residui
```{r}
...
`r i2 <- i2+1;item()` (**Punti 2**) Quando in un modello di regressione lineare un punto è considerato **influente**?
...
`r i2 <- i2+1;item()` (**Punti 3**) Calcolare la percentuale approssimata di aziende 
...
`r i2 <- i2+1;item()` (**Punti 14**) Calcolare la probabilità che la somma dei numeri sia maggiore uguale a 2.
...
`r i2 <- i2+1;item()` (**Punti 3**) Calcolare la probabilità che la somma sia maggiore uguale a 1 dato che dall'urna $A$ è uscito $\fbox{0}$.
...
`r i2 <- i2+1;item()` (**Punti 3**) Sia $X\sim \text{Pois}(10)$ e $Y\sim\text{Binom}(n,0.5)$.
Sia
$$
W=X-Y
$$
...
`r i2 <- i2+1;item()` (**Punti 2**) Siano $X_1,...,X_6$, $6$ variabili casuali IID,
tali che
$$
...
(**Punti 14**) Si lancia un dado perfetto, si vince se esce un numero maggiore o uguale a 5.
...
`r i2 <- i2+1;item()` (**Punti 3**) Definire la funzione di verosimiglianza.
...
`r i2 <- i2+1;item()` (**Punti 3**) Si sono osservati due gruppi di dati quantitativi e si è osservato, $\hat\mu_1=10.2$, $\hat\sigma_1=1.12$ e $\hat\mu_2=15.6$, $\hat\sigma_2=3.72$. Posto a test
$$
\begin{cases}
H_0:\sigma_1=\sigma_2\\
H_1:\sigma_1\ne \sigma_2
...
(**Punti 14**) In uno studio sui consumi sono stati intervistati $n=25$ individui 
...
Si consideri il modello di regressione dove $Y$ viene spiegata da $X$
  ...
  `r i2 <- i2+1;item()` (**Punti 14**) Prevedere il consumo per un individuo che guadagna $x=`r mean(x)`$ e
...
:::
  ...
  `r i2 <- i2+1;item()` (**Punti 3**) Quale delle due previsioni, per $x=`r mean(x)`$ e per $x=12.3$,
...
:::
  ...
  `r i2 <- i2+1;item()` (**Punti 3**) Calcolare le quantità $TSS$, $RSS$ e $ESS$.
...
:::
 ...
 `r i2 <- i2+1;item()` (**Punti 2**) Cosa vuol dire che $r$ è invariante ai cambiamenti di scala?
...
`r i2 <- i2+1;item()`  (**Punti 14**) Individuare il 25-esimo, 50-esimo e l'75-esimo percentile
...
`r i2 <- i2+1;item()` (**Punti 3**) La somma dei dati è `r sum(samp)`. Come dobbiamo aspettarci l'istogramma di densità?
...
`r i2 <- i2+1;item()` (**Punti 14**) Qual è la probabilità di vincere almeno 5 volte su 6 estrazioni?
...
`r i2 <- i2+1;item()` (**Punti 3**) Calcolare la probabilità di vincere la prima volta alla quarta estrazione.
...
`r i2 <- i2+1;item()` (**Punti 3**) Se $A$ e $B$ sono due eventi tali che, $P(A)>0$, $P(B)>0$, $P(A\cap B)=0$. 
...
`r i2 <- i2+1;item()` (**Punti 2**) Sia $X\sim N(3.2,(1.1)^2)$ e sia $Y\sim\chi_{n-1}^2$, posto
$$
...
(**Punti 14**) Un'urna contiene un numero imprecisato di palline numerate. Si conoscono solo
...
`r i2 <- i2+1;item()` (**Punti 3**) Scrivere la funzione di verosimiglianza di una Poisson.
...
`r i2 <- i2+1;item()` (**Punti 3**) Definire il $p_{\text{value}}$, la probabilità di significatività osservata.
...
(**Punti 14**) In uno studio sulle preferenze tra canali televisivi, vengono analizzati `r n<-92; n`
...
`r i2 <- i2+1;item()` (**Punti 14**) Stimare il modello di regressione dove la qualità della vita è spiegata dagli investimenti provinciali.
...
`r i2 <- i2+1;item()` (**Punti 3**) Ricavare le quantità $TSS$, $RSS$ e $ESS$.
...
`r i2 <- i2+1;item()` (**Punti 2**) Cosa significa che gli stimatori di massima dei minimi quadrati $\hat\beta_0$ e $\hat\beta_1$ sono _BLUE_?
...
`r i2 <- i2+1;item()` (**Punti 3**) Che relazione dobbiamo aspettarci tra media e mediana?
...
`r i2 <- i2+1;item()` (**Punti 2**) La varianza dei dati è pari a $\sigma^2=`r s2c(samp)`$. Nell'ipotesi che i costi per metano aumentassero del 10%, quanto varrebbe la varianza?
...
`r i2 <- i2+1;item()` (**Punti 14**) Calcolare la probabilità che il *totale* delle persone in transito nella stazione, in una data ora, sia maggiore o uguale a due.
...
`r i2 <- i2+1;item()` (**Punti 3**) Calcolare la probabilità di osservare una persona su un binario, una persona su un altro binario e nessuna persona sul rimanetene.
...
`r i2 <- i2+1;item()` (**Punti 3**) Siano $X_1,...,X_n$, $n$ VC IID, tali che
$$
X_i\sim N(0,1).
$$
Posto
...
`r i2 <- i2+1;item()` (**Punti 2**) Siano $X_1\sim \text{Binom}(5,0.3)$ e $X_2\sim \text{Binom}(3,0.3)$.
Come si distribuisce,
$$X_1+X_2\sim ~~?$$
...
`r i2 <- i2+1;item()` (**Punti 3**) Se $h$ è uno stimatore tale che la sua distorsione va a zero per $n$ che tende all'infinito:
$$
...
`r i2 <- i2+1;item()` (**Punti 3**) Definire la funzione di verosimiglianza della Bernoulli.
...
`r i2 <- i2+1;item()` (**Punti 3**) Un dado, che non sappiamo se è perfetto oppure no, viene lanciato 40 volte.
...
`r i2 <- i2+1;item()` (**Punti 10**) Costruire un intervallo di confidenza al 99% per il tempo medio di azione.
...
`r i2 <- i2+1;item()` (**Punti 4**) È ben noto, da studi pregressi, che quella tipologia di farmaci ha un tempo medio 
...
h1 <- "<"
alpha <- 0.01
cat(ttest_mu(muh = muh,sh = s,n = n,mu0 = mu0,h1 = h1,alpha = alpha))
```
:::
...
```
  ...
  `r i2 <- i2+1;item()` (**Punti 14**) Stimare il modello di regressione dove il risparmio è funzione del reddito e quello in cui il reddito è funzione del risparmio.
...
:::
  ...
  `r i2 <- i2+1;item()` (**Punti 3**) I due modelli si adattano bene ai dati?
...
:::
  ...
  `r i2 <- i2+1;item()` (**Punti 3**) Discutere il diagramma dei residui del modello di regressione dove $Y$ viene spiegata da $X$.
```{r}
...
:::
 ...
 `r i2 <- i2+1;item()` (**Punti 2**) Se ogni individuo risparmiasse 10€ in più al mese, quanto varrebbe $r$?
...
`r i2 <- i2+1;item()`  (**Punti 14**) Calcolare la colonna delle densità percentuali.
...
`r i2 <- i2+1;item()`  (**Punti 3**) Calcolare il numero approssimato di aziende con consumo di gas inferiore al 25-esimo percentile $x_{0.25}$. 
...
`r i2 <- i2+1;item()` (**Punti 2**) È vero che la media aritmetica minimizza la somma dei valori assoluti degli scarti? Perché?
...
`r i2 <- i2+1;item()` (**Punti 14**) Qual è la probabilità di vincere almeno 1 volta su 5 giocate?
...
`r i2 <- i2+1;item()` (**Punti 3**) Per giocare si devono puntare due euro ad ogni giocata. Se vinciamo ci vengono dati 9€, i 2€ giocati più 7€ di vincita. Quindi:
...
`r i2 <- i2+1;item()` (**Punti 3**) Se $A$ e $B$ sono due eventi tali che, $P(A)=0.5$, $P(B)=0.4$ e $P(A\cup B)=0.58$. $A$ e $B$ sono indipendenti? Perché?
...
`r i2 <- i2+1;item()` (**Punti 2**) Siano $Y_1,...,Y_n$, $n$ VA IID, tali che, $Y_i\sim\chi_{1}^2$, posto
$$
X=\sum_{i=1}^n Y_i
$$
...
`r i2 <- i2+1;item()` (**Punti 3**) Sia $\hat\sigma^2$ lo stimatore di massima verosimiglianza per il parametro $\sigma^2$ della normale. $\hat\sigma^2$ è corretto? Quanto vale la sua distorsione?
...
`r i2 <- i2+1;item()` (**Punti 3**) Descrivere la tavola della verità di un test.

`r i2 <- i2+1;item()` (**Punti 3**) In uno studio clinico si è osservato l'effeto sul numero di anticorpi su due gruppi trattati con un farmaco differente. Si è osservato $\hat\mu_1=10.2$, $\hat\sigma_1=1.12$ e $\hat\mu_2=15.6$, $\hat\sigma_2=3.72$. Posto a test
$$
\begin{cases}
H_0:\sigma_1=\sigma_2\\
H_1:\sigma_1\ne \sigma_2
...
`r i2 <- i2+1;item()` (**Punti 10**) Costruire un intervallo di confidenza al 95% per il consumo medio.
...
`r i2 <- i2+1;item()` (**Punti 4**) È noto che quella fascia di motori ha un consuo medio pari $23.9~\text{Km/litro}$. Testare al livello di significatività dell'5% che il nuovo motore consumi come gli altri contro l'alternativa che abbia consumi _diversi_.
...
`r i2 <- i2+1;item()` (**Punti 14**) Stimare il modello di regressione dove l'indice di stress ($Y$) è spiegato da $X$ e quello dove $Y$ è spiegato da $W$.
...
`r i2 <- i2+1;item()` (**Punti 3**) Quale dei due modelli è più affidabile?
...
`r i2 <- i2+1;item()` (**Punti 3**) Considerata la scomposizine della TSS di $X$ rispetto ad $Y$.
$$
TSS = ESS + RSS
$$
...
`r i2 <- i2+1;item()` (**Punti 2**) Sotto ipotesi di normalità dei residui, come sono distribuiti gli stimatori dei mini quadrati $\hat\beta_0$ e $\hat\beta_1$?
...
`r i2 <- i2+1;item()` (**Punti 3**) Quante aziende, approssimativamente, hanno un bilancio inferiore a 1.5?
...
`r i2 <- i2+1;item()` (**Punti 2**) La varianza dei dati è pari a $\sigma^2=`r s2c(samp)`$. Nell'ipotesi che tutte le aziende aumentassero il loro bilancio di 10 mila ero, quanto varrebbe la varianza dei così trasformati?
...
`r i2 <- i2+1;item()` (**Punti 14**) Quanto vale $P\Big((A\cap B)\cap C\Big)$? 
...
`r i2 <- i2+1;item()` (**Punti 3**) Posto $Y=X_1-X_2$, calcolare $P(Y<-1)$.
...
`r i2 <- i2+1;item()` (**Punti 3**) Siano $X_1,...,X_n$, $n$ VC IID, tali che $X_i\sim \text{Ber}(\pi)$, posto
\[
 X = X_1+...+X_n
\]
come si distribuisce $X$? 

`r i2 <- i2+1;item()` (**Punti 2**) Sia $X\sim \text{Pois}(1.5)$, per quali valori $x$, $F(x)$, la funzione di ripartizione di $X$ è minore o uguale a 0.5? Ovvero, per quali $x$ la seguente è rispettata $F(x)\le 0.5$?
...
`r i2 <- i2+1;item()` (**Punti 2**) Enunciare il teorema centrale del limite per la somma. 

`r i2 <- i2+1;item()` (**Punti 14**) Il numero di incidenti giornalieri sul lavoro del comparto $A$ è distribuito come una
...
`r i2 <- i2+1;item()` (**Punti 3**) Siano $h_1$ e $h_2$ due stimatori corretti per $\theta$, cosa significa che $h_1$ è più efficiente di $h_2$? 

`r i2 <- i2+1;item()` (**Punti 3**) Definire la funzione di verosimiglianza della Poisson. 

`r i2 <- i2+1;item()` (**Punti 3**) Una moneta, che non sappiamo se è perfetta oppure no, viene lanciata 40 volte.
...
`r i2 <- i2+1;item()` (**Punti 2**) In caso di campionamento casuale semplice **senza reintroduzione**
...
`r i2 <- i2+1;item()` (**Punti 7**) Determinare un intervallo di confidenza al 95%
...
`r i2 <- i2+1;item()` (**Punti 7**) Una indagine più vasta dell'anno precedente
...
`r i2 <- i2+1;item()` (**Punti 2**) Se in un modello di regressione si conoscono $\hat\beta_1$, $r^2$ e $\hat\sigma_X$ è possibile ricavare $\hat\sigma_Y$? In che modo?
...
`r i2 <- i2+1;item()` (**Punti 2**) Cosa significa che il coefficiente di correlazione è invariante alle trasformazioni lineari?
