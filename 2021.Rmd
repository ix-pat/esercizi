...
`r punti_p(nex = T)` Sia $\hat\theta$ uno stimatore per theta, tale che
$$
...
::: {.sol data-latex=""}
$\hat\theta$ **non è corretto**, infatti
$$
E(\hat\theta)=\theta+\frac\theta {\sqrt{ n}}\neq\theta
$$
$\hat\theta$ **è asintoticamente corretto**, infatti
$$
\lim_{n\to\infty}E(\hat\theta)=\lim_{n\to\infty}\left(\theta+\frac\theta {\sqrt{ n}}\right)=\theta+0=\theta
$$
:::

`r punti_p()` Siano $\hat\theta_1$ e $\hat\theta_2$ due stimatori per $\theta$, tali che:
\begin{eqnarray*}
...
::: {.sol data-latex=""}
$\hat\theta_1$ **è più efficiente** di $\hat\theta_2$, infatti
\begin{eqnarray*}
MSE(\hat\theta_1) &=&   \frac\theta n\\
...
`r punti_p(nex = T)` Sia $\hat\theta$ uno stimatore per $\theta$, tale che
$$
...
`r punti_p(nex = T)` Se $\hat\theta$ è uno stimatore tale che la sua distorsione va a zero per $n$ che tende all'infinito:
$$
...
`r punti_p(nex = T)` Siano $\hat\theta_1$ e $\hat\theta_2$ due stimatori corretti per $\theta$, cosa significa che $\hat\theta_1$ è più efficiente di $\hat\theta_2$? 
