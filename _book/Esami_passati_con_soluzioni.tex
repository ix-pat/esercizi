...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-9-1} \end{center}
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-12-1} \end{center}
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-14-1} \end{center}
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-15-1} \end{center}

\end{sol}

\hypertarget{varianti-con-contesto-eserciziario}{%
\section{Varianti con contesto (eserciziario)}\label{varianti-con-contesto-eserciziario}}

\hypertarget{esercizio-conteggi}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-17-1} \end{center}
...
4.b Qual è il numero di imprese che hanno addetti
nella classe \([0,\, 9]\)?
...
4.c In quale classe si trova il 15\(^{o}\) percentile?
...
4.e In quale classe si trova il 75\(^{o}\) percentile?
...
4.f Quale relazione ci si deve attendere
...
4.g Determinare il valore approssimato della mediana,
...
\hypertarget{esercizio-dati-continui}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-21-1} \end{center}
...
5.c Determinare il valore approssimato della mediana,
...
\hypertarget{esercizio-variante-1}{%
...
\hypertarget{esercizio-variante-2}{%
...
7.a Disegnare l'istogramma (delle densità relative) della
...
\hypertarget{esercizio-variante-3}{%
...
8.a Disegnare l'istogramma (delle densità percentuali) della
distribuzione della cilindrata delle auto in circolazione.
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-27-1} \end{center}

\end{sol}

\hypertarget{esercizio-dati-non-ordinati}{%
...
9.a il campo di variazione,
9.b la mediana,
9.c la media sapendo che la somma è pari a 187.64 per l'impianto A e 227.07 per l'impianto B.
...
9.d \(\,\)il primo quartile,
9.e \(\,\)il terzo quartile,
9.f \(\,\)la differenza interquartile.
...
\item
  ...
  al soggetto nella posizione successiva a \(\lfloor n\, p \rfloor = \lfloor 20 \,\times\, 0.25 \rfloor = 5\), ossia \(x_{A;\, 0.25} = x_{A;\ (\lfloor n\, p \rfloor + 1)} = x_{A;\ (6)} = \textbf{7.50}\)
  e \(x_{B;\, 0.25} = x_{B;\ (\lfloor n\, p \rfloor + 1)} = x_{B;\ (6)} = \textbf{9.17}\).
...
\item
  ...
  al soggetto nella posizione successiva a \(\lfloor n\, p \rfloor = \lfloor 20 \,\times\, 0.75 \rfloor = 15\), ossia \(x_{A;\, 0.75} = x_{A;\ (\lfloor n\, p \rfloor + 1)} = x_{A;\ (16)} = \textbf{11.42}\)
  e \(x_{B;\, 0.75} = x_{B;\ (\lfloor n\, p \rfloor + 1)} = x_{B;\ (16)} = \textbf{14.25}\).
...
9.g Calcolare la varianza, sapendo che
\(\sum_{i=1}^{20} x_{A;\, i}^{2} = 2064.08\) e
\(\sum_{i=1}^{20} x_{B;\, i}^{2} = 3077.31\).
9.h Calcolare la deviazione standard.
...
9.i La distribuzione è asimmetrica?
Se sì, di quale tipo di asimmetria si tratta?
9.j Dalle risposte date, emergono differenze tra i due impianti?
...
9.k Calcolare la probabilità di ottenere due biglietti
con il numero 1.
...
9.l La probabilità che almeno uno dei biglietti sia
...
9.m La probabilità che la somma dei numeri riportati sui
...
9.a Qual è la probabilità di trovare la combinazione giusta
...
9.b Qual è la probabilità di trovare la combinazione giusta
estraendo completamente a caso le tre cifre per un massimo di
...
9.a Quali sono le coppie di palline che formano l'unione
...
9.b Quali sono le coppie di palline che formano l'intersezione
...
\hypertarget{applicazione-regole}{%
...
9.b Calcolare la probabilità che si verifichi B,
...
9.a Qual è la probabilità che lo studente
...
9.b Qual è la probabilità che lo studente superi
...
9.a Calcolare la probabilità che Giulio non debba pagare
la multa (evento \(C\)).
...
9.b Calcolare la probabilità che Giulio in tre giorni
...
9.b Determinare la probabilità di un
punto del supporto.
...
9.d Qual è la probabilità che la somma (\(S\))
...
9.e Calcolare la probabilità che la SOMMA (\(S\))
...
\hypertarget{scatola-e-biglietti}{%
...
9.a Ricavare la distribuzione di probabilità di \(X=A_{1}+B_{1}\).
...
9.b Calcolare la probabilità dell'evento \(\{A_{1}>B_{1}\}\).
...
9.c Si ripete l'esperimento 10 volte reinserendo i biglietti
...
\hypertarget{urna-con-colori-e-lettere-diverse}{%
...
9.b Calcolare la probabilità di estrarre una pallina timbrata con \(\mbox{A}\)
...
\hypertarget{urne-che-portano-ad-altre-urne}{%
...
\hypertarget{estrazioni-con-e-senza-reintroduzione}{%
...
9.a si estrae 3 volte \textbf{con} reintroduzione.
...
9.b si estrae 3 volte \textbf{senza} reintroduzione.
Calcolare la probabilità di aver 3 colori diversi.
...
\hypertarget{urne-e-palline-numerate}{%
...
9.a Sia \(S\) consideri la somma dei numeri estratti, calcolare la probabilità che
\(S=0\).
...
9.b Calcolare la probabilità che dall'urna \(A\) sia uscito \(\mbox{+1}\), \textbf{dato} che
la somma fa 1
...
\hypertarget{esercizi-di-probabilituxe0-e-variabili-casuali}{%
...
9.c (\textbf{Punti 14}) Calcolare la probabilità di avere al massimo 2 automobili al casello \(C\).
...
9.d (\textbf{Punti 3}) Qual è la varianza della VC che conta il
...
9.e (\textbf{Punti 3}) Se \(X\sim\text{Binom}(15,0.3)\), qual è il supporto di \(X\)?
...
9.f (\textbf{Punti 2}) Se \(X\sim N(0,1)\) e \(Y\sim \chi^2_5\),
...
9.a (\textbf{Punti 14}) Calcolare la probabilità di avere un rendimento negativo.
...
9.b (\textbf{Punti 3}) Sotto ipotesi di indipendenza tra gli anni, qual è la probabilità che il portafoglio abbia rendimento negativo per 3 anni di seguito?
...
9.c (\textbf{Punti 3}) Se \(X\sim\text{Pois}(15.3)\), qual è la varianza di \(X\)?
...
9.d (\textbf{Punti 2}) Se \(X_1\sim N(0,1)\), \(X_2\sim N(0,1)\) e \(X_2\sim N(0,1)\)
...
9.a (\textbf{Punti 14}) Qual è la probabilità di avere almeno 2 bussolotti rossi su 6 estrazioni?
...
9.b (\textbf{Punti 3}) Quali sono valore atteso e varianza della VC che conta il numero di palline Rosse su 6 estrazioni con reintroduzione dall'urna \(A\)?
...
9.c (\textbf{Punti 3}) Se \(X\sim N(\mu_X,\sigma_X^2)\) e \(Y\sim N(\mu_Y,\sigma_Y^2)\), come si distribuisce
\[W=X-Y ~~~?\]
...
9.d (\textbf{Punti 2}) Si lancia una moneta perfetta (\(P(T)=P(C)=\frac 12\)).
...
9.a (\textbf{Punti 14}) Si estrae \(n=2\) volte \textbf{senza} reintroduzione. Qual è la probabilità di ottenere due colori diversi in 2 estrazioni? (esempio: prima bianco poi rosso \emph{oppure} prima nero poi bianco oppure\ldots)
...
9.b (\textbf{Punti 3}) Si ricompone l'urna \(U\) e si estrae una volta, si assegna
...
9.c (\textbf{Punti 3}) La varianza di una VC \(X\) può essere zero?
...
9.d (\textbf{Punti 2}) Se \(X\sim\text{Bin}(10;0.3)\) e \(Y\sim\text{Pois}(3.23)\),
...
\hypertarget{esercizio-sul-teorema-di-bayes}{%
...
9.b Qual è la probabilità di Sergio che dall'urna \(U_1\) sia stata estratta la palline etichettata con 3?
...
9.c Qual è distribuzione aggiornata su \(\pi\) in base al fatto che \(X=2\)?
...
9.d Costruire le distribuzioni condizionate di \(X\) a \(\pi\).
...
9.e Costruire la distribuzione doppia \textbf{congiunta} di tutte le possibili combinazioni e le relative probabilità.
...
9.f Costruire le distribuzioni condizionate di \(\pi\) ad \(X\).
...
\hypertarget{esercizi-sul-tlc}{%
\chapter{Esercizi sul TLC}\label{esercizi-sul-tlc}}

\hypertarget{una-vc-qualunque-somma-s_n}{%
...
\hypertarget{una-vc-qualunque-media-barx}{%
...
\hypertarget{bernoulli-somma-s_n}{%
...
\hypertarget{bernoulli-proporzione-widehatpi}{%
...
\hypertarget{urne-proporzione-widehatpi}{%
...
\hypertarget{poisson-media-barx}{%
...
\hypertarget{poisson-somma-s_n-1}{%
...
\hypertarget{proporzione-poisson-widehatpi}{%
...
\hypertarget{esercizi-su-test-e-intervalli-di-confidenza}{%
\chapter{Esercizi su Test e Intervalli di Confidenza}\label{esercizi-su-test-e-intervalli-di-confidenza}}

\hypertarget{t-test-a-due-campioni}{%
...
\[
     S^2_\text{ A }=\frac{n_\text{ A }}{n_\text{ A }-1}\hat\sigma^2_\text{ A }=\frac{ 10 }{ 10 -1} 5.4 ^2= 32.4  \qquad
     S^2_\text{ B }=\frac{n_\text{ B }}{n_\text{ B }-1}\hat\sigma^2_\text{ B }=\frac{ 20 }{ 20 -1} 5.9 ^2= 36.64 
   \]
...
\begin{eqnarray*}
   ...
   t_{\text{obs}}
   &=& \frac{ ( 95 -  92 )} {\sqrt{\frac{ 32.4 }{ 10 }+\frac{ 36.64 }{ 20 }}}
   =   1.332 \, .
   \end{eqnarray*}

\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 10 + 20 -2);\, 0.01 } = 2.467\).
...
Il \(p_{\text{value}}\) è
\[ P(T_{n1+n2-2}>t_{\text{obs}})=P(T_{n1+n2-2}> 1.332 )= 0.09678 \]

\end{sol}

\hypertarget{un-campione-idc-e-z-test}{%
\section{Un campione: IdC e z-test}\label{un-campione-idc-e-z-test}}
...
\tightlist
\item
  L'indagine dell'anno precedente, condotta
  su un campione molto pi\textquotesingle u numeroso, mostrava un costo medio
...
\[z_{\text{obs}} =  2.062 > z_{ 0.05 } =  1.645 \] CONCLUSIONE: i dati non sono coerenti con \(H_{0}\) al LdS del 5 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-62-1} \end{center}

Il \(p_{\text{value}}\) è \[ P(Z>z_{\text{obs}})=P(Z> 2.06 )= 0.01962 \]

\end{sol}

\hypertarget{un-campione-idc-e-t-test}{%
\section{Un campione: IdC e t-test}\label{un-campione-idc-e-t-test}}
...
\[
     S =\sqrt{\frac {n}{n-1}}\cdot\hat\sigma =
     \sqrt{\frac { 20 }{ 19 }}\cdot 10.88 = 11.1579 \]
\begin{eqnarray*}
  Idc: & &  \hat\mu \pm  t_{n-1;\alpha/2} \times \frac{ S }{\sqrt{n}} \\
     & &  48.5 \pm  2.093 \times \frac{ 10.8754 }{\sqrt{ 20 }} \\
     & &  48.5 \pm  2.093 \times  2.495 \\
     & & [ 43.28 ,  53.72 ]
  \end{eqnarray*}
...
\tightlist
\item
  è noto che l'età media dei politici europei
  è di 55 anni.
...
\begin{eqnarray*}
   ...
   \end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 20 -1);\, 0.01 } = -2.539\).
\[t_{\text{obs}} =  -2.673   <  t_{ 19 ;\,  0.01 } =  -2.539 \]
CONCLUSIONE: i dati non sono coerenti con \(H_{0}\) al LdS del 1 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-64-1} \end{center}

Il \(p_{\text{value}}\) è
\[ P(T_{n-1}<t_{\text{obs}})=P(T_{n-1}< -2.673 )= 0.007521 \]

\end{sol}

\hypertarget{un-campione-idc-e-t-test-1}{%
\section{Un campione: IdC e t-test}\label{un-campione-idc-e-t-test-1}}
...
\[
     S =\sqrt{\frac {n}{n-1}}\cdot\hat\sigma =
     \sqrt{\frac { 26 }{ 25 }}\cdot 12.24 = 12.48 \]
\begin{eqnarray*}
  Idc: & &  \hat\mu \pm  t_{n-1;\alpha/2} \times \frac{ S }{\sqrt{n}} \\
     & &  78 \pm  2.06 \times \frac{ 12.2376 }{\sqrt{ 26 }} \\
     & &  78 \pm  2.06 \times  2.448 \\
     & & [ 72.96 ,  83.04 ]
  \end{eqnarray*}
...
\tightlist
\item
  Si supponga che il punteggio medio del test in un ampio
...
\begin{eqnarray*}
   ...
   \end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 26 -1);\, 0.005 } = 2.787\).
...
\hypertarget{due-campioni-proporzione}{%
...
\(\fbox{B}\) SCELTA E CALCOLO STATISTICA-TEST, \(Z\)\[\hat\pi_\text{ A }=\frac{s_\text{ A }}{n_\text{ A }}=\frac{ 60 }{ 120 }= 0.5 \qquad
   \hat\pi_\text{ B }=\frac{s_\text{ B }}{n_\text{ B }}=\frac{ 30 }{ 120 }= 0.25 \]Calcoliamo la proporzione comune sotto \(H_0\)
\[
...
\[z_{\text{obs}} =  4   >  z_{ 0.05 } =  1.645 \]CONCLUSIONE: i dati non sono coerenti con \(H_{0}\) al LdS del 5 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-67-1} \end{center}

Il \(p_{\text{value}}\) è
\[ P(Z>z_{\text{obs}})=P(Z> 4 )= 0.00003167 \]

\end{sol}

\hypertarget{test-del-chi-quadro-per-indipendenza}{%
...
\toprule
  & Cioccolato & Fragola & Vaniglia & Limone & Tot\\
\midrule
\textbf{Uomo} & 0.087 & 0.277 & 1.212 & 0.303 & 0\\
\textbf{Donna} & 0.095 & 0.305 & 1.333 & 0.333 & 0\\
\textbf{Tot} & 0.000 & 0.000 & 0.000 & 0.000 & 0\\
\bottomrule
\end{tabular}
\end{table}
...
Essendo
\[
\chi^2_{obs}=3.945 <\chi_{1,0.05}^2=15.51
\]

allora non rifiuto \(H_0\) al lds dell'5 percento.
...
Il \(p_{\text{value}}\) è
\[
P(\chi^2_{ 8 }> \chi^2_{\text{obs}})= 0.862 
\]
...
\toprule
  & Acqua & Succo d'arancia & Bibita gassata & Tot\\
\midrule
\textbf{consuma frutta} & 0.714 & 0.227 & 2.045 & 0\\
\textbf{non consuma frutta} & 0.714 & 0.227 & 2.045 & 0\\
...
Essendo
\[
\chi^2_{obs}=5.974 <\chi_{1,0.01}^2=16.81
\]
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-71-1} \end{center}

Il \(p_{\text{value}}\) è
\[
P(\chi^2_{ 6 }> \chi^2_{\text{obs}})= 0.4261 
\]
...
\toprule
  & Sostenitore & Neutrale & Contrario & Tot\\
\midrule
\textbf{Elettori di Destra} & 0.213 & 3.972 & 1.144 & 0\\
\textbf{Elettori di Sinistra} & 0.241 & 4.501 & 1.296 & 0\\
\textbf{Tot} & 0.000 & 0.000 & 0.000 & 0\\
\bottomrule
\end{tabular}
\end{table}
...
(3-1)\times(4-1)=6
\]
\(\alpha=0.05\) e quindi \(\chi_{1,0.05}^2=12.59\)

Essendo
\[
\chi^2_{obs}=11.37 <\chi_{1,0.05}^2=12.59
\]
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-73-1} \end{center}

Il \(p_{\text{value}}\) è
\[
P(\chi^2_{ 6 }> \chi^2_{\text{obs}})= 0.07766 
\]
...
\toprule
  & Sport & Lettura & Arte & Tot\\
\midrule
\textbf{Giovani} & 4.334 & 0.025 & 4.315 & 0\\
\textbf{Adulti} & 2.070 & 6.834 & 1.997 & 0\\
\textbf{Anziani} & 0.586 & 9.933 & 19.497 & 0\\
\textbf{Tot} & 0.000 & 0.000 & 0.000 & 0\\
\bottomrule
\end{tabular}
\end{table}
...
Essendo
\[
\chi^2_{obs}=49.59 >\chi_{1,0.05}^2=16.92
\]

allora rifiuto \(H_0\) al lds dell'5 percento.

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-75-1} \end{center}

Il \(p_{\text{value}}\) è
\[
P(\chi^2_{ 9 }> \chi^2_{\text{obs}})= 0.0000001286 
\]

\end{sol}

\hypertarget{test-del-chi-quadro-per-conformituxe0}{%
...
Il chi quadro osservato è 10.13 è maggiore di \(\chi^2_{2;0.05}=5.99\), e quindi \textbf{rifiuto} \(H_0\), al livello di significatività del 5\(\%\).

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-77-1} \end{center}

Il \(p_{\text{value}}\) è
\[
P(\chi^2_{ 2 }> \chi^2_{\text{obs}})= 0.006304 
\]
...
Il chi quadro osservato è 50.15 è maggiore di \(\chi^2_{4;0.05}=9.488\), e quindi \textbf{rifiuto} \(H_0\), al livello di significatività del 5\(\%\).

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-79-1} \end{center}

Il \(p_{\text{value}}\) è
\[
P(\chi^2_{ 4 }> \chi^2_{\text{obs}})= 0.0000000003367 
\]

\end{sol}

\hypertarget{esericizi-sulla-regressione}{%
\chapter{Esericizi sulla Regressione}\label{esericizi-sulla-regressione}}

\hypertarget{esercizio-dati-maternituxe0-usa}{%
...
\hypertarget{la-rappresentazione-dei-dati}{%
\subsection{La rappresentazione dei dati}\label{la-rappresentazione-dei-dati}}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-81-1} \end{center}
...
\hat\sigma_\varepsilon^2 & = \hat\sigma_Y^2(1-r^2)=29.6 &
S_\varepsilon^2 & = \frac{n}{n-2}\hat\sigma_\varepsilon^2 = 30.8089\\
\hat\sigma_\varepsilon & = \hat\sigma_Y\sqrt{(1-r^2)}=5.4406 & \qquad
S_\varepsilon & = \sqrt{\frac{n}{n-2}}\hat\sigma_\varepsilon = 5.5506\\
\end{alignat*}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-82-1} \end{center}
...
ossia, è composto da un quantitativo fisso di \(4.2673\) di tasso di natalità delle minorenni in un ipotetico
stato a povertà zero (\(x=0\)), a cui si aggiunge un incremento di \(1.3733\) per ogni incremento unitario del tasso di povertà.
...
\end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{(51-2);\, 0.025} = 2.01\).
\[t_{\text{obs}} = 1.687 < t_{49;\, 0.025} = 2.01\]
CONCLUSIONE: i dati \textbf{sono} coerenti con \(H_{0}\) al LdS del 2.5\%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-85-1} \end{center}
...
\end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{(51-2);\, 0.01} = 2.403\).
\[t_{\text{obs}} = 7.481 > t_{49;\, 0.01} = 2.403\]
CONCLUSIONE: i dati \textbf{non} sono coerenti con \(H_{0}\) al LdS del 1\%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-86-1} \end{center}
...
\tightlist
\item
  Calcolare i parametri \(\beta_{0}\) e \(\beta_{1}\) della
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-89-1} \end{center}
...
\tightlist
\item
  Calcolare un indicatore che sintetizzi l'ordine
...
\tightlist
\item
  Prevedere il valore dell'indice industriale per un valore
...
\tightlist
\item
  Dal diagramma di dispersione sotto riportato,
  spiegare se la retta di regressione è adeguata o no a
  rappresentare il fenomeno.
\end{enumerate}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-91-1} \end{center}
...
\tightlist
\item
  Si consideri il diagramma dei residui sotto riportato.
...
\end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{(16-2);\, 0.01} = 2.602\).
\[t_{\text{obs}} = 6.584 > t_{14;\, 0.01} = 2.602\]
CONCLUSIONE: i dati \textbf{non} sono coerenti con \(H_{0}\) al LdS del 1\%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-94-1} \end{center}
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-95-1} \end{center}
...
\tightlist
\item
  Calcolare i parametri \(\beta_{0}\) e \(\beta_{1}\)
...
\begin{sol}
\begin{eqnarray*}
r&=&\frac{\text{cov}(X,Y)}{\sigma_X\sigma_Y}=\frac{ 1.562 }{ 3.162 \times 3.46 }= 0.1427 \\r^2&=& 0.02036 < 0.75
\end{eqnarray*}
Il modello \textbf{non} si adatta bene ai dati.
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-100-1} \end{center}
...
\tightlist
\item
  Prevedere il valore relativo a \(x=5\)
...
\end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{(11-2);\, 0.025} = 2.262\).
\[t_{\text{obs}} = 0.4327 < t_{9;\, 0.025} = 2.262\]
CONCLUSIONE: i dati \textbf{sono} coerenti con \(H_{0}\) al LdS del 2.5\%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-101-1} \end{center}
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-102-1} \end{center}
...
\begin{sol}
\begin{eqnarray*}
r&=&\frac{\text{cov}(X,Y)}{\sigma_X\sigma_Y}=\frac{ 35.24 }{ 7.574 \times 4.733 }= 0.9831 \\r^2&=& 0.9665 > 0.75
\end{eqnarray*}
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-107-1} \end{center}
...
V(\hat\beta_{1}) &=& \frac{\sigma_{\varepsilon}^{2}} {n \hat{\sigma}^{2}_{X}} \\
\widehat{V(\hat\beta_{1})} &=& \frac{S_{\varepsilon}^{2}} {n \hat{\sigma}^{2}_{X}} \\
 &=& \frac{ 0.8562 } { 16 \times  57.36 } =  0.0009 \\
 \widehat{SE(\hat\beta_{1})}        &=&  \sqrt{ 0.0009 }\\
 &=&  0.03 
\end{eqnarray*}
...
\begin{eqnarray*}
 \frac{\hat\beta_{ 1 } - \beta_{ 1 ;H_0}} {\widehat{SE(\hat\beta_{ 1 })}}&\sim&t_{n-2}\\
   t_{\text{obs}}
&=& \frac{ ( 0.6144 -  0.5 )} { 0.03 }
 =   3.813 \, .
\end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{(16-2);\, 0.025} = 2.145\).
\[t_{\text{obs}} = 3.813 > t_{14;\, 0.025} = 2.145\]
CONCLUSIONE: i dati \textbf{non} sono coerenti con \(H_{0}\) al LdS del 2.5\%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-109-1} \end{center}
...
\end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{(16-2);\, 0.05} = -1.753\).
\[t_{\text{obs}} = -1.494 > t_{14;\, 0.05} = -1.753\]
CONCLUSIONE: i dati \textbf{sono} coerenti con \(H_{0}\) al LdS del 5\%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-110-1} \end{center}

Il \(p_{\text{value}}\) è
\[ P(T_{n-2}<t_{\text{obs}})=P(T_{n-2}< -1.494 )= 0.07873 \]
...
\begin{sol}
\begin{eqnarray*}
r&=&\frac{\text{cov}(X,Y)}{\sigma_X\sigma_Y}=\frac{ 4145 }{ 43.2 \times 102.1 }= 0.9397 \\r^2&=& 0.883 > 0.75
\end{eqnarray*}
Il modello si adatta bene ai dati.
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-115-1} \end{center}
...
\tightlist
\item
  Prevedere il valore del fatturato per un numero
...
\widehat{V(\hat\beta_{1})} &=& \frac{S_{\varepsilon}^{2}} {n \hat{\sigma}^{2}_{X}} \\
 ...
 \widehat{SE(\hat\beta_{1})}        &=&  \sqrt{ 0.0503 }\\
 &=&  0.2243 
\end{eqnarray*}
...
\end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{(15-2);\, 0.05} = 1.761\).
\[t_{\text{obs}} = 0.9841 < t_{13;\, 0.05} = 1.761\]
CONCLUSIONE: i dati \textbf{sono} coerenti con \(H_{0}\) al LdS del 5\%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-117-1} \end{center}
...
CONCLUSIONE: i dati \textbf{non} sono coerenti con \(H_{0}\) al LdS del 5\%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-118-1} \end{center}
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-122-1} \end{center}
...
\begin{sol}
\begin{eqnarray*}
r&=&\frac{\text{cov}(X,Y)}{\sigma_X\sigma_Y}=\frac{ 92.42 }{ 8.5 \times 11.43 }= 0.9517 \\r^2&=& 0.9057 > 0.75
\end{eqnarray*}
Il modello si adatta bene ai dati.
...
\tightlist
\item
  Determinare il residuo (o l'errore) derivante
...
V(\hat\beta_{1}) &=& \frac{\sigma_{\varepsilon}^{2}} {n \hat{\sigma}^{2}_{X}} \\
\widehat{V(\hat\beta_{1})} &=& \frac{S_{\varepsilon}^{2}} {n \hat{\sigma}^{2}_{X}} \\
 &=& \frac{ 14.08 } { 16 \times  72.24 } =  0.0122 \\
 \widehat{SE(\hat\beta_{1})}        &=&  \sqrt{ 0.0122 }\\
 &=&  0.1105 
\end{eqnarray*}
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-127-1} \end{center}
...
\tightlist
\item
  Qual è l'incremento di fatturato, che
...
\end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{(15-2);\, 0.025} = -2.16\).
\[t_{\text{obs}} = -3.06 < t_{13;\, 0.025} = -2.16\]
CONCLUSIONE: i dati \textbf{non} sono coerenti con \(H_{0}\) al LdS del 2.5\%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-128-1} \end{center}
...
\tightlist
\item
  Calcolare i parametri \(\beta_{0}\) e \(\beta_{1}\) della retta
...
\begin{sol}
\begin{eqnarray*}
r&=&\frac{\text{cov}(X,Y)}{\sigma_X\sigma_Y}=\frac{ 1.6 }{ 1.414 \times 1.357 }= 0.8341 \\r^2&=& 0.6957 < 0.75
\end{eqnarray*}
Il modello \textbf{non} si adatta bene ai dati.
...
\tightlist
\item
  Determinare il residuo per un voto di matematica
  uguale 6, ossia per \(x=6\).
\end{enumerate}
...
\widehat{V(\hat\beta_{1})} &=& \frac{S_{\varepsilon}^{2}} {n \hat{\sigma}^{2}_{X}} \\
 ...
 \widehat{SE(\hat\beta_{1})}        &=&  \sqrt{ 0.0933 }\\
 &=&  0.3055 
\end{eqnarray*}
...
\end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{(5-2);\, 0.005} = 5.841\).
\[t_{\text{obs}} = 2.619 < t_{3;\, 0.005} = 5.841\]
CONCLUSIONE: i dati \textbf{sono} coerenti con \(H_{0}\) al LdS del 0.5\%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-133-1} \end{center}
...
\hypertarget{anno-2021}{%
\chapter{Anno 2021}\label{anno-2021}}

\hypertarget{prova-di-statistica-20210611-1}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-136-1} \end{center}
...
1.a (\textbf{Punti 14}) Calcolare il valore approssimato della mediana.
...
1.b (\textbf{Punti 3}) Qual è il numero di imprese con utile negativo?
...
1.c (\textbf{Punti 2}) (\textbf{Punti 2}) L'utile medio è pari a \(\bar x=2.1489\), e la sua standard deviation
...
2.a (\textbf{Punti 14}) Calcolare la probabilità che le persone \textbf{totali} (\(C_1+...+C_4\)) in fila al supermercato a mezzogiorno, siano almeno due.
...
2.b (\textbf{Punti 3}) Qual è la probabilità di avere esattamente \emph{due} casse su \emph{quattro} senza fila?
...
e quindi
\[
P(X=2)={4\choose 2}0.6065^2(1-0.6065)^{4-2}=0.3417
\]

\end{sol}

2.c (\textbf{Punti 3}) Quando due eventi \(A\) e \(B\) si dicono \emph{indipendenti} e quando \emph{incompatibili}?
...
2.d (\textbf{Punti 2}) Se \(X_1\sim N(2,1)\), \(X_2\sim N(1,1)\) e \(X_3\sim N(1,1)\),
\(X_1\), \(X_2\) e \(X_3\) indipendenti, come si distribuisce
...
(\textbf{Punti 14}) Un'urna contiene \(4\) bussolotti Rossi, \(3\) bussolotti Blu e \(5\) bussolotti Gialli. Si estrae 60 volte con reintroduzione; qual è la probabilità che il numero di rossi in 60 estrazioni sia maggiore di 21?
...
4.a (\textbf{Punti 3}) Sia \(h\) uno stimatore per theta, tale che
\[
...
4.b (\textbf{Punti 3}) Siano \(h_1\) e \(h_2\) due stimatori per \(\theta\), tali che:
\begin{eqnarray*}
MSE(h_1) &=&   \frac\theta n\\
...
4.c (\textbf{Punti 3}) Si sono osservati due gruppi di dati quantitativi e si è osservato, \(\hat\mu_1=10.2\) e \(\hat\mu_2=15.6\). Posto a test
\[
\begin{cases}
H_0:\mu_1=\mu_2\\
H_1:\mu_1\ne \mu_2
...
(\textbf{Punti 14}) In uno studio comparato sui livelli di occupazione femminile, nel comune \(A\) sono state intervistate 50 donne e 30 hanno dichiarato di avere un lavoro stabile; nel comune \(B\) sono state intervistate 60 donne e 40 hanno dichiarato di avere un lavoro stabile.

Testare all'1\% l'ipotesi che la proporzione di donne che hanno un lavoro stabile nel comune \(A\) sia uguale a quelle del come \(B\), contro l'alternativa che siano \textbf{diverse}.
...
\(\fbox{B}\) SCELTA E CALCOLO STATISTICA-TEST, \(Z\)\[\hat\pi_\text{ A }=\frac{s_\text{ A }}{n_\text{ A }}=\frac{ 30 }{ 50 }= 0.6 \qquad
...
\[
   ...
   =   -0.7237 \, .
   \end{eqnarray*}
\(\fbox{C}\) DECISIONE Dalle tavole si ha \(z_{ 0.005 } = -2.576\).
\[z_{\text{obs}} =  -0.7237   >  z_{ 0.005 } =  -2.576 \]CONCLUSIONE: i dati sono coerenti con \(H_{0}\) al LdS del 1 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-140-1} \end{center}
...
6.a (\textbf{Punti 14}) Prevedere il voto nel test di scienze per uno studente che ha ottenuto 6 nel test di matematica.
...
6.b (\textbf{Punti 3}) Calcolare la percentuale di varianza spiegata dal modello.
...
6.c (\textbf{Punti 3}) Discutere il qq-plot dei residui

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-144-1} \end{center}
...
6.d (\textbf{Punti 2}) Cosa vuol dire che \(r\) è un numero puro?
...
\hypertarget{prova-di-statistica-20210611-2}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-147-1} \end{center}
...
1.a (\textbf{Punti 14}) Calcolare il valore approssimato del 25-esimo percentile.
...
1.b (\textbf{Punti 3}) Analizzando l'istogramma, individuare il valore della media aritmetica e della mediana.
...
1.c (\textbf{Punti 2}) Qual è la percentuale di dati compresa tra il 25-esimo e il 75-esimo percentile?
...
X_L=X_E-X_U.
\]
2.a (\textbf{Punti 14}) Calcolare la probabilità che la variazione di livello sia negativa (\(X_L<0\)).
...
2.b (\textbf{Punti 3}) Nell'ipotesi di indipendenza tra i giorni, calcolare la probabilità di avere esattamente \emph{due} giorni su \emph{cinque} con livello negativo.
...
e quindi
\[
P(X=2)={5\choose 2}0.8077^2(1-0.8077)^{5-2}=0.0464
\]

\end{sol}

2.c (\textbf{Punti 3}) Se \(P(A)=0.6\) e \(P(B)=0.8\), \(A\) e \(B\) possono essere incompatibili?
...
2.d (\textbf{Punti 2}) Se \(X\sim N(\mu_X,\sigma^2_X)\) come si distribuisce
...
(\textbf{Punti 14}) Un'urna contiene \(4\) bussolotti numerati con \(\fbox{−1}\), \(3\) numerati con \(\fbox{0}\) e \(4\) numerati con \(\fbox{+1}\). Si estrae 60 volte con reintroduzione; qual è la probabilità che la media delle 60 estrazioni sia minore di 0.1?
...
4.a (\textbf{Punti 3}) Sia \(h\) uno stimatore per \(\theta\), tale che
\[
...
4.b (\textbf{Punti 3}) Siano \(X_1,...,X_n\) \(n\) VC IID, replicazioni di \(X\sim \mathscr{L}(\theta)\) e sia \(\hat\theta\) lo stimatore di massima verosimiglianza per \(\theta\),
\(\hat\theta\) è corretto?
...
4.c (\textbf{Punti 3}) Definire la significatività e la potenza di un test.
...
Sotto ipotesi di omogeneità, testare all'1\% l'ipotesi che il reddito medio femminile nel comune \(A\) sia uguale a quello del comune \(B\), contro l'alternativa che siano \textbf{diversi}.
...
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 15 + 18 -2);\, 0.005 } = 2.744\).
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-154-1} \end{center}
...
6.a (\textbf{Punti 14}) Si osservino le prime 5 coppie di dati
...
6.b (\textbf{Punti 3}) Il modello si adatta bene ai dati?

\begin{sol}
Il modello spiega il \(r^2\times100=(0.937)^2\times 100=87.7969\%>75\%\) della viarbilità totale di \(Y\). Sì, il modello spiega bene i dati.

\end{sol}

6.c (\textbf{Punti 3}) Discutere il diagramma dei residui

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-158-1} \end{center}
...
6.d (\textbf{Punti 2}) Quando in un modello di regressione lineare un punto è considerato \textbf{influente}?
...
\hypertarget{prova-di-statistica-20210630-1}{%
...
1.a (\textbf{Punti 14}) Individuare la classe modale.
...
1.b (\textbf{Punti 3}) Calcolare la percentuale approssimata di aziende
...
1.c (\textbf{Punti 3}) Calcolare la percentuale approssimata di aziende
con spese maggiori di 10 mila euro.
...
2.a (\textbf{Punti 14}) Calcolare la probabilità che la somma dei numeri sia maggiore uguale a 2.
...
2.b (\textbf{Punti 3}) Calcolare la probabilità che la somma sia maggiore uguale a 1 dato che dall'urna \(A\) è uscito \(\fbox{0}\).
...
2.c (\textbf{Punti 3}) Sia \(X\sim \text{Pois}(10)\) e \(Y\sim\text{Binom}(n,0.5)\).
Sia
\[
W=X-Y
\]
...
2.d (\textbf{Punti 2}) Siano \(X_1,...,X_6\), \(6\) variabili casuali IID,
tali che
\[
...
(\textbf{Punti 14}) Si lancia un dado perfetto, si vince se esce un numero maggiore o uguale a 5.
...
4.b (\textbf{Punti 3}) Definire la funzione di verosimiglianza.
...
4.c (\textbf{Punti 3}) Si sono osservati due gruppi di dati quantitativi e si è osservato, \(\hat\mu_1=10.2\), \(\hat\sigma_1=1.12\) e \(\hat\mu_2=15.6\), \(\hat\sigma_2=3.72\). Posto a test
\[
\begin{cases}
H_0:\sigma_1=\sigma_2\\
H_1:\sigma_1\ne \sigma_2
...
(\textbf{Punti 14}) In uno studio sui consumi sono stati intervistati \(n=25\) individui
...
\widehat{V(\hat\beta_{0})} &=& S_{\varepsilon}^{2}\left( \frac{1} {n}  +  \frac{\bar{x}^{2}} {n \hat{\sigma}^{2}_{X}} \right)\ \\
 ...
 \widehat{SE(\hat\beta_{0})}        &=&  \sqrt{ 0.027 }\\
 &=&  0.1643 
...
\end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{(25-2);\, 0.05} = 1.711\).
\[t_{\text{obs}} = 0.5848 < t_{23;\, 0.05} = 1.711\]
CONCLUSIONE: i dati \textbf{sono} coerenti con \(H_{0}\) al LdS del 5\%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-166-1} \end{center}
...
6.a (\textbf{Punti 14}) Prevedere il consumo per un individuo che guadagna \(x=4.126\) e
...
\begin{verbatim}
    ...
    \end{eqnarray*}\[\hat y_{X= 12.3 }=\hat\beta_0+\hat\beta_1 x= 0.0286 + 0.8604 \times 12.3 = 10.61 \]
\end{verbatim}

\end{sol}

6.b (\textbf{Punti 3}) Quale delle due previsioni, per \(x=4.126\) e per \(x=12.3\),
...
6.c (\textbf{Punti 3}) Calcolare le quantità \(TSS\), \(RSS\) e \(ESS\).
...
\begin{eqnarray*}
   ...
   RSS &=& (1-R^2)\cdot TSS\\
      &=& (1- 0.7053 )\cdot 537.3 \\
      &=&  158.4 \\
   TSS &=& RSS+TSS \\ 537.3  &=&  379 + 158.4 
  \end{eqnarray*}

\end{sol}

6.d (\textbf{Punti 2}) Cosa vuol dire che \(r\) è invariante ai cambiamenti di scala?
...
\hypertarget{prova-di-statistica-20210630-2}{%
...
1.a (\textbf{Punti 14}) Individuare il 25-esimo, 50-esimo e l'75-esimo percentile
...
1.b (\textbf{Punti 3}) La somma dei dati è 94.6. Come dobbiamo aspettarci l'istogramma di densità?
...
1.c (\textbf{Punti 2}) Cosa significa che la media aritmetica gode della proprietà di linearità?
...
2.a (\textbf{Punti 14}) Qual è la probabilità di vincere almeno 5 volte su 6 estrazioni?
...
2.b (\textbf{Punti 3}) Calcolare la probabilità di vincere la prima volta alla quarta estrazione.
...
2.c (\textbf{Punti 3}) Se \(A\) e \(B\) sono due eventi tali che, \(P(A)>0\), \(P(B)>0\), \(P(A\cap B)=0\).
...
2.d (\textbf{Punti 2}) Sia \(X\sim N(3.2,(1.1)^2)\) e sia \(Y\sim\chi_{n-1}^2\), posto
\[
...
(\textbf{Punti 14}) Un'urna contiene un numero imprecisato di palline numerate. Si conoscono solo
...
4.b (\textbf{Punti 3}) Scrivere la funzione di verosimiglianza di una Poisson.
...
4.c (\textbf{Punti 3}) Definire il \(p_{\text{value}}\), la probabilità di significatività osservata.
...
(\textbf{Punti 14}) In uno studio sulle preferenze tra canali televisivi, vengono analizzati 92
...
Essendo
\[
\chi^2_{obs}=16.87 >\chi_{1,0.01}^2=16.81
\]
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-175-1} \end{center}

Il \(p_{\text{value}}\) è
\[
P(\chi^2_{ 6 }> \chi^2_{\text{obs}})= 0.009755 
\]
...
6.a (\textbf{Punti 14}) Stimare il modello di regressione dove la qualità della vita è spiegata dagli investimenti provinciali.
...
\]
Il modello si adatta molto bene ai dati, spiegando il 234.6718\% (\textgreater75\%) della variabilità totale della Y.

\end{sol}

6.c (\textbf{Punti 3}) Ricavare le quantità \(TSS\), \(RSS\) e \(ESS\).
...
\begin{eqnarray*}
   ...
   RSS &=& (1-R^2)\cdot TSS\\
      &=& (1- 2.347 )\cdot 1.75 \\
      &=&  -2.357 \\
   TSS &=& RSS+TSS \\ 1.75  &=&  4.107 + -2.357 
  \end{eqnarray*}

\end{sol}

6.d (\textbf{Punti 2}) Cosa significa che gli stimatori di massima dei minimi quadrati \(\hat\beta_0\) e \(\hat\beta_1\) sono \emph{BLUE}?
...
\hypertarget{prova-di-statistica-20210722-1}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-182-1} \end{center}
...
1.b (\textbf{Punti 3}) Che relazione dobbiamo aspettarci tra media e mediana?
...
1.c (\textbf{Punti 2}) La varianza dei dati è pari a \(\sigma^2=4.8382\). Nell'ipotesi che i costi per metano aumentassero del 10\%, quanto varrebbe la varianza?
...
2.a (\textbf{Punti 14}) Calcolare la probabilità che il \emph{totale} delle persone in transito nella stazione, in una data ora, sia maggiore o uguale a due.
...
2.b (\textbf{Punti 3}) Calcolare la probabilità di osservare una persona su un binario, una persona su un altro binario e nessuna persona sul rimanetene.
...
2.c (\textbf{Punti 3}) Siano \(X_1,...,X_n\), \(n\) VC IID, tali che
\[
X_i\sim N(0,1).
\]
Posto
...
2.d (\textbf{Punti 2}) Siano \(X_1\sim \text{Binom}(5,0.3)\) e \(X_2\sim \text{Binom}(3,0.3)\).
Come si distribuisce,
\[X_1+X_2\sim ~~?\]
...
(\textbf{Punti 14}) Un'urna contiene 4 palline numerate
...
4.a (\textbf{Punti 3}) Se \(h\) è uno stimatore tale che la sua distorsione va a zero per \(n\) che tende all'infinito:
\[
...
4.b (\textbf{Punti 3}) Definire la funzione di verosimiglianza della Bernoulli.
...
4.c (\textbf{Punti 3}) Un dado, che non sappiamo se è perfetto oppure no, viene lanciato 40 volte.
...
5.a (\textbf{Punti 10}) Costruire un intervallo di confidenza al 99\% per il tempo medio di azione.
...
5.b (\textbf{Punti 4}) È ben noto, da studi pregressi, che quella tipologia di farmaci ha un tempo medio
di azione pari a 29.3. Testare al livello di significatività dell'1\% che il farmaco in sperimentazione agisca in modo uguale agli altri contro l'alternativa che abbia tempi di azione inferiori.

\begin{sol}
\(\fbox{A}\) FORMULAZIONE DELLE IPOTESI
\[\begin{cases}
   H_0:\mu=\mu_0= 29.3 \text{  }\\
   H_1:\mu <  \mu_0= 29.3 \text{  }
   \end{cases}\]
...
\begin{eqnarray*}
   ...
   \end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 15 -1);\, 0.01 } = -2.624\).
\[t_{\text{obs}} =  -12.89   <  t_{ 14 ;\,  0.01 } =  -2.624 \]
CONCLUSIONE: i dati non sono coerenti con \(H_{0}\) al LdS del 1 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-190-1} \end{center}
...
6.a (\textbf{Punti 14}) Stimare il modello di regressione dove il risparmio è funzione del reddito e quello in cui il reddito è funzione del risparmio.
...
\begin{eqnarray*}
           ...
           \hat\sigma_Y^2&=&\frac 1 n\sum_{i=1}^n y_i^2-\bar y^2=\frac {1}{ 15 }  1.77  - 0.2667 ^2= 0.0469 \\
...
6.b (\textbf{Punti 3}) I due modelli si adattano bene ai dati?
...
6.c (\textbf{Punti 3}) Discutere il diagramma dei residui del modello di regressione dove \(Y\) viene spiegata da \(X\).

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-194-1} \end{center}
...
6.d (\textbf{Punti 2}) Se ogni individuo risparmiasse 10€ in più al mese, quanto varrebbe \(r\)?
...
\hypertarget{prova-di-statistica-20210722-2}{%
...
1.a (\textbf{Punti 14}) Calcolare la colonna delle densità percentuali.
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-198-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Calcolare il numero approssimato di aziende con consumo di gas inferiore al 25-esimo percentile \(x_{0.25}\).
...
1.c (\textbf{Punti 2}) È vero che la media aritmetica minimizza la somma dei valori assoluti degli scarti? Perché?
...
2.a (\textbf{Punti 14}) Qual è la probabilità di vincere almeno 1 volta su 5 giocate?
...
2.b (\textbf{Punti 3}) Per giocare si devono puntare due euro ad ogni giocata. Se vinciamo ci vengono dati 9€, i 2€ giocati più 7€ di vincita. Quindi:
...
2.c (\textbf{Punti 3}) Se \(A\) e \(B\) sono due eventi tali che, \(P(A)=0.5\), \(P(B)=0.4\) e \(P(A\cup B)=0.58\). \(A\) e \(B\) sono indipendenti? Perché?
...
2.d (\textbf{Punti 2}) Siano \(Y_1,...,Y_n\), \(n\) VA IID, tali che, \(Y_i\sim\chi_{1}^2\), posto
\[
X=\sum_{i=1}^n Y_i
\]
...
(\textbf{Punti 14}) Un'urna contiene 150 di palline numerate.
...
\hypertarget{esercizio-4-8}{%
\subsection{Esercizio 4}\label{esercizio-4-8}}

4.a (\textbf{Punti 3}) Sia \(\hat\sigma^2\) lo stimatore di massima verosimiglianza per il parametro \(\sigma^2\) della normale. \(\hat\sigma^2\) è corretto? Quanto vale la sua distorsione?
...
4.b (\textbf{Punti 3}) Descrivere la tavola della verità di un test.

4.c (\textbf{Punti 3}) In uno studio clinico si è osservato l'effeto sul numero di anticorpi su due gruppi trattati con un farmaco differente. Si è osservato \(\hat\mu_1=10.2\), \(\hat\sigma_1=1.12\) e \(\hat\mu_2=15.6\), \(\hat\sigma_2=3.72\). Posto a test
\[
\begin{cases}
H_0:\sigma_1=\sigma_2\\
H_1:\sigma_1\ne \sigma_2
...
5.a (\textbf{Punti 10}) Costruire un intervallo di confidenza al 95\% per il consumo medio.
...
5.b (\textbf{Punti 4}) È noto che quella fascia di motori ha un consuo medio pari \(23.9~\text{Km/litro}\). Testare al livello di significatività dell'5\% che il nuovo motore consumi come gli altri contro l'alternativa che abbia consumi \emph{diversi}.
...
6.a (\textbf{Punti 14}) Stimare il modello di regressione dove l'indice di stress (\(Y\)) è spiegato da \(X\) e quello dove \(Y\) è spiegato da \(W\).
...
6.b (\textbf{Punti 3}) Quale dei due modelli è più affidabile?
...
6.c (\textbf{Punti 3}) Considerata la scomposizine della TSS di \(X\) rispetto ad \(Y\).
\[
TSS = ESS + RSS
\]
...
6.d (\textbf{Punti 2}) Sotto ipotesi di normalità dei residui, come sono distribuiti gli stimatori dei mini quadrati \(\hat\beta_0\) e \(\hat\beta_1\)?
...
1.b (\textbf{Punti 3}) Quante aziende, approssimativamente, hanno un bilancio inferiore a 1.5?

\begin{sol}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-207-1} \end{center}
...
1.c (\textbf{Punti 2}) Che relazione dobbiamo aspettarci tra media e mediana?

1.d (\textbf{Punti 2}) La varianza dei dati è pari a \(\sigma^2=36.8775\). Nell'ipotesi che tutte le aziende aumentassero il loro bilancio di 10 mila ero, quanto varrebbe la varianza dei così trasformati?

\hypertarget{esercizio-2-10}{%
\subsection{Esercizio 2}\label{esercizio-2-10}}

Siano \(X_1\sim N(10,2)\) e sia \(X_2\sim N(10,3)\). Posto \(A=\{X_1<11\}\), \(B=\{X_1>9\}\), e \(C=\{9<X_2<10\}\)

2.a (\textbf{Punti 14}) Quanto vale \(P\Big((A\cap B)\cap C\Big)\)?
...
2.b (\textbf{Punti 3}) Posto \(Y=X_1-X_2\), calcolare \(P(Y<-1)\).
...
2.c (\textbf{Punti 3}) Siano \(X_1,...,X_n\), \(n\) VC IID, tali che \(X_i\sim \text{Ber}(\pi)\), posto
\[
 X = X_1+...+X_n
\]
come si distribuisce \(X\)?

2.d (\textbf{Punti 2}) Sia \(X\sim \text{Pois}(1.5)\), per quali valori \(x\), \(F(x)\), la funzione di ripartizione di \(X\) è minore o uguale a 0.5? Ovvero, per quali \(x\) la seguente è rispettata \(F(x)\le 0.5\)?
...
\hypertarget{esercizio-3-9}{%
\subsection{Esercizio 3}\label{esercizio-3-9}}

3.a (\textbf{Punti 2}) Enunciare il teorema centrale del limite per la somma.

3.b (\textbf{Punti 14}) Il numero di incidenti giornalieri sul lavoro del comparto \(A\) è distribuito come una
...
\hypertarget{esercizio-4-9}{%
\subsection{Esercizio 4}\label{esercizio-4-9}}

4.a (\textbf{Punti 3}) Siano \(h_1\) e \(h_2\) due stimatori corretti per \(\theta\), cosa significa che \(h_1\) è più efficiente di \(h_2\)?

4.b (\textbf{Punti 3}) Definire la funzione di verosimiglianza della Poisson.

4.c (\textbf{Punti 3}) Una moneta, che non sappiamo se è perfetta oppure no, viene lanciata 40 volte.
...
4.d (\textbf{Punti 2}) In caso di campionamento casuale semplice \textbf{senza reintroduzione}
...
5.a (\textbf{Punti 7}) Determinare un intervallo di confidenza al 95\%
...
5.b (\textbf{Punti 7}) Una indagine più vasta dell'anno precedente
...
\(\fbox{A}\) FORMULAZIONE DELLE IPOTESI
\[\begin{cases}
   H_0:\mu=\mu_0= 3250 \text{  }\\
   H_1:\mu <  \mu_0= 3250 \text{  }
   \end{cases}\]
...
\begin{eqnarray*}
   ...
   \end{eqnarray*}\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(z_{ 0.05 } = -1.645\).
\[z_{\text{obs}} =  -1.667 < z_{ 0.05 } =  -1.645 \] CONCLUSIONE: i dati non sono coerenti con \(H_{0}\) al LdS del 5 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-213-1} \end{center}

Il \(p_{\text{value}}\) è \[ P(Z<z_{\text{obs}})=P(Z< -1.67 )= 0.04779 \]

\end{sol}

\hypertarget{esercizio-6-7}{%
...
6.a (\textbf{Punti 14}) Stimare il modello di regressione dove \(Y\) viene spiegata da \(X\)
...
\begin{eqnarray*}
           ...
           \hat\sigma_Y^2&=&\frac 1 n\sum_{i=1}^n y_i^2-\bar y^2=\frac {1}{ 15 }  2  - 0.2667 ^2= 0.0622 \\
...
6.b (\textbf{Punti 3}) Qual è la percentuale di varianza spiegata dal modello?
...
6.c (\textbf{Punti 2}) Se in un modello di regressione si conoscono \(\hat\beta_1\), \(r^2\) e \(\hat\sigma_X\) è possibile ricavare \(\hat\sigma_Y\)? In che modo?
...
6.d (\textbf{Punti 2}) Cosa significa che il coefficiente di correlazione è invariante alle trasformazioni lineari?

\hypertarget{anno-2022}{%
\chapter{Anno 2022}\label{anno-2022}}

\hypertarget{prova-di-statistica-20220616-1}{%
\section{Prova di Statistica 2022/06/16-1}\label{prova-di-statistica-20220616-1}}

\hypertarget{esercizio-1-11}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-219-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Quante famiglie hanno una spesa superiore al 75-esimo percentile?
...
1.d (\textbf{Punti 2}) La spesa media è pari a \(\bar x=7.1245\), mentre la SD è pari a \(SD=4.7792\).
...
2.a (\textbf{Punti 14}) Quanto vale \(P\Big((A\cap B)\cup C\Big)\)?
...
2.b (\textbf{Punti 3}) Si estrae 5 volte da \(X\sim N(102,1.5)\), posto \(A=\{X>100\}\), quale è la probabilità che \(A\) si avveri 3 volte su 5?
...
2.c (\textbf{Punti 2}) Se \(F\) è la funzione di ripartizione della VA \(X\), posto \(a < b\) due numeri qualunque, a cosa equivale
\[
F(b)-F(a) \qquad?
\]
...
\hypertarget{esercizio-3-10}{%
\subsection{Esercizio 3}\label{esercizio-3-10}}

3.a (\textbf{Punti 14}) Ogni giorno un impianto di produzione confeziona 1000 lotti. In media il 10 di questi lotti sono fallati, con una deviazione standard pari a 0.3. Dopo 300 giorni di produzione qual è la probabilità che il numero di lotti fallati sia maggiore di 3100?
...
\hypertarget{esercizio-4-10}{%
\subsection{Esercizio 4}\label{esercizio-4-10}}

4.a (\textbf{Punti 3}) Sia \(\hat \lambda\) lo stimatore di massima verosimiglianza di \(\lambda\) del modello di Poisson.
\begin{eqnarray*}
  \hat\lambda &=&  \frac 1n\sum_{i=1}^nx_i\\
\end{eqnarray*}
Dimostrare le correttezza di \(\hat\lambda\) in almeno tre passaggi.

4.b (\textbf{Punti 2}) Definire lo \emph{Standard Error} di uno stimatore.

4.c (\textbf{Punti 3}) Definire gli errori di primo e di secondo tipo di un test statistico.

4.d (\textbf{Punti 3}) In un test statistico, per quali valori di \(p_\text{value}\) si tende a rifiutare \(H_0\)?

\hypertarget{esercizio-5-8}{%
\subsection{Esercizio 5}\label{esercizio-5-8}}

(\textbf{Punti 14}) Il Supermercato \(S\), della catena \(C\), ha monitorato gli accessi al suo interno per una settimana.
...
Il chi quadro osservato è 4.832 è minore di \(\chi^2_{4;0.05}\) 9.4877, 13.2767, e quindi \textbf{non rifiuto} \(H_0\).

\end{sol}

\hypertarget{esercizio-6-8}{%
...
6.a (\textbf{Punti 14}) Stimare la previsione per \(x=1.5\) nel modello di regressione dove \(Y\) viene spiegata da \(X\).
...
\begin{eqnarray*}
           ...
           \text{cov}(X,Y)&=&\frac 1 n\sum_{i=1}^n x_i~y_i-\bar x\bar y=\frac {1}{ 150 }  74.32 - 0.737 \cdot 0.7512 = -0.0582 \\
           \hat\beta_1 &=& \frac{\text{cov}(X,Y)}{\hat\sigma_X^2} \\
...
\begin{sol}
\begin{eqnarray*}
r&=&\frac{\text{cov}(X,Y)}{\sigma_X\sigma_Y}=\frac{ -0.0582 }{ 0.5511 \times 0.1144 }= -0.9223 \\r^2&=& 0.8506 > 0.75
\end{eqnarray*}
Il modello si adatta bene ai dati.

\end{sol}

6.c (\textbf{Punti 2}) Interpretare i parametri di regressione \(\hat\beta_0\) e \(\hat\beta_1\).

6.d (\textbf{Punti 2}) Se \(W=-10\times Y\), quanto varrà \(r_{XW}\), coefficiente di correlazione tra \(X\) e \(W\)?
...
1.b (\textbf{Punti 3}) Quante famiglie hanno una spesa inferiore al 25-esimo percentile?
...
1.c (\textbf{Punti 2}) La spesa media è pari a \(\bar x=6.9973\), che forma ci dobbiamo aspettare dell'istogramma di densità?

1.d (\textbf{Punti 2}) La varianza della spesa è pari a \(Var=18.3466\).
...
2.a (\textbf{Punti 14}) Quanto vale \(P(A\cup B)\)?
...
2.b (\textbf{Punti 3}) Si estrae 6 volte da \(X\sim \text{Pois}(1.5)\), posto \(A=\{X<2\}\), quale è la probabilità che \(A\) si avveri 3 volte su 6?
...
2.c (\textbf{Punti 2}) Se \(X\) è una VC con valore atteso \(E(X)=0.5\) e \(V(X)=1.2\), posto \(Y=X^2\) è vero che \(E(Y)=E^2(X)\)?

\hypertarget{esercizio-3-11}{%
\subsection{Esercizio 3}\label{esercizio-3-11}}

3.a (\textbf{Punti 14}) Si lancia un dado perfetto 100 volte. Qual è la probabilità che la proporzione di volte che si osserva la faccia sei (⚅) sia maggiore di 0.2?
...
\hypertarget{esercizio-4-11}{%
\subsection{Esercizio 4}\label{esercizio-4-11}}

4.a (\textbf{Punti 3}) Sia \(\hat \mu\) lo stimatore di massima verosimiglianza di \(\mu\) del modello Normale.
\begin{eqnarray*}
  \hat\mu &=&  \frac 1n\sum_{i=1}^nx_i\\
\end{eqnarray*}
Dimostrare le consistenza di \(\hat\mu\) in almeno tre passaggi.

4.b (\textbf{Punti 2}) Siano \(h_1\) e \(h_2\) due stimatori per \(\theta\), cosa significa dire che \(h_1\) è più efficiente di \(h_2\)?

4.c (\textbf{Punti 3}) Definire la significatività e la potenza di un test statistico.

4.d (\textbf{Punti 3}) Se in un test statistico il \(p_\text{value}>0.1\) possiamo rifiutare \(H_0\)?

\hypertarget{esercizio-5-9}{%
\subsection{Esercizio 5}\label{esercizio-5-9}}

5.a (\textbf{Punti 12}) In un'indagine sull'opinione sul reddito di cittadinanza sono stati intervistate 140 persone che vivono al nord e 170 che vivono al sud: 60 su 140 che vivono al nord sono favorevoli al reddito di cittadinanza mentre 95 su 170 che vivono al sud sono favorevoli.
...
5.b (\textbf{Punti 2}) Calcolare e discutere il \(p\)-value del test precedente.
...
\(\fbox{B}\) SCELTA E CALCOLO STATISTICA-TEST, \(Z\)\[\hat\pi_\text{ A }=\frac{s_\text{ A }}{n_\text{ A }}=\frac{ 60 }{ 140 }= 0.4286 \qquad
   \hat\pi_\text{ B }=\frac{s_\text{ B }}{n_\text{ B }}=\frac{ 95 }{ 170 }= 0.5588 \]Calcoliamo la proporzione comune sotto \(H_0\)
\[
     \pi_C=\frac{s_\text{ A }+s_\text{ B }}{n_\text{ A }+n_\text{ B }}=
     \frac{ 155 }{ 310 }= 0.5 
   \]\begin{eqnarray*}
   ...
   \end{eqnarray*}
\(\fbox{C}\) DECISIONE Dalle tavole si ha \(z_{ 0.005 } = -2.576\).
\[z_{\text{obs}} =  -2.283   >  z_{ 0.005 } =  -2.576 \]CONCLUSIONE: i dati sono coerenti con \(H_{0}\) al LdS del 1 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-233-1} \end{center}
...
6.a (\textbf{Punti 14}) Stimare la previsione per \(x=1.0\) nel modello di regressione dove \(Y\) viene spiegata da \(X\).
...
\begin{eqnarray*}
           ...
           \hat\sigma_Y^2&=&\frac 1 n\sum_{i=1}^n y_i^2-\bar y^2=\frac {1}{ 150 }  83.81  - 0.7361 ^2= 0.0168 \\
...
\begin{sol}
\begin{eqnarray*}
r&=&\frac{\text{cov}(X,Y)}{\sigma_X\sigma_Y}=\frac{ -0.0762 }{ 0.6152 \times 0.1297 }= -0.9552 \\r^2&=& 0.9124 > 0.75
\end{eqnarray*}
Il modello si adatta bene ai dati.

\end{sol}

6.c (\textbf{Punti 2}) Cosa sono i \emph{punti di leva} e cosa gli \emph{outliers}?

6.d (\textbf{Punti 2}) Se \(W=10\times Y\), posto
...
1.b (\textbf{Punti 3}) Qual è la percentuale di famiglie con spesa superiore al 25-esimo percentile?
...
1.c (\textbf{Punti 2}) La spesa media è pari a \(\bar x=7.143\), che forma ci dobbiamo aspettare dell'istogramma di densità?

1.d (\textbf{Punti 2}) La varianza della spesa è pari a \(Var=19.055\).
...
2.a (\textbf{Punti 14}) Calcolare \(P(W< 2)\).
...
2.b (\textbf{Punti 3}) Posto \(V=2+5\cdot W\), ricavare valore atteso e varianza di \(V\).
...
2.c (\textbf{Punti 2}) Se \(A\) e \(B\) sono due eventi diversi dal vuoto, è possibile che \(P(A)+P(B)>1\)?

\hypertarget{esercizio-3-12}{%
\subsection{Esercizio 3}\label{esercizio-3-12}}

3.a (\textbf{Punti 14}) Un'urna contiene 3 palline bianche, 2 nere e 5 blu. Si estrae 200 volte con reimmissione.
...
\hypertarget{esercizio-4-12}{%
\subsection{Esercizio 4}\label{esercizio-4-12}}

4.a (\textbf{Punti 3}) Sia \(\hat \lambda\) lo stimatore di massima verosimiglianza di \(\lambda\) del modello Poisson.
\begin{eqnarray*}
  \hat\lambda &=&  \frac 1n\sum_{i=1}^nx_i\\
\end{eqnarray*}
Dimostrare le consistenza di \(\hat\lambda\) in almeno tre passaggi.

4.b (\textbf{Punti 2}) Siano \(h\) uno stimatore per \(\theta\), cosa significa dire che \(h\) è asintoticamente corretto?

4.c (\textbf{Punti 3}) Definire la probabilità di significatività.

4.d (\textbf{Punti 3}) Se in un test statistico \(0.01 < p_\text{value} <0.05\) cosa possiamo concludere?

\hypertarget{esercizio-5-10}{%
\subsection{Esercizio 5}\label{esercizio-5-10}}

(\textbf{Punti 14}) In un'indagine sull'opinione sul reddito sono stati rilevati i redditi di 140 persone che vivono al nord e quelli di 170 che vivono al sud: il reddito medio di chi vive al nord è di 32.2 mila euro annui con una SD pari a 2.4 mila euro annui, mentre il
...
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 140 + 170 -2);\, 0.025 } = 1.968\).
\[t_{\text{obs}} =  19.43   >  t_{ 308 ;\,  0.025 } =  1.968 \]
CONCLUSIONE: i dati non sono coerenti con \(H_{0}\) al LdS del 5 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-243-1} \end{center}

Il \(p_{\text{value}}\) è
\[ P(|T_{n1+n2-2}|>|t_{\text{obs}}|)=2P(T_{n1+n2-2}>|t_{\text{obs}}|)=2P(T_{n1+n2-2}>| 19.4256 |)= 1.957e-55 \]

\end{sol}

\hypertarget{esercizio-6-10}{%
...
6.a (\textbf{Punti 14}) Stimare la previsione per \(x=1.4\) nel modello di regressione dove \(Y\) viene spiegata da \(X\).
...
\begin{eqnarray*}
           ...
           \hat\sigma_Y^2&=&\frac 1 n\sum_{i=1}^n y_i^2-\bar y^2=\frac {1}{ 150 }  87.18  - 0.7516 ^2= 0.0162 \\
         ...
         \end{eqnarray*}\[\hat y_{X= 1 }=\hat\beta_0+\hat\beta_1 x= 0.9002 + (-0.2032) \times 1 = 0.697 \]

\end{sol}

6.b (\textbf{Punti 3}) Calcolare e discutere \(R^2\).

\begin{sol}
\begin{eqnarray*}
r&=&\frac{\text{cov}(X,Y)}{\sigma_X\sigma_Y}=\frac{ -0.0733 }{ 0.6006 \times 0.1274 }= -0.9578 \\r^2&=& 0.9174 > 0.75
\end{eqnarray*}
Il modello si adatta bene ai dati.

\end{sol}

6.c (\textbf{Punti 2}) Cos'è un \emph{punto influente}?

6.d (\textbf{Punti 2}) Se \(W=10+ Y\), posto
\[w_i=\beta_0'+\beta_1'x_ì +\epsilon_i'\]
...
la classe modale è la classe \(`r `\) essendo la classe con densità \(h_{3}=18.4091\) maggiore.

\end{sol}

1.b (\textbf{Punti 3}) Quante imprese hanno una spesa compresa tra 15 mila euro e il 75-esimo percentile?
...
1.c (\textbf{Punti 2}) La spesa media è pari a \(\bar x=12.0227\) mila euro,
considerato la classe modale ricavata al punto 1.a, quale relazione ci dobbiamo attendere tra media e mediana?

\begin{sol}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-251-1} \end{center}

\end{sol}

1.d (\textbf{Punti 2}) La spesa media in investimenti green su 220 aziende modenesi è pari a \(\bar x=12.0227\) mila euro, mentre la spesa media in investimenti green della provincia di Reggio, su un campione di 150 aziende è pari a \(12.22\) mila euro.
...
2.a (\textbf{Punti 14}) Quanto vale \(P\Big((A\cup B)\cup C\Big)\)?
...
2.b (\textbf{Punti 3}) Sia \(X\sim N(10,1)\), posto \(A=\{X>8\}\). Si estrae ripetutamente da \(X\) e si finisce quando \(A\) si avvera \textbf{2} volte. Calcolare la probabilità di finire dopo 6 estrazioni.
...
2.c (\textbf{Punti 2}) Siano \(A\) e \(B\) due eventi diversi dal vuoto. È vero che se \(A\) e \(B\) sono
...
2.d (\textbf{Punti 2}) Se \(F\) è la funzione di ripartizione della VA \(X\),
quali sono il valore massimo e quello minimo che \(F\) può assumere?

\hypertarget{esercizio-3-13}{%
\subsection{Esercizio 3}\label{esercizio-3-13}}

(\textbf{Punti 14}) Ogni giorno il centralino di un servizio di assistenza riceve in media 26.34 telefonate con una deviazione standard pari a 1.3 telefonate.
...
\hypertarget{esercizio-4-13}{%
\subsection{Esercizio 4}\label{esercizio-4-13}}

4.a (\textbf{Punti 3}) Sia \(\hat \lambda\) lo stimatore di massima verosimiglianza di \(\lambda\) del modello di Poisson: \(\hat\lambda = \frac 1n\sum_{i=1}^nx_i\)
Scrivere lo Standard Error di \(\hat \lambda\).

4.b (\textbf{Punti 3}) Se \(h\) è uno stimatore per \(\theta\) tale che
\(E(h)\ne \theta\) e che \(\lim_ {n\to +\infty}E(h)=\theta\) di quale proprietà gode \(h\)?

4.c (\textbf{Punti 3}) Definire gli errori di primo e di secondo tipo di un test statistico e le relative probabilità.

4.d (\textbf{Punti 3}) In un confronto tra due campioni viene messo a test
\[
\begin{cases}
H_0:\sigma_A=\sigma_B\\
H_1:\sigma_A\ne \sigma_B
...
(\textbf{Punti 7}) Costruire un Intervallo di Confidenza al 95\% per la media di popolazione \(\mu\).
...
(\textbf{Punti 7}) La stessa domanda è stata posta ad un secondo campione di \(n_S=38\) consumatori privati,scelti a caso tra i cittadini del comune di Sassuolo.
Il campione ha restituito una media pari a 20.2 €/mese, con una deviazione standard pari a 2.9 €/mese. Sotto ipotesi di omogeneità testare al 5\% l'ipotesi che i due comuni abbiano uguale media, contro l'alternativa che sia diversa.
...
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 34 + 38 -2);\, 0.025 } = -1.994\).
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-256-1} \end{center}

Il \(p_{\text{value}}\) è
\[ P(|T_{n1+n2-2}|>|t_{\text{obs}}|)=2P(T_{n1+n2-2}>|t_{\text{obs}}|)=2P(T_{n1+n2-2}>| -1.3184 |)= 0.1917 \]

\end{sol}

\hypertarget{esercizio-6-11}{%
...
6.a (\textbf{Punti 14}) Calcolare il residuo del quarto dato nel modello di regressione dove \(Y\) viene spiegata da \(X\).
...
6.b (\textbf{Punti 3}) Scrivere la scomposizione della varianza del modello di regressione
...
\begin{sol}
\begin{eqnarray*}
r&=&\frac{\text{cov}(X,Y)}{\sigma_X\sigma_Y}=\frac{ 7.173 }{ 3.197 \times 2.8 }= 0.8014 \\r^2&=& 0.6422 < 0.75
\end{eqnarray*}
Il modello \textbf{non} si adatta bene ai dati.

\end{sol}

6.c (\textbf{Punti 3}) Il parametro di regressione \(\hat\beta_0\), in questo caso, è interpretabile?

6.d (\textbf{Punti 2}) Una previsione per \(x=40\) è attendibile? Perché?

6.e (\textbf{Punti 2}) Se \(W=5+ Y\), posto \(w_i=\beta_0'+\beta_1'x_ì +\epsilon_i'\)
il modello in cui \(W\) viene spiegata da \(X\), quanto varranno \(\beta_0'\) e \(\beta_1'\)?

\hypertarget{prova-di-statistica-20220701-2}{%
\section{Prova di Statistica 2022/07/01-2}\label{prova-di-statistica-20220701-2}}

\hypertarget{esercizio-1-15}{%
...
1.a (\textbf{Punti 14}) Disegnare l'istogramma delle densità percentuali.
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-264-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Quante aziende hanno una spesa compresa tra il 25-esimo percentile e 15 mila euro?
...
1.c (\textbf{Punti 2}) Che relazione dobbiamo attenderci tra media, mediana e moda?

1.d (\textbf{Punti 2}) La varianza della spesa è pari a \(Var=17.997\).
Se ogni azienda aumentasse la sua spesa del 15\%, quanto varrebbe varianza dei dati così trasformati?

\hypertarget{esercizio-2-15}{%
...
2.a (\textbf{Punti 14}) Si consideri l'evento \(E\)=``più di due veicoli impegnino la rotatoria''. Calcolare \(P(E)\).
...
2.b (\textbf{Punti 3}) Si osserva la rotatoria per \(n=6\) minuti. Qual è
...
2.c (\textbf{Punti 2}) Se \(X\sim N(0,2)\) e \(X\sim N(1,1.2)\), è vero che
\[
X-Y\sim N(-1,0.8) \qquad ?
\]

2.d (\textbf{Punti 2}) Se \(X\) è una VC con supporto \{0,1,2\} e \(Y\) è una VC con supporto \{2,3,4,5\}.
...
\hypertarget{esercizio-3-14}{%
\subsection{Esercizio 3}\label{esercizio-3-14}}

(\textbf{Punti 14}) Un'urna contiene 10 palline numerate da 1 a 10.
...
\hypertarget{esercizio-4-14}{%
\subsection{Esercizio 4}\label{esercizio-4-14}}

4.a (\textbf{Punti 3}) Siano \(h_1\) e \(h_2\) due stimatori per \(\theta\), tali che
...
4.b (\textbf{Punti 3}) Siano \(T_1\) e \(T_2\) due test statistici per la stessa \(H_0\) e con la stessa significatività \(\alpha\). Cosa significa dire che \(T_1\) e più potente di \(T_2\)?

4.c (\textbf{Punti 3}) Definire la probabilità di significatività osservata.

4.d (\textbf{Punti 3}) Se in un test statistico che utilizza la statistica test \emph{t} con 10 gradi di libertà \(t_\text{obs}=1.4\), il \(p_\text{value}\) sarà maggiore o minore di 0.05? Perché?

\hypertarget{esercizio-5-12}{%
...
5.a (\textbf{Punti 4}) Costruire un intervallo di confidenza la 95\% per \(\pi\) la quota di persone del quartiere \emph{Q} favorevole alle corsie preferenziali
...
5.b (\textbf{Punti 10}) Un'indagine molto più ampia condotta su tutta la città ha mostrato che
la percentuale di favorevoli alle corsie preferenziali è del 55\%. Testare al 5\% l'ipotesi che nel quartiere \emph{Q} la quota di favorevoli sia uguale a quella cittadina contro l'alternativa che sia maggiore.

\begin{sol}
La stima
\[\hat\pi=\frac { 45 } { 75 }= 0.6  \] \(\fbox{A}\) FORMULAZIONE DELLE IPOTESI
\[\begin{cases}
   H_0:\pi=\pi_0= 0.55 \\
   H_1:\pi >  \pi_0= 0.55 
   \end{cases}\]
...
\begin{eqnarray*}
   ...
   =   0.87 \, .
   \end{eqnarray*}\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(z_{ 0.05 } = 1.64\).
\[z_{\text{obs}} =  0.87   <  z_{ 0.05 } =  1.64 \]
CONCLUSIONE: i dati sono coerenti con \(H_{0}\) al LdS del 5 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-269-1} \end{center}

Il \(p_{\text{value}}\) è \[ P(Z>z_{\text{obs}})=P(Z> 0.87 )= 0.192 \]

\end{sol}

\hypertarget{esercizio-6-12}{%
...
6.a (\textbf{Punti 14}) Stimare la previsione per \(x=16\) nel modello di regressione dove \(Y\) viene spiegata da \(X\).
...
6.c (\textbf{Punti 2}) Cosa sono i \emph{punti influenti}?

6.d (\textbf{Punti 2}) Se \(W=2\times Y\) e \(V=X+3\), posto \(w_i=\beta_0'+\beta_1'v_ì +\epsilon_i'\)
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-275-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Qual è la percentuale di aziende che hanno una spesa
...
2.a (\textbf{Punti 14}) Qual è la probabilità di vincere?
...
2.b (\textbf{Punti 3}) Si ripete il gioco di sopra finché non si vince due volte. Qual è la probabilità di finire alla quarta giocata?
...
2.c (\textbf{Punti 2}) Se \(X\sim \text{Pois}(2)\) e \(Y\sim\text{Pois}(1)\), è vero che
\[
X-Y\sim\text{Pois}(1)\qquad ?
\]

2.d (\textbf{Punti 2}) Se \(X\) è una VC con supporto \{0,1,2\} e \(X\) è una VC con supporto \{-2,-1,0\}.
...
\hypertarget{esercizio-3-15}{%
\subsection{Esercizio 3}\label{esercizio-3-15}}

(\textbf{Punti 14}) Il supermercato \emph{S} vende, in media ogni giorno, 102.3 kg di pasta, con una deviazione standard di 10.2. Dopo 60 giorni di apertura, qual è la probabilità che il totale di pasta venduta sia maggiore di 6700 kg?
...
4.b (\textbf{Punti 3}) Se uno stimatore \(h\) per \(\theta\) è tale che \(\lim_{n\to\infty}V(h)=0\), di quali proprietà gode \(h\)?

4.c (\textbf{Punti 3}) Un intervallo di confidenza per \(\theta\) al 95\% è più ampio o meno ampio di uno al 99\%? Perché?

4.d (\textbf{Punti 3}) Se in un test statistico che utilizza la statistica test \emph{t} con 10 gradi di libertà \(t_\text{obs}=14\), il \(p_\text{value}\) sarà maggiore o minore di 0.05? Perché?

\hypertarget{esercizio-5-13}{%
...
5.a (\textbf{Punti 4}) Costruire un intervallo di confidenza la 99\% per \(\mu\) il punteggio medio che le persone del quartiere \emph{Q} esprimono riguardo alle corsie preferenziali.
...
Testare al 5\% l'ipotesi che nel quartiere \emph{Q} il punteggio medio sia uguale a quello cittadino contro l'alternativa che sia maggiore.

\begin{sol}
\(\fbox{A}\) FORMULAZIONE DELLE IPOTESI
\[\begin{cases}
   H_0:\mu=\mu_0= 66.3 \text{  }\\
   H_1:\mu >  \mu_0= 66.3 \text{  }
   \end{cases}\]
...
\begin{eqnarray*}
   ...
   \end{eqnarray*}\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(z_{ 0.05 } = 1.64\).
\[z_{\text{obs}} =  11.7 > z_{ 0.05 } =  1.64 \] CONCLUSIONE: i dati non sono coerenti con \(H_{0}\) al LdS del 5 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-279-1} \end{center}

Il \(p_{\text{value}}\) è \[ P(Z>z_{\text{obs}})=P(Z> 11.74 )= 0 \]

\end{sol}

\hypertarget{esercizio-6-13}{%
...
6.a (\textbf{Punti 14}) Questi sono alcuni dei dati osservati
...
6.b (\textbf{Punti 3}) Ricavare numericamente la scomposizione della varianza del modello di regressione sopra stimato.
...
\begin{eqnarray*}
   ...
   RSS &=& (1-R^2)\cdot TSS\\
      &=& (1- 0.639 )\cdot 821 \\
      &=&  296 \\
   TSS &=& RSS+TSS \\ 821  &=&  525 + 296 
  \end{eqnarray*}

\end{sol}

6.c (\textbf{Punti 2}) Che differenza c'è tra \emph{interpolazione} e \emph{estrapolazione}?

6.d (\textbf{Punti 2}) Cosa significa che \(r\) è un numero puro?

\hypertarget{prova-di-statistica-20220727-1}{%
\section{Prova di Statistica 2022/07/27-1}\label{prova-di-statistica-20220727-1}}

\hypertarget{esercizio-1-17}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-286-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Quante imprese hanno un bilancio compreso tra \(-4\) mila
euro e zero.
...
1.c (\textbf{Punti 2}) La media è risultata essere \(\bar x=8.446\); che
...
1.d (\textbf{Punti 2}) Siano \(x_1,...,x_n\), \(n\) numeri, \(n\) dispari.
...
2.b (\textbf{Punti 3}) Posto \(Z=X-Y\), Calcolare la probabilità che \(P(Z>1|Z\le 2)\).
...
2.c (\textbf{Punti 2}) Siano \(A\) e \(B\) due eventi diversi dal vuoto.
...
2.d (\textbf{Punti 2}) Sia \(X\sim\text{Pois}(\lambda=1)\), disegnare la
funzione di ripartizione \(F(x)\) di \(X\), per \(x\) compreso tra \(-1\) e \(2.5\).

\begin{sol}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-289-1} \end{center}

\end{sol}

\hypertarget{esercizio-3-16}{%
\subsection{Esercizio 3}\label{esercizio-3-16}}

(\textbf{Punti 14}) Un'urna contiene tre palline, una Rossa, una Blue e una Nera. Si vince
...
4.c (\textbf{Punti 3}) Definire gli errori di primo e di secondo
...
\hypertarget{esercizio-5-14}{%
\subsection{Esercizio 5}\label{esercizio-5-14}}

(\textbf{Punti 14}) In uno studio sule preferenze di genere è stato chiesto ad un campione di 150 persone,
...
\toprule
  & A & B & C & Tot\\
\midrule
\textbf{M} & 14.6 & 17.6 & 1.00 & 0\\
\textbf{F} & 16.6 & 20.1 & 1.14 & 0\\
\textbf{Tot} & 0.0 & 0.0 & 0.00 & 0\\
\bottomrule
\end{tabular}
\end{table}
...
Essendo
\[
\chi^2_{obs}=71 >\chi_{1,0.01}^2=16.8
\]
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-293-1} \end{center}

Il \(p_{\text{value}}\) è
\[
P(\chi^2_{ 6 }> \chi^2_{\text{obs}})= 2.606e-13 
\]

\end{sol}

\hypertarget{esercizio-6-14}{%
...
6.a (\textbf{Punti 14}) Calcolare il residuo del comune B nel modello di regressione dove \(Y\) viene spiegata da \(X\).
...
\begin{eqnarray*}
   ...
   RSS &=& (1-R^2)\cdot TSS\\
      &=& (1- 0.447 )\cdot 9.73 \\
      &=&  5.39 \\
   TSS &=& RSS+TSS \\ 9.73  &=&  4.35 + 5.39 
  \end{eqnarray*}

\end{sol}

6.c (\textbf{Punti 3}) Interpretare il parametro di regressione \(\hat\beta_1\).

6.d (\textbf{Punti 2}) Descrivere la differenza tra punti di leva e punti influenti.

6.e (\textbf{Punti 2}) Gli stimatori \(\hat\beta_0\) e \(\hat\beta_1\) dei minimi
quadrati per \(\beta_0\) e \(\beta_1\) sono corretti?

\hypertarget{prova-di-statistica-20220727-2}{%
\section{Prova di Statistica 2022/07/27-2}\label{prova-di-statistica-20220727-2}}

\hypertarget{esercizio-1-18}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-300-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Quante aziende hanno un bilancio compreso tra il 25-esimo
...
1.d (\textbf{Punti 2}) Siano \(x_1,...,x_n\), \(n\) numeri. Si consideri la funzione:
\[g(x)=(x_1-x)^2+...+(x_n-x)^2.\]
Per quale valore di \(x\), \(g(x)\) è minima?

\hypertarget{esercizio-2-18}{%
...
2.a (\textbf{Punti 14}) Si consideri l'evento \(E\)=``più di due veicoli
...
2.c (\textbf{Punti 2}) Siano \(X\sim N(0,1)\) e \(Y\sim N(0,1)\), \(X\) e \(Y\) indipendenti.
Come si distribuisce \(X^2+Y^2\)?

2.d (\textbf{Punti 2}) Se \(X\) è una VC con supporto \{2,3,4,5\} e \(Y\) è una VC con supporto
\{1,2,6\}.
Qual è il supporto di \(X-Y\)?
...
\hypertarget{esercizio-3-17}{%
\subsection{Esercizio 3}\label{esercizio-3-17}}

(\textbf{Punti 14}) Un'urna contiene 4 palline numerate da 1 a 4.
...
4.b (\textbf{Punti 3}) Sia \(\hat\sigma^2\) lo stimatore di massima verosimiglianza
di \(\sigma^2\). In virtù di quale proprietà \(\hat \sigma\) è lo stimatore di
massima verosimiglianza per \(\sigma\)?

4.c (\textbf{Punti 3}) Siano \(T_1\) e \(T_2\) due test per lo stesso sistema di ipotesi,
...
(\textbf{Punti 4}) Costruire un Intervallo di Confidenza al 95\% per la media di popolazione \(\mu\).
...
\begin{eqnarray*}
  \left[~~12.4\pm t_{34;.005}{\frac{2.131}{\sqrt{35}}}~~\right] &=&  
  \left[~~12.4\pm 2.728\times0.36~~\right]\\
  &=&\left[~~11.417; 13.383~~\right]
\end{eqnarray*}

\end{sol}

(\textbf{Punti 10}) La stessa domanda è stata posta ad un secondo campione di \(n_F=38\) consumatori privati,scelti a caso tra i cittadini del comune di Ferrara.
Il campione ha restituito una media pari a 19.2 €/mese, con una deviazione standard pari a 3.9 €/mese. Sotto ipotesi di eterogeneità testare al 5\% l'ipotesi che i due comuni abbiano uguale media, contro l'alternativa che a Ferrara consumino di più.
...
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 35 + 38 -2);\, 0.05 } = -1.67\).
\[t_{\text{obs}} =  -9.25   <  t_{ 71 ;\,  0.05 } =  -1.67 \]
CONCLUSIONE: i dati non sono coerenti con \(H_{0}\) al LdS del 5 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-303-1} \end{center}

Il \(p_{\text{value}}\) è
\[ P(T_{n1+n2-2}<t_{\text{obs}})=P(T_{n1+n2-2}< -9.247 )= 4.113e-14 \]

\end{sol}

\hypertarget{esercizio-6-15}{%
...
6.a (\textbf{Punti 14}) Stimare la previsione per \(x=1.6\) nel modello di regressione dove \(Y\) viene spiegata da \(X\).
...
6.b (\textbf{Punti 3}) Calcolare numericamente \(RSS\):
\[
RSS=\sum_{i=1}^n \hat\epsilon_i^2
\]

\begin{sol}
\[RSS=n(1-r^2)\hat\sigma_Y^2=7.299\]

\end{sol}

6.c (\textbf{Punti 3}) Gli stimatori \(\hat\beta_0\) e \(\hat\beta_1\) sono consistenti?
Perché?

6.d (\textbf{Punti 2}) Se in un modello di regressione con 11 dati,
il residuo studentizzato del dato \(i\) è \(\tilde \epsilon_i=1.23\), cosa possiamo concludere?

6.e (\textbf{Punti 2}) Sia \(\hat\beta_1\) lo stimatore dei minimi quadrati per \(\beta_1\).
Scrivere il suo Standard Error teorico.

\hypertarget{prova-di-statistica-20220727-3}{%
\section{Prova di Statistica 2022/07/27-3}\label{prova-di-statistica-20220727-3}}

\hypertarget{esercizio-1-19}{%
...
1.b (\textbf{Punti 3}) Quante aziende hanno un bilancio compreso tra \(-1\) e il 75-esimo
percentile?
...
1.d (\textbf{Punti 2}) Sapendo che \(\sigma_X=2.82\) la Standard Deviation di \(X\)
e posto \(y_i=-x_i, \forall i\), quanto varrà \(\sigma_Y\), la standard deviation dei dati così
trasformati?

\hypertarget{esercizio-2-19}{%
...
2.b (\textbf{Punti 3}) Si ripete il gioco di sopra finché non si vince tre volte. Qual è la probabilità di finire alla quinta giocata?
...
2.c (\textbf{Punti 2}) Se \(X\) è una VC con supporto \{1,2,3\} e \(Y\) è una VC con
supporto \{1,3,5\}.
Qual è il supporto di \(X\times Y\)?
...
\hypertarget{esercizio-3-18}{%
\subsection{Esercizio 3}\label{esercizio-3-18}}

(\textbf{Punti 14}) Il supermercato \emph{S} accoglie, in media ogni giorno, 3.242 mila persone, con una
...
4.b (\textbf{Punti 3}) Sia \(\hat\theta\) lo stimatore di massima verosimiglianza
per \(\theta\). Qual è la distribuzione asintotica di \(\hat\theta\)?

4.c (\textbf{Punti 3}) Un intervallo di confidenza per \(\theta\) al 90\% è più ampio o meno ampio di uno al 95\%? Perché?

4.d (\textbf{Punti 3}) Se in un test statistico bilaterale che utilizza la statistica test \emph{z} , risulta \(z_\text{obs}=14\), il \(p_\text{value}\) sarà maggiore o minore di 0.01? Perché?

\hypertarget{esercizio-5-16}{%
...
5.a (\textbf{Punti 4}) Costruire un intervallo di confidenza la 95\% per \(\pi\) la quota di tassisti favorevoli al decreto liberalizzazione
...
5.b (\textbf{Punti 10}) La stessa domanda è stata posta ad un secondo
campione di \(n_M=95\) tassisti milanesi. Lo studio ha riportato che 32 persone su 95 (l'20\% del campione) è favorevole.

Testare al 5\% l'ipotesi la quota di tassisti favorevoli sia uguale tra le due città contro
...
\(\fbox{B}\) SCELTA E CALCOLO STATISTICA-TEST, \(Z\)\[\hat\pi_\text{ A }=\frac{s_\text{ A }}{n_\text{ A }}=\frac{ 15 }{ 75 }= 0.2 \qquad
   \hat\pi_\text{ B }=\frac{s_\text{ B }}{n_\text{ B }}=\frac{ 32 }{ 95 }= 0.337 \]Calcoliamo la proporzione comune sotto \(H_0\)
\[
     \pi_C=\frac{s_\text{ A }+s_\text{ B }}{n_\text{ A }+n_\text{ B }}=
     \frac{ 47 }{ 170 }= 0.277 
   \]\begin{eqnarray*}
   ...
   {\sqrt{\frac {\pi_C(1-\pi_C)}{n_\text{ A }}+\frac {\pi_C(1-\pi_C)}{n_\text{ B }}}}&\sim&N(0,1)\\
   z_{\text{obs}}
   &=& \frac{ ( 0.2 -  0.337 )} {\sqrt{\frac{ 0.277 (1- 0.277 )}{ 75 }+\frac{ 0.277 (1- 0.277 )}{ 95 }}}
   =   -1.98 \, .
   \end{eqnarray*}
\(\fbox{C}\) DECISIONE Dalle tavole si ha \(z_{ 0.05 } = -1.64\).
\[z_{\text{obs}} =  -1.98   <  z_{ 0.05 } =  -1.64 \]CONCLUSIONE: i dati non sono coerenti con \(H_{0}\) al LdS del 5 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-311-1} \end{center}
...
6.b (\textbf{Punti 3}) Calcolare la percentuale di varianza di \(Y\) spiegata dal modello.
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-319-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Che percentuale di famiglie investe più di 4.5 mila euro all'anno?
...
1.d (\textbf{Punti 2}) La spesa media è pari a \(\bar x=3.0676\), mentre la SD è pari a \(SD=1.3653\).
...
\hypertarget{esercizio-2-20}{%
\subsection{Esercizio 2}\label{esercizio-2-20}}

2.a (\textbf{Punti 14}) Siano \(X\sim N(10,1.5)\) e sia \(Y\sim N(10,0.5)\), \(X\) e \(Y\) indipendenti. Posto \(A=\{8<X<10\}\), \(B=\{Y<11\}\).
...
2.b (\textbf{Punti 3}) Un'urna contiene due palline rosse, due bianche e una nera. Si estrae due volte senza reinserimento. Qual è la probabilità di avere due colori diversi?
...
2.c (\textbf{Punti 2}) Se \(X\sim\text{Pois}(3.2)\) e \(Y\sim\text{Pois}(1.2)\), \(X\) e \(Y\) indipendenti, quali sono valore atteso e varianza di \(X+Y\) e di \(X-Y\)?

2.d (\textbf{Punti 2}) Sia \(X\) una VC e sia \(F\) la sua funzione di ripartizione. Cosa significa dire che \(F\) è continua a destra?

\hypertarget{esercizio-3-19}{%
\subsection{Esercizio 3}\label{esercizio-3-19}}

3.a (\textbf{Punti 14}) Un'urna contiene 4 palline numerate: \(\fbox{0}\), \(\fbox{0}\), \(\fbox{0}\) e \(\fbox{1}\). Si estrae 100 volte con reinserimento. Qual è la probabilità che la proporzione di palline timbrate con 1 nelle 100 estrazioni sia maggiore di 0.27?
...
4.b (\textbf{Punti 3}) Si \(h\) uno stimatore per \(\theta\) tale che \(E(h)=\theta\). Di quale proprietà gode \(h\)?

4.c (\textbf{Punti 3}) Definire la potenza di un test.

4.d (\textbf{Punti 3}) In uno studio sull'efficacia degli integratori alimentari, su un gruppo di 238 atleti è stato misurato il rendimento atletico (ottimo, buono e scarso) e l'assunzione di integratori (alto, medio e basso). Qui di seguito i dati:
...
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 35 + 35 -2);\, 0.025 } = -1.996\).
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-324-1} \end{center}

Il \(p_{\text{value}}\) è
\[ P(|T_{n1+n2-2}|>|t_{\text{obs}}|)=2P(T_{n1+n2-2}>|t_{\text{obs}}|)=2P(T_{n1+n2-2}>| -2.5719 |)= 0.0123 \]

\end{sol}

\hypertarget{esercizio-6-17}{%
...
\begin{eqnarray*}
           ...
           \hat\sigma_X^2&=&\frac 1 n\sum_{i=1}^n x_i^2-\bar x^2=\frac {1}{ 150 }  51.33  - 0.5129 ^2= 0.0792 \\
           \hat\sigma_Y^2&=&\frac 1 n\sum_{i=1}^n y_i^2-\bar y^2=\frac {1}{ 150 }  27.67  - 0.4203 ^2= 0.0078 \\
           \text{cov}(X,Y)&=&\frac 1 n\sum_{i=1}^n x_i~y_i-\bar x\bar y=\frac {1}{ 150 }  35.59 - 0.5129 \cdot 0.4203 = 0.0217 \\
           \hat\beta_1 &=& \frac{\text{cov}(X,Y)}{\hat\sigma_X^2} \\
                    &=& \frac{ 0.0217 }{ 0.0792 }  =  0.2744 \\
           \hat\beta_0 &=& \bar y - \hat\beta_1 \bar x\\
...
6.c (\textbf{Punti 2}) Interpretare il diagramma dei residui.

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-327-1} \end{center}

6.d (\textbf{Punti 2}) Se \(W=- Y\), quanto varrà \(r_{XW}\), coefficiente di correlazione tra \(X\) e \(W\)?

\hypertarget{prova-di-statistica-20230111-2}{%
\section{Prova di Statistica 2023/01/11-2}\label{prova-di-statistica-20230111-2}}

\hypertarget{esercizio-1-21}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-331-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Quante famiglie investono più di 8 mila euro all'anno?
...
1.d (\textbf{Punti 2}) La spesa media è pari a \(\bar x=6.9001\), mentre la SD è pari a \(SD=1.5027\).
...
\hypertarget{esercizio-2-21}{%
\subsection{Esercizio 2}\label{esercizio-2-21}}

2.a (\textbf{Punti 14}) I clienti in fila nell'ora di punta alla cassa di un supermercato sono distribuiti come una Poisson di parametro 2.5 (\(X\sim\text{Pois}(\lambda=2.5)\)). Qual è la probabilità di trovare almeno 2 clienti in coda (\(X\ge 2\))?
...
2.b (\textbf{Punti 3}) Un'urna contiene 5 palline: 2 rosse e 3 bianche. Si estrae con reintroduzione finché non escono 2 bianche consecutive. Calcolare la probabilità di finire in \textbf{esattamente} 3 estrazioni.
...
2.c (\textbf{Punti 2}) Se \(X\sim N(3.2,1.5)\) e \(Y\sim N(1.2,1.1)\), \(X\) e \(Y\) indipendenti, come si distribuiscono \(X+Y\) e \(X-Y\)?

2.d (\textbf{Punti 2}) Sia \(X\) una VC e sia \(F\) la sua funzione di ripartizione. Cosa significa dire che \(F\) è crescente?

\hypertarget{esercizio-3-20}{%
\subsection{Esercizio 3}\label{esercizio-3-20}}

3.a (\textbf{Punti 14}) Un'urna contiene 4 palline numerate con \(\fbox{0}\), \(\fbox{3}\), \(\fbox{4}\) e \(\fbox{6}\). Si estrae 100 volte con reinserimento. Qual è la probabilità che la somme delle 100 estrazioni sia maggiore di 310?
...
\hypertarget{esercizio-4-20}{%
\subsection{Esercizio 4}\label{esercizio-4-20}}

4.a (\textbf{Punti 3}) Sia \(\hat \pi\) lo stimatore di massima verosimiglianza di \(\pi\) del modello Binomiale.
\begin{eqnarray*}
  \hat\pi &=&  \frac 1n\sum_{i=1}^nx_i\\
\end{eqnarray*}
Ricavare il suo \(MSE\) (Mean Squared Error).

4.b (\textbf{Punti 3}) Si \(h\) uno stimatore per \(\theta\) tale che \(E(h)=\theta\) e \(\lim_{n\to\infty} V(h)=0\). Di quale proprietà gode \(h\)?

4.c (\textbf{Punti 3}) Definire la significatività di un test.

4.d (\textbf{Punti 3}) In uno studio sull'efficacia della formazione nella scelta degli investimenti finanziari sono stati analizzati due gruppi di investitori, un primo gruppo senza alcuno studio specifico e un secondo gruppo di laureati in scienze economiche o affini. Sotto ipotesi di eterogeneità della varianze si è messo a test l'ipotesi che il rendimento medio degli investimenti dei laureati in scienze economiche \(\mu_E\) sia uguale a quello dei non laureati \(\mu_N\), contro l'alternativa che \(\mu_E>\mu_N\). Il test ha restituito un \(p_\text{value}\) pari a 0.092. Cosa possiamo concludere?

\hypertarget{esercizio-5-18}{%
\subsection{Esercizio 5}\label{esercizio-5-18}}

5.a (\textbf{Punti 12}) In uno studio sul gradimento dell'azione politica della regione, nel comune \(A\) si è rilevato che 26 persone su 35 intervistati è soddisfatto, mentre nel comune \(B\), 45 su 50 sono soddisfatti.
Testare all'1\% l'ipotesi che la proporzione di disoccupati sia uguale nei due comuni, contro l'alternativa che si \textbf{maggiore} nel comune \(B\).

5.b (\textbf{Punti 2}) Calcolare e interpretare il \(p_\text{value}\) del test precedente.
...
\(\fbox{B}\) SCELTA E CALCOLO STATISTICA-TEST, \(Z\)\[\hat\pi_\text{ A }=\frac{s_\text{ A }}{n_\text{ A }}=\frac{ 26 }{ 35 }= 0.7429 \qquad
...
\[z_{\text{obs}} =  -1.922   >  z_{ 0.01 } =  -2.326 \]CONCLUSIONE: i dati sono coerenti con \(H_{0}\) al LdS del 1 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-336-1} \end{center}
...
\begin{eqnarray*}
           ...
           \hat\sigma_X^2&=&\frac 1 n\sum_{i=1}^n x_i^2-\bar x^2=\frac {1}{ 150 }  51.33  - 0.5129 ^2= 0.0792 \\
           \hat\sigma_Y^2&=&\frac 1 n\sum_{i=1}^n y_i^2-\bar y^2=\frac {1}{ 150 }  71.69  - 0.6856 ^2= 0.0079 \\
           \text{cov}(X,Y)&=&\frac 1 n\sum_{i=1}^n x_i~y_i-\bar x\bar y=\frac {1}{ 150 }  49.45 - 0.5129 \cdot 0.6856 = -0.0219 \\
           \hat\beta_1 &=& \frac{\text{cov}(X,Y)}{\hat\sigma_X^2} \\
                    &=& \frac{ -0.0219 }{ 0.0792 }  =  -0.2769 \\
           \hat\beta_0 &=& \bar y - \hat\beta_1 \bar x\\
                    &=&  0.6856 - (-0.2769) \times  0.5129 = 0.8276 
         \end{eqnarray*}\[\hat y_{X= 1.5 }=\hat\beta_0+\hat\beta_1 x= 0.8276 + (-0.2769) \times 1.5 = 0.4123 \]
...
\begin{sol}
\begin{eqnarray*}
r&=&\frac{\text{cov}(X,Y)}{\sigma_X\sigma_Y}=\frac{ -0.0219 }{ 0.2814 \times 0.0888 }= -0.8775 \\r^2&=& 0.77 > 0.75
\end{eqnarray*}
Il modello si adatta bene ai dati.
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-340-1} \end{center}

6.d (\textbf{Punti 2}) Se \(W= 1 - Y\), quanto varrà \(r_{XW}\), coefficiente di correlazione tra \(X\) e \(W\)?

\hypertarget{prova-di-statistica-20230111-3}{%
\section{Prova di Statistica 2023/01/11-3}\label{prova-di-statistica-20230111-3}}

\hypertarget{esercizio-1-22}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-344-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Quante famiglie investono meno di 4.5 mila euro all'anno?
...
1.d (\textbf{Punti 2}) La spesa media è pari a \(\bar x=4.9926\), mentre la SD è pari a \(SD=1.5124\).
Se ogni famiglia diminuisse il proprio investimento del 2\%, quanto varrebbero la media e la SD dei dati così trasformati?

\hypertarget{esercizio-2-22}{%
\subsection{Esercizio 2}\label{esercizio-2-22}}

2.a (\textbf{Punti 14}) Un'urna contiene 20 palline: 8 bianche e 12 nere, si estrae con reintroduzione 5 volte.
...
2.b (\textbf{Punti 3}) Dalla stessa urna di prima, si estrae con reintroduzione finché non escono due palline bianche consecutive. Qual è la probabilità di finire in \textbf{al massimo} 3 tentativi?
...
2.c (\textbf{Punti 2}) Se \(X\sim N(3.2,1.1)\) e \(Y\sim\text{Binom}(12,0.3)\), \(X\) e \(Y\) indipendenti, quali sono valore atteso e varianza di \(X+Y\) e di \(X-Y\)?

2.d (\textbf{Punti 2}) Sia \(X\) una VC e sia \(F\) la sua funzione di ripartizione. Quanto valgono \(\lim_{x\to-\infty}F(x)\) e \(\lim_{x\to+\infty}F(x)\)?

\hypertarget{esercizio-3-21}{%
\subsection{Esercizio 3}\label{esercizio-3-21}}

3.a (\textbf{Punti 14}) Un'urna contiene 4 palline numerate: \(\fbox{0}\), \(\fbox{1}\), \(\fbox{4}\) e \(\fbox{6}\). Si estrae 100 volte con reinserimento. Qual è la probabilità che la media delle 100 estrazioni sia maggiore di 2.8?
...
\hypertarget{esercizio-4-21}{%
\subsection{Esercizio 4}\label{esercizio-4-21}}

4.a (\textbf{Punti 3}) Siano \(\hat \mu\) e \(\hat\sigma^2\) gli stimatori di massima verosimiglianza di \(\mu\) e \(\sigma^2\) del modello di Normale.
\begin{eqnarray*}
...
4.b (\textbf{Punti 3}) Si \(h\) uno stimatore per \(\theta\) tale che \(E(h)\ne\theta\), ma che \(\lim_{n\to\infty}E(h)=\theta\). Di quale proprietà gode \(h\)?

4.c (\textbf{Punti 3}) Quali sono gli errori di primo e secondo tipo?
...
5.b (\textbf{Punti 7}) Un'indagine analoga, svolta sull'intera regione, ha mostrato un gradimento medio pari a \(\mu_0=71\). Testare al livello di significatività del 5\% l'ipotesi che nel comune A il livello di gradimento sia uguale a quello regionale contro l'alternativa che sia \textbf{minore}.
...
\begin{eqnarray*}
   ...
   \end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 35 -1);\, 0.05 } = -1.691\).
\[t_{\text{obs}} =  -3.499   <  t_{ 34 ;\,  0.05 } =  -1.691 \]
CONCLUSIONE: i dati non sono coerenti con \(H_{0}\) al LdS del 5 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-349-1} \end{center}
...
\begin{eqnarray*}
           ...
           \hat\sigma_X^2&=&\frac 1 n\sum_{i=1}^n x_i^2-\bar x^2=\frac {1}{ 150 }  54.9  - 0.532 ^2= 0.083 \\
           \hat\sigma_Y^2&=&\frac 1 n\sum_{i=1}^n y_i^2-\bar y^2=\frac {1}{ 150 }  72.4  - 0.6287 ^2= 0.0874 \\
           \text{cov}(X,Y)&=&\frac 1 n\sum_{i=1}^n x_i~y_i-\bar x\bar y=\frac {1}{ 150 }  62.7 - 0.532 \cdot 0.6287 = 0.0835 \\
           \hat\beta_1 &=& \frac{\text{cov}(X,Y)}{\hat\sigma_X^2} \\
                    &=& \frac{ 0.0835 }{ 0.083 }  =  1.007 \\
           \hat\beta_0 &=& \bar y - \hat\beta_1 \bar x\\
                    &=&  0.6287 - 1.0069 \times  0.532 = 0.093 
         \end{eqnarray*}\[\hat y_{X= 1.5 }=\hat\beta_0+\hat\beta_1 x= 0.093 + 1.0069 \times 1.5 = 1.603 \]

\end{sol}

6.b (\textbf{Punti 3}) Calcolare e interpretare \(R^2\).

\begin{sol}
\begin{eqnarray*}
r&=&\frac{\text{cov}(X,Y)}{\sigma_X\sigma_Y}=\frac{ 0.0835 }{ 0.2881 \times 0.2957 }= 0.9808 \\r^2&=& 0.962 > 0.75
\end{eqnarray*}
Il modello si adatta bene ai dati.
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-353-1} \end{center}

6.d (\textbf{Punti 2}) Se \(W=1+ Y\), quanto varrà \(r_{XW}\), coefficiente di correlazione tra \(X\) e \(W\)?

\hypertarget{prova-di-statistica-20230216-1}{%
\section{Prova di Statistica 2023/02/16-1}\label{prova-di-statistica-20230216-1}}

\hypertarget{esercizio-1-23}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-357-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Quale è il numero di imprese che hanno un utile maggiore di zero?
...
1.c (\textbf{Punti 2}) L'utile medio è pari a \(\bar x=1.7013\) che forma avrà l'istogramma?

1.d (\textbf{Punti 2}) Qual è la proprietà di associatività della media aritmetica?

\hypertarget{esercizio-2-23}{%
\subsection{Esercizio 2}\label{esercizio-2-23}}

2.a (\textbf{Punti 14}) Siano \(X\sim N(1,1.5)\) e sia \(Y\sim N(1,0.5)\), \(X\) e \(Y\) indipendenti. Posto \(A=\{-2<X<1\}\), \(B=\{Y<1\}\).
...
2.b (\textbf{Punti 3}) Un'urna contiene 10 palline rosse, 10 bianche e 10 nere. Si estrae tre volte senza reinserimento. Qual è la probabilità di avere tre colori uguali?
...
2.c (\textbf{Punti 2}) Se \(X\sim\text{Ber}(0.2)\) e \(Y\sim\text{Ber}(0.2)\), \(X\) e \(Y\) indipendenti, com'è distribuita \(X+Y\)?

2.d (\textbf{Punti 2}) Se \(X\) è \(Y\) sono due VC, tali che \(V(X)=\sigma_X^2\) e \(Y=\sigma_Y^2\), quanto vale \(V(X-Y)\)?

\hypertarget{esercizio-3-22}{%
\subsection{Esercizio 3}\label{esercizio-3-22}}

3.a (\textbf{Punti 14}) Un'urna contiene 4 palline numerate: \(\fbox{0}\), \(\fbox{1}\), \(\fbox{2}\) e \(\fbox{3}\). Si estrae 100 volte con reinserimento. Qual è la probabilità che la media delle 100 estrazioni sia maggiore di 1.6?
...
4.b (\textbf{Punti 3}) Sia \(h\) uno stimatore per \(\theta\). Cosa significa dire che \(h\) è consistente?

4.c (\textbf{Punti 3}) Siano \(T_1\) e \(T_2\) due test per lo stesso sistema d'ipotesi e con probabilità di errore di secondo tipo pari a, \(\beta_1=0.2\) e \(\beta_2=0.3\), rispettivamente. A parità di significatività, quale dei due è più potente? Perché?
...
\hypertarget{esercizio-5-20}{%
\subsection{Esercizio 5}\label{esercizio-5-20}}

(\textbf{Punti 14}) In uno studio sull'efficacia degli integratori alimentari, su un gruppo di 238 atleti è stato misurato il rendimento atletico (ottimo, buono e scarso) e l'assunzione di integratori (alto, medio e basso). Qui di seguito i dati:
...
\addlinespace[0.3em]
\multicolumn{4}{l}{\textbf{rendimento}}\\
\hspace{1em}ottimo & 17.60 & 29.53 & 23.87\\
\hspace{1em}buono & 18.84 & 31.61 & 25.55\\
\hspace{1em}scarso & 22.56 & 37.85 & 30.59\\
\bottomrule
\end{tabular}
\end{table}
...
\addlinespace[0.3em]
\multicolumn{4}{l}{\textbf{rendimento}}\\
\hspace{1em}ottimo & 0.6565 & 1.0118 & 3.2934\\
\hspace{1em}buono & 0.0714 & 0.9968 & 0.7765\\
\hspace{1em}scarso & 0.9213 & 0.0006 & 0.6363\\
\bottomrule
\end{tabular}
\end{table}
...
\text{gdl} &=& (3-1)\times (3-1) = 4\\
   \chi^2_\text{obs}&=&8.3644\\
   \chi^2_{0.05;4}&=&9.4877\\
\end{eqnarray*}

\end{sol}

\hypertarget{esercizio-6-20}{%
...
6.a (\textbf{Punti 14}) Stimare la previsione per \(x=3.5\) nel modello di regressione dove \(Y\) viene spiegata da \(X\).
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-365-1} \end{center}

6.d (\textbf{Punti 2}) Se \(W=- Y\), quanto varrà \(r_{XW}\), coefficiente di correlazione tra \(X\) e \(W\)?

\hypertarget{prova-di-statistica-20230216-1-1}{%
\section{Prova di Statistica 2023/02/16-1}\label{prova-di-statistica-20230216-1-1}}

\hypertarget{esercizio-1-24}{%
...
1.a (\textbf{Punti 14}) Individuare il valore approssimato della mediana.
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-370-1} \end{center}
...
1.c (\textbf{Punti 2}) L'utile medio è pari a \(\bar x=-1.662\) che forma avrà l'istogramma?

1.d (\textbf{Punti 2}) Qual è la proprietà di linearità della media aritmetica?

\hypertarget{esercizio-2-24}{%
\subsection{Esercizio 2}\label{esercizio-2-24}}

2.a (\textbf{Punti 14}) Il numero di telefonate in arrivo nell'ora di punta di un centralino è distribuito come una Poisson di parametro 3.1 (\(X\sim\text{Pois}(\lambda=3.1)\)). Qual è la probabilità di trovare al massimo 3 telefonate (\(X\le 3\))?
...
2.c (\textbf{Punti 2}) Se \(X\sim N(\mu_X,\sigma_X^2)\) e \(Y\sim N(\mu_Y,\sigma_Y^2)\), \(X\) e \(Y\) indipendenti, come si distribuiscono \(X+Y\) e \(X-Y\)?

2.d (\textbf{Punti 2}) Siano \(A\) e \(B\) due eventi diversi dal vuoto tali che \(A\cap B=\emptyset\), motivare perché \(A\) e \(B\) non possono essere indipendenti.

\hypertarget{esercizio-3-23}{%
\subsection{Esercizio 3}\label{esercizio-3-23}}

3.a (\textbf{Punti 14}) Un'urna contiene 2 palline numerate con \(\fbox{0}\), 3 numerate con \(\fbox{1}\) e 2 numerate con \(\fbox{2}\). Si estrae 100 volte con reinserimento. Qual è la probabilità che la somma delle 100 estrazioni sia compresa tra 80 e 120?
...
\hypertarget{esercizio-5-21}{%
\subsection{Esercizio 5}\label{esercizio-5-21}}

5.a (\textbf{Punti 14}) In uno studio comparativo tra i redditi, nel comune \(A\) si è rilevato il reddito di 12 individui e si è osservata una media pari 27 mila euro con una standard deviation pari a 4.2 mila euro , mentre nel comune \(B\) si è rilevato il reddito di 25 individui e si è osservata una media pari 24 mila euro con una standard deviation pari a 3.1 mila euro.
Sotto ipotesi di eterogeneità, testare al livello di \textbf{significatività del'5\% e dell'1\%} l'ipotesi che il reddito medio sia uguale nei due comuni, contro l'alternativa che sia maggiore nel comune \(A\).
...
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 12 + 25 -2);\, 0.01 } = 2.44\).
\[t_{\text{obs}} =  2.12   <  t_{ 35 ;\,  0.01 } =  2.44 \]
CONCLUSIONE: i dati sono coerenti con \(H_{0}\) al LdS del 1 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-375-1} \end{center}

Il \(p_{\text{value}}\) è
\[ P(T_{n1+n2-2}>t_{\text{obs}})=P(T_{n1+n2-2}> 2.119 )= 0.02062 \]

\end{sol}

\hypertarget{esercizio-6-21}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-379-1} \end{center}

6.d (\textbf{Punti 2}) Se \(V= 1 - Y\) e \(W= 1 - X\) quanto varrà \(r_{VW}\), coefficiente di correlazione tra \(V\) e \(W\)?

\hypertarget{prova-di-statistica-20230216-3}{%
\section{Prova di Statistica 2023/02/16-3}\label{prova-di-statistica-20230216-3}}

\hypertarget{esercizio-1-25}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-384-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Quante imprese hanno un utile negativo?
...
1.d (\textbf{Punti 2}) La varianza dei dati è pari a \(\hat\sigma^2=15.1386\), se gli utili di ogni impresa aumentassero del 3\%, quanto varrebbe la varianza dei dati così trasformati?
...
\hypertarget{esercizio-2-25}{%
\subsection{Esercizio 2}\label{esercizio-2-25}}

2.a (\textbf{Punti 14}) Si lancia una moneta perfetta 8 volte. Qual è la probabilità di avere un numero di volte Testa maggiore o uguale a 6 su 8 lanci?
...
2.b (\textbf{Punti 3}) Sia \(Z\sim N(0,1)\) e siano \(A=\{Z<0\}\) e \(B=\{Z<1\}\). Calcolare \(P(A|B)\)
...
2.c (\textbf{Punti 2}) Se \(X\sim\text{Pois} (n)\), a cosa tende \(X\) se \(n\) diverge?
...
2.d (\textbf{Punti 2}) Sia \(X\sim N(10,1)\) e sia \(F\) la sua funzione di ripartizione. Disegnare approssimativamente \(F(x)\) per \(x\) che varia tra 6 e 14.

\hypertarget{esercizio-3-24}{%
\subsection{Esercizio 3}\label{esercizio-3-24}}

3.a (\textbf{Punti 14}) Un'urna conteni tre palline rosse, due bianche e una nera. Si estrae \(n=100\) volte con reintroduzione. Qual è la probabilità che la proporzione di palline rosse sia minore di 0.45?
...
4.b (\textbf{Punti 3}) Se \(h_1\) e \(h_2\) sono due stimatori per \(\theta\) cosa significa dire che \(h_1\) è più efficiente di \(h_2\)?

4.c (\textbf{Punti 3}) Definire gli errori di primo e secondo tipo e le relativa probabilità.

4.d (\textbf{Punti 3}) Una moneta, che non sappiamo se è perfetta oppure no, viene lanciata 40 volte. Abbiamo osservato 14 volte testa su 40 lanci. Posto \(\pi\) la probabilità che la moneta mostri testa, si è testato
\[
\begin{cases}
H_0:\pi=\frac 12\\
H_1:\pi\ne\frac 12
...
\hypertarget{esercizio-5-22}{%
\subsection{Esercizio 5}\label{esercizio-5-22}}

5.a (\textbf{Punti 7}) In uno studio sui redditi, nel comune \(A\) si è rilevato il reddito di \(n=35\) individui e si è osservata una media pari \(68\) mila euro con una standard deviation pari a \(5\) mila euro.
...
5.b (\textbf{Punti 7}) Un'indagine analoga, svolta sull'intera regione, ha mostrato un reddito medio pari a \(\mu_0=71\). Testare al livello di \textbf{significatività del 5\%} l'ipotesi che nel comune \(A\) il reddito medio sia uguale a quello regionale contro l'alternativa che sia \textbf{diverso}.
...
\begin{eqnarray*}
   ...
   \end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 35 -1);\, 0.025 } = -2.032\).
\[t_{\text{obs}} =  -3.499   <  t_{ 34 ;\,  0.025 } =  -2.032 \]
CONCLUSIONE: i dati non sono coerenti con \(H_{0}\) al LdS del 5 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-389-1} \end{center}
...
6.c (\textbf{Punti 2}) Interpretare il qq-plot dei residui.

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-393-1} \end{center}

6.d (\textbf{Punti 2}) Se \(V=1+ Y\) e \(W=1-X\), quanto varrà \(r_{VW}\), il coefficiente di correlazione tra \(V\) e \(W\)?

\hypertarget{prova-di-statistica-20230608-1}{%
\section{Prova di Statistica 2023/06/08-1}\label{prova-di-statistica-20230608-1}}

\hypertarget{esercizio-1-26}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-397-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Qual è la percentuale di famiglie con spesa superiore a 4 mila euro?
...
1.c (\textbf{Punti 2}) Che relazione dobbiamo attenderci tra moda, mediana e media?

1.d (\textbf{Punti 2}) Cosa significa che la media aritmetica rende zero la somma degli scarti?

\hypertarget{esercizio-2-26}{%
...
2.a (\textbf{Punti 14}) Calcolare la probabilità di \(A\cup B\).
...
2.b (\textbf{Punti 3}) Calcolare la probabilità che \textbf{solo} uno dei due eventi \(A\) oppure \(B\) sia vero.

2.c (\textbf{Punti 2}) Calcolare valore atteso e varianza di \(X_A-X_B\).

2.d (\textbf{Punti 2}) Sia \(X\sim\text{Ber}(\pi=0.4)\). Disegnare la sua funzione di ripartizione.

\hypertarget{esercizio-3-25}{%
\subsection{Esercizio 3}\label{esercizio-3-25}}

(\textbf{Punti 14}) Un'urna contiene 3 palline Rosse, 3 Bianche e 4 Blu. Si estrae senza reintroduzione per \(n=81\) volte.
...
4.b (\textbf{Punti 3}) Se \(h_1\) e \(h_2\) sono due stimatori per \(\theta\) tale per cui \(MSE(h_1)=1.2\), e \(MSE(h_2)=2.3\), quale dei due è più efficiente e perché?

4.c (\textbf{Punti 3}) Definire gli errori di primo e secondo tipo e le relative probabilità.

4.d (\textbf{Punti 3}) Una moneta, che non sappiamo se è perfetta oppure no, viene lanciata 80 volte. Abbiamo osservato 28 volte testa su 80 lanci. Posto \(\pi\) la probabilità che la moneta mostri testa, si è testato
\[
\begin{cases}
H_0:\pi=\frac 12\\
H_1:\pi\ne\frac 12
...
5.a (\textbf{Punti 14}) Stimare il modello di regressione dove \(Y\) viene spiegata da \(X\).
...
5.b (\textbf{Punti 3}) Qual è la percentuale di varianza spiegata dal modello?
...
5.c (\textbf{Punti 14}) Testare l'ipotesi che l'intercetta sia uguale a zero contro l'alternativa che sia diversa da zero al livello di significatività del 5 percento.
...
V(\hat\beta_{0}) &=& \sigma_{\varepsilon}^{2} \left( \frac{1} {n}  +  \frac{\bar{x}^{2}} {n \hat{\sigma}^{2}_{X}} \right)\\
\widehat{V(\hat\beta_{0})} &=& S_{\varepsilon}^{2}\left( \frac{1} {n}  +  \frac{\bar{x}^{2}} {n \hat{\sigma}^{2}_{X}} \right)\ \\
 &=&  10.4 \times\left( \frac{1} { 50 }  +  \frac{ 13.8 ^{2}} { 50 \times  76.92 } \right)\\
 \widehat{SE(\hat\beta_{0})}        &=&  \sqrt{ 0.7236 }\\
 &=&  0.8506 
\end{eqnarray*}
...
\end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{(50-2);\, 0.025} = 2.011\).
\[t_{\text{obs}} = 1.274 < t_{48;\, 0.025} = 2.011\]
CONCLUSIONE: i dati \textbf{sono} coerenti con \(H_{0}\) al LdS del 2.5\%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-405-1} \end{center}
...
5.d (\textbf{Punti 2}) Interpretare il diagramma dei residui.

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-406-1} \end{center}

5.e (\textbf{Punti 2}) Perché una previsione per \(x=15\) è più affidabile di una per \(x=50\)?

\hypertarget{prova-di-statistica-20230608-2}{%
\section{Prova di Statistica 2023/06/08-2}\label{prova-di-statistica-20230608-2}}

\hypertarget{esercizio-1-27}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-410-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Qual è la percentuale di famiglie con spesa compresa tra a 4 mila e 8 mila euro?

\begin{sol}
\begin{eqnarray*}
     \%( 4 <X< 8 ) &=& ( 5 - 4 )\times h_{ 1 }+ f_{ 2 }\times 100 +( 8 - 7 )\times h_{ 2 } \\
...
1.d (\textbf{Punti 2}) Cosa significa che la media aritmetica gode della proprietà di linearità?

\hypertarget{esercizio-2-27}{%
...
2.b (\textbf{Punti 3}) Si continua ad estrarre da \(X_A\sim N(1.5,1.5)\) e si interrompe quando \(X_A\le 1\). Calcolare la probabilità di interrompere alla quinta estrazione.
...
2.c (\textbf{Punti 2}) Come si distribuisce \(X_A-X_B\)?

2.d (\textbf{Punti 2}) Se \(A\) e \(B\) sono due eventi tali che \(A\cap B=\emptyset\), \(A\) e \(B\) possono essere indipendenti? Perché?

\hypertarget{esercizio-3-26}{%
\subsection{Esercizio 3}\label{esercizio-3-26}}

(\textbf{Punti 14}) Un'urna contiene 3 palline col numero \(\fbox{0}\), 3 col numero \(\fbox{1}\) e 4 col numero \(\fbox{2}\). Si estrae senza reintroduzione per \(n=81\) volte.
...
\hypertarget{esercizio-4-26}{%
\subsection{Esercizio 4}\label{esercizio-4-26}}

4.a (\textbf{Punti 3}) Sono stati estratti \(n=10\) valori da una Poisson di parametro \(\lambda\) incognito e si è ottenuta un media pari a \(\bar x=3.4\). Ricavare \(\widehat{SE(\hat\lambda)}\) lo Standard Error stimato \(\hat\lambda\) di massima verosimiglianza.

4.b (\textbf{Punti 3}) Sia \(h\) uno stimatore per \(\theta\). Cosa significa dire che \(h\) è consistente?

4.c (\textbf{Punti 3}) Siano \(T_1\) e \(T_2\) due test per lo stesso sistema d'ipotesi e con probabilità di errore di secondo tipo pari a, \(\beta_1=0.02\) e \(\beta_2=0.3\), rispettivamente. A parità di significatività, quale dei due è più potente? Perché?
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-419-1} \end{center}

6.d (\textbf{Punti 2}) Posto \(W=-2\cdot Y\) calcolare \(\beta_0'\) e \(\beta_1'\) i coefficienti di regressione del modello
\[
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-423-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Qual è il numero di famiglie con spesa compresa tra a 4 mila e 8 mila euro?

\begin{sol}
\begin{eqnarray*}
     \%( 4 <X< 8 ) &=& ( 5 - 4 )\times h_{ 3 }+ f_{ 4 }\times 100 +( 8 - 6.5 )\times h_{ 4 } \\
...
1.d (\textbf{Punti 2}) Cosa significa che la media aritmetica goe della proprietà di linearità?

\hypertarget{esercizio-2-28}{%
...
2.b (\textbf{Punti 3}) Si estrae ripetutamente da \(X\sim\text{Binom}(5,0.5)\) e ci si ferma quando \(X\le 1\). Calcolare la probabilità di finire entro due estrazioni.
...
2.c (\textbf{Punti 2}) Calcolare valore atteso e varianza di \(X-Y\).

2.d (\textbf{Punti 2}) Siano \(A\) e \(B\) due eventi tali che \(P(A)=0.6\) e \(P(B)=0.5\). \(A\) e \(B\) possono essere incompatibili? Perché?

\hypertarget{esercizio-3-27}{%
\subsection{Esercizio 3}\label{esercizio-3-27}}

(\textbf{Punti 14}) Un'urna contiene 3 palline col numero \(\fbox{0}\), 4 col numero \(\fbox{1}\) e 3 col numero \(\fbox{2}\). Si estrae senza reintroduzione per \(n=81\) volte.
...
\hypertarget{esercizio-4-27}{%
\subsection{Esercizio 4}\label{esercizio-4-27}}

4.a (\textbf{Punti 3}) Sono stati estratti \(n=10\) valori da una Bernoulli di parametro \(\pi\) incognito e si è ottenuti 4 successi in 10 estrazioni. Ricavare, \(\widehat{SE(\hat\pi)}\) lo Standard Error stimato di \(\hat\pi\) di massima verosimiglianza.

4.b (\textbf{Punti 3}) Sia \(h\) uno stimatore per \(\theta\). Cosa significa dire che \(h\) è corretto asintoticamente?

4.c (\textbf{Punti 3}) Definire gli errori di primo e di secondo tipo e le relative probabilità.

4.d (\textbf{Punti 3}) Un dado viene lanciato 60 volte e si ottiene
...
\hypertarget{esercizio-5-25}{%
\subsection{Esercizio 5}\label{esercizio-5-25}}

5.a (\textbf{Punti 7}) In uno studio sui consumi per generi di igiene personale su un campione di 35 famiglie della provincia di Reggio è stata chiesta la loro spesa annuale in questo genere di beni. I dati campionari hanno evidenziato una media pari a \(\hat\mu=0.5\) e una deviazione standard osservata pari a \(\hat\sigma=0.25\).
Costruire un intervallo di confidenza al 95\% per il gradimento medio \(\mu\).

\begin{sol}
\[
     S =\sqrt{\frac {n}{n-1}}\cdot\hat\sigma =
     \sqrt{\frac { 35 }{ 34 }}\cdot 0.2536 = 0.2574 \]
\begin{eqnarray*}
  Idc: & &  \mu \pm  t_{n-1;\alpha/2} \times \frac{ S }{\sqrt{n}} \\
     & &  0.5 \pm  2.032 \times \frac{ 0.2536 }{\sqrt{ 35 }} \\
     & &  0.5 \pm  2.032 \times  0.0435 \\
     & & [ 0.4116 ,  0.5884 ]
  \end{eqnarray*}

\end{sol}

5.b (\textbf{Punti 7}) Un'indagine analoga, svolta sull'intera regione, ha mostrato un gradimento medio pari a \(\mu_0=0.6\). Testare al livello di \textbf{significatività del 5\%} l'ipotesi che nel comune A il livello di gradimento sia uguale a quello regionale contro l'alternativa che sia \textbf{minore}.
...
\begin{eqnarray*}
   S
   &=& \sqrt{\frac{n} {n-1}}\ \widehat{\sigma}
   =  \sqrt{\frac{ 160 } { 160 -1}} \times  0.25  =  0.2508  
   \end{eqnarray*}
\begin{eqnarray*}
   \frac{\hat\mu - \mu_{0}} {S/\,\sqrt{n}}&\sim&t_{n-1}\\
   t_{\text{obs}}
   &=& \frac{ ( 0.5 -  0.6 )} { 0.2508 /\sqrt{ 160 }}
   =   -5.044 \, .
   \end{eqnarray*}
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 160 -1);\, 0.05 } = -1.655\).
\[t_{\text{obs}} =  -5.044   <  t_{ 159 ;\,  0.05 } =  -1.655 \]
CONCLUSIONE: i dati non sono coerenti con \(H_{0}\) al LdS del 5 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-430-1} \end{center}
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-434-1} \end{center}

6.d (\textbf{Punti 2}) Posto \(W=-2\cdot X\) calcolare \(\beta_0'\) e \(\beta_1'\) i coefficienti di regressione del modello
\[
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-438-1} \end{center}
...
1.d (\textbf{Punti 2}) La spesa media è pari a \(\bar x=70.6296\) e la standard deviation \(\hat\sigma=56.9043\), se tutte le famiglie risparmiassero
il 10\%, come cambierebbero la media e la standard deviation?

\hypertarget{esercizio-2-29}{%
...
2.a (\textbf{Punti 14}) Calcolare la probabilità che il totale di telefonate in arrivo in un'ora sia maggiore
...
2.b (\textbf{Punti 3}) Sapendo che \(S=X+Y\ge 3\), calcolare la probabilità che \(S=X+Y= 5\) (\(P(S=5|S\ge 3\))
...
2.c (\textbf{Punti 2}) Se \(X\) ed \(Y\) sono due variabili casuali, è sempre vero che
\[
V(X-Y)=V(X)+V(Y)~~~?
\]
motivare la risposta.

2.d (\textbf{Punti 2}) Sia \(X\sim\text{Binom}(2,\pi=0.5)\). Disegnare la sua funzione di ripartizione.

\begin{sol}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-441-1} \end{center}

\end{sol}

\hypertarget{esercizio-3-28}{%
\subsection{Esercizio 3}\label{esercizio-3-28}}

(\textbf{Punti 14}) Una catena di montaggio a ciclo continuo produce ogni giorno un numero di pezzi variabile, con una media pari a
...
4.b (\textbf{Punti 3}) Sia \(h\) uno stimatore per \(\theta\) tale che
\[V(h)=\frac\theta {\sqrt{n}};~~~~E(h)=\theta\frac{n+2}{n}\]
di quali proprietà gode \(h\)?

4.c (\textbf{Punti 3}) Definire il p-value e descrivere la sua interpretazione.

4.d (\textbf{Punti 3}) Se un test è significativo al 5\% può essere significativo all'1\%? (scegliere la risposta tra: \emph{mai}, \emph{non sempre} oppure \emph{sempre} e motivare la risposta)

\hypertarget{esercizio-5-26}{%
...
5.a (\textbf{Punti 7}) Costruire un intervallo di confidenza al 99\%
...
5.b (\textbf{Punti 7}) Nel comune \(B\) si è condotta un'intervista analoga.
Sono state intervistate 230 persone e 75 di loro sono favorevoli. Testare al 5\%
...
\(\fbox{B}\) SCELTA E CALCOLO STATISTICA-TEST, \(Z\)\[\hat\pi_\text{ A }=\frac{s_\text{ A }}{n_\text{ A }}=\frac{ 75 }{ 230 }= 0.3261 \qquad
   \hat\pi_\text{ B }=\frac{s_\text{ B }}{n_\text{ B }}=\frac{ 70 }{ 250 }= 0.28 \]Calcoliamo la proporzione comune sotto \(H_0\)
\[
     \pi_C=\frac{s_\text{ A }+s_\text{ B }}{n_\text{ A }+n_\text{ B }}=
     \frac{ 145 }{ 480 }= 0.3021 
   \]\begin{eqnarray*}
   ...
   \end{eqnarray*}
\(\fbox{C}\) DECISIONE Dalle tavole si ha \(z_{ 0.05 } = 1.645\).
\[z_{\text{obs}} =  1.099   <  z_{ 0.05 } =  1.645 \]CONCLUSIONE: i dati sono coerenti con \(H_{0}\) al LdS del 5 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-444-1} \end{center}
...
6.a (\textbf{Punti 14}) Stimare il modello di regressione dove \(Y\) viene spiegata da \(X\) e interpretare
...
6.b (\textbf{Punti 3}) Scrivere la scomposizione della varianza e
...
\begin{eqnarray*}
   ...
   RSS &=& (1-R^2)\cdot TSS\\
      &=& (1- 0.8451 )\cdot 38.84 \\
      &=&  6.017 \\
   TSS &=& RSS+TSS \\ 38.84  &=&  32.83 + 6.017 
  \end{eqnarray*}

\end{sol}

6.c (\textbf{Punti 2}) Perché una previsione per \(x=0.15\) è più affidabile di una per \(x=50\)?

6.d (\textbf{Punti 2}) Interpretare il diagramma dei residui.

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-448-1} \end{center}

6.e (\textbf{Punti 2}) Cosa significa dire che \(r\) è un numero puro?

\hypertarget{prova-di-statistica-20230627-2}{%
\section{Prova di Statistica 2023/06/27-2}\label{prova-di-statistica-20230627-2}}

\hypertarget{esercizio-1-30}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-452-1} \end{center}

\end{sol}

1.b (\textbf{Punti 3}) Qual è il numero di imprese con spesa superiore ai 100 euro?
...
1.d (\textbf{Punti 2}) La spesa media è pari a \(\bar x=180.6032\) e varianza \(\hat\sigma^2=3061.3555\), se tutte le famiglie aumentassero
del 10\% la loro spesa, come cambierebbero la media e la varianza?

\hypertarget{esercizio-2-30}{%
...
2.b (\textbf{Punti 3}) Si lancia una moneta, se esce testa si estrae da una normale \(X\sim N(0,1)\), se esce croce si estrae
...
2.c (\textbf{Punti 2}) Se \(X\sim\text{Pois}(\lambda_X)\) e \(Y\sim\text{Pois}(\lambda_Y)\), quali sono valore atteso e varianza
di \(X-Y\)?

2.d (\textbf{Punti 2}) Se \(A\) e \(B\) sono due eventi \emph{non} indipendenti come si può scrivere \(P(A\cap B)\)?

\hypertarget{esercizio-3-29}{%
...
4.c (\textbf{Punti 3}) Definire la significatività di un test statistico.

4.d (\textbf{Punti 3}) Se un test è significativo all'1\% può essere significativo all'5\%? (scegliere la risposta tra: \emph{mai}, \emph{non sempre} oppure \emph{sempre} e motivare la risposta)

\hypertarget{esercizio-5-27}{%
...
5.a (\textbf{Punti 7}) Costruire un intervallo di confidenza al 95\%
per la proporzione dei favorevoli in popolazione.
...
\(\fbox{C}\) DECISIONE
Dalle tavole si ha \(t_{( 25 + 24 -2);\, 0.025 } = 2.012\).
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-458-1} \end{center}
...
\begin{eqnarray*}
   ...
   RSS &=& (1-R^2)\cdot TSS\\
      &=& (1- 0.7061 )\cdot 36.24 \\
      &=&  10.65 \\
   TSS &=& RSS+TSS \\ 36.24  &=&  25.59 + 10.65 
  \end{eqnarray*}

\end{sol}

6.c (\textbf{Punti 2}) Perché una previsione per \(x=0.8\) è più affidabile di una per \(x=0\)?

6.d (\textbf{Punti 2}) Interpretare il diagramma dei residui.

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-462-1} \end{center}

6.e (\textbf{Punti 2}) Cosa significa dire che \(r\) è invariante ai cambiamenti di scala?

\hypertarget{prova-di-statistica-20230627-3}{%
\section{Prova di Statistica 2023/06/27-3}\label{prova-di-statistica-20230627-3}}

\hypertarget{esercizio-1-31}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-466-1} \end{center}
...
1.b (\textbf{Punti 3}) Qual è il numero di imprese con spesa superiore ai 60 euro?
...
1.d (\textbf{Punti 2}) Scrivere la proprietà di associatività della media aritmetica.

\hypertarget{esercizio-2-31}{%
...
2.b (\textbf{Punti 3}) Sia \(X\sim N(2,1)\) e sia \(A=\{X>2.5\}\).
...
2.d (\textbf{Punti 2}) Se due eventi \(A\ne\emptyset\) e \(B\ne\emptyset\) non sono indipendenti allora sono necessariamente incompatibili? Motivare la risposta.

\hypertarget{esercizio-3-30}{%
\subsection{Esercizio 3}\label{esercizio-3-30}}

(\textbf{Punti 14}) Una catena di montaggio a ciclo continuo produce un pezzo al giorno e la proporzione di pezzi fallati è pari
a \(\pi=0.15\).
...
4.b (\textbf{Punti 3}) 4.c (\textbf{Punti 3}) Sia \(h\) uno stimatore per \(\theta\) tale che
\[V(h)=2\frac\theta {\sqrt{n}};~~~~E(h)=\theta\frac{n+2}{n-1}\]
scrive il Mean Squared Error di \(h\) (\(MSE(h)\)).

4.d (\textbf{Punti 3}) Definire la potenza di un test statistico.

4.e (\textbf{Punti 3}) Se un t-test bilaterale con 13 gradi libertà presenta una
t osservata pari a \(t_\text{obs}=1.974\), il \(p_\text{value}\) sarà maggiore o minore di 0.05? Perché?

\hypertarget{esercizio-5-28}{%
...
5.b (\textbf{Punti 7}) Nella regione è stata condotta un'indagine analoga
di larga scala che ha evidenziato un gradimento medio pari a \(\mu_0=76\)
Testare al 5\% l'ipotesi che nel comune \(C\) il gradimento sia uguale a quello regionale contro l'alternativa che sia maggiore.
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-471-1} \end{center}
...
\begin{eqnarray*}
           ...
           \hat\sigma_X^2&=&\frac 1 n\sum_{i=1}^n x_i^2-\bar x^2=\frac {1}{ 4 }  1.755  - 0.6025 ^2= 0.0758 \\
...
6.c (\textbf{Punti 2}) Che differenza c'è tra interpolazione ed estrapolazione?

6.d (\textbf{Punti 2}) Definire il diagramma dei residui.

6.e (\textbf{Punti 2}) Cosa significa che \(\hat\beta_0\) e \(\hat\beta_1\) sono BLUE?

\hypertarget{prova-di-statistica-20230723-1}{%
\section{Prova di Statistica 2023/07/23-1}\label{prova-di-statistica-20230723-1}}

\hypertarget{esercizio-1-32}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-478-1} \end{center}

1.b (\textbf{Punti 3}) Qual è la percentuale di imprese con bilancio inferiore a 3 (centomila) euro?
...
1.c (\textbf{Punti 2}) la media aritmetica è pari a -2.6643, in base al punto 1a che relazione dobbiamo attenderci tra media, mediana e moda?

1.d (\textbf{Punti 2}) Si considerino i seguenti dati
...
2.a (\textbf{Punti 14}) Calcolare la probabilità che in almeno uno dei due semafori ci siano 2 o più automobili in coda (suggerimento: \emph{almeno uno} si rappresenta con l'unione).
...
2.b (\textbf{Punti 3}) Per andare al lavoro Michele prende la strada che
...
2.c (\textbf{Punti 2}) Sia \(Z\sim N(0,1)\) e \(Y\sim \chi^2_2\), \(Z\) e \(Y\) indipendenti.
Come si distribuisce
\[
\frac{Z}{\sqrt{Y/2}} ~~~?
\]

2.d (\textbf{Punti 2}) Se \(P(A)=0.3\), \(P(B|A)=0.6\), e \(P(B|\bar A)=0.4\), quanto valgono \(P(B)\) e \(P(A|B)\)?
...
\hypertarget{esercizio-3-31}{%
\subsection{Esercizio 3}\label{esercizio-3-31}}

(\textbf{Punti 14}) Un'urna contiene le seguenti palline numerate \{3,5,7,11\}. Si estrae con reintroduzione per \(n=81\) volte.
...
5.b (\textbf{Punti 10}) L'azienda afferma che la media delle spese mensili dei dipendenti è pari a \(1100€\). Effettuare un test di ipotesi al livello di significatività del 1\% per verificare se la media delle spese mensili sia superiore a \(1100€\).

\(\fbox{A}\) FORMULAZIONE DELLE IPOTESI
\[\begin{cases}
   H_0:\mu=\mu_0= 1100 \text{ € }\\
   H_1:\mu >  \mu_0= 1100 \text{ € }
   \end{cases}\]
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-486-1} \end{center}

Il \(p_{\text{value}}\) è
\[ P(T_{n-1}>t_{\text{obs}})=P(T_{n-1}> 1 )= 0.1717 \]

\hypertarget{esercizio-6-28}{%
...
6.a (\textbf{Punti 14}) Stimare il modello di regressione dove \(Y\) viene spiegata da \(X\) e prevedere il reddito per un individuo con 12 anni di studio.
...
\begin{eqnarray*}
r&=&\frac{\text{cov}(X,Y)}{\sigma_X\sigma_Y}=\frac{ 21.09 }{ 3.335 \times 8.106 }= 0.7801 \\r^2&=& 0.6086 < 0.75
\end{eqnarray*}
Il modello \textbf{non} si adatta bene ai dati.

6.c (\textbf{Punti 2}) Stimare lo Standard Error di \(\hat\beta_1\).
...
\widehat{V(\hat\beta_{1})} &=& \frac{S_{\varepsilon}^{2}} {n \hat{\sigma}^{2}_{X}} \\
 ...
 \widehat{SE(\hat\beta_{1})}        &=&  \sqrt{ 0.0482 }\\
 &=&  0.2195 
\end{eqnarray*}

6.d (\textbf{Punti 2}) Definire i punti di leva.

6.e (\textbf{Punti 2}) Se in un modello di regressione sappiamo che \(\hat\sigma_X=0.5\), \(\hat\sigma_Y=1.2\) e \(r=0.8\), quanto varrà \(\hat\beta_1\), il coefficiente angolare del modello di regressione dove \(Y\) viene spiegata da \(X\)?

\hypertarget{prova-di-statistica-20230723-2}{%
\section{Prova di Statistica 2023/07/23-2}\label{prova-di-statistica-20230723-2}}

\hypertarget{esercizio-1-33}{%
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-493-1} \end{center}

1.b (\textbf{Punti 3}) Qual è il numero di famiglie che consuma più di 30 ore di streaming?
...
1.c (\textbf{Punti 2}) la media aritmetica è pari a 14.7934, in base al punto 1a che relazione dobbiamo attenderci tra media, mediana e moda?
...
2.a (\textbf{Punti 14}) Calcola la probabilità che almeno uno dei due eventi sia vero (\(A \cup B\)).
...
2.b (\textbf{Punti 3}) Quando il tempo di manutenzione del macchinario \(X\) è inferiore a 14 ore (\(A=\{X< 14\}\)), la perdita economica dell'azienda è di €100. Mentre quando è superiore a 14 la perdita è di 500€.
...
2.c (\textbf{Punti 2}) Siano \(Z_1\sim N(0,1)\), \(Z_2\sim N(0,1)\), \(Z_3\sim N(0,1)\), tre VC normali standard indipendenti.
Come si distribuisce
\[
Y=Z_1^2+Z_2^2+Z_3^3~~~?
\]

2.d (\textbf{Punti 2}) Se \(A\) e \(B\) sono due eventi tali che \(A\cap B\ne\emptyset\), \(A\) e \(B\) sono indipendenti? (scegliere tra \emph{sempre, mai, dipende} e motivare la risposta)

\hypertarget{esercizio-3-32}{%
\subsection{Esercizio 3}\label{esercizio-3-32}}

(\textbf{Punti 14}) Un'urna contiene le seguenti palline numerate \{3,4,5,6,7,11\}. Si vine se esce un numero pari. Si estrae con reintroduzione per \(n=81\) volte.
...
\hypertarget{esercizio-4-32}{%
\subsection{Esercizio 4}\label{esercizio-4-32}}

4.a (\textbf{Punti 3}) Sia \(\hat \beta_1\) lo stimatore dei minimi quadrati di \(\beta_1\) del modello di regressione lineare.
\begin{eqnarray*}
  \hat\beta_1 &=&  \frac{\text{cov}(X,Y)}{\hat\sigma_X^2}\\
\end{eqnarray*}
Dimostrare la consistenza di \(\hat\beta_1\).

4.b (\textbf{Punti 3}) Come si distribuiscono gli stimatori di massima verosimiglianze asintoticamente?
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-501-1} \end{center}

Il \(p_{\text{value}}\) è \[ P(Z>z_{\text{obs}})=P(Z> 1.79 )= 0.03682 \]

(\textbf{Punti 2}) Calcolare e interpretare il \(p_\text{value}\) del test precedente.

\hypertarget{esercizio-6-29}{%
...
6.a (\textbf{Punti 14}) Si è osservato \(x_3=10\) e \(y_3=31.84\), stimare il modello di regressione dove \(Y\) viene spiegata da \(X\) e calcolare il residuo per il punto \(i=3\).
...
6.c (\textbf{Punti 2}) Stimare lo Standard Error di \(\hat\beta_0\).
...
6.d (\textbf{Punti 2}) Definire il qq-plot.

6.e (\textbf{Punti 2}) Se in un modello di regressione \(r=0.35\), \(\hat\sigma_X=1.2\) e \(\hat\sigma_X=0.5\), quanto varrà
\(\hat\beta_1\), il coefficiente angolare della retta di regressione in cui \(Y\) è spiegata da \(X\)?

\hypertarget{prova-di-statistica-20230723-3}{%
\section{Prova di Statistica 2023/07/23-3}\label{prova-di-statistica-20230723-3}}

\hypertarget{esercizio-1-34}{%
...
1.a (\textbf{Punti 14}) Ricavare il valore approssimato della mediana.
...
\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-508-1} \end{center}
...
1.b (\textbf{Punti 3}) Qual è il numero di individui che spende più di 150€ al mese?
...
1.c (\textbf{Punti 2}) Individuare la classe modale, metterla in relazione con la mediana e indicare la loro relazione con la media.

1.d (\textbf{Punti 2}) Michele ha sostenuto tre esami e ha la media del 26, al quarto esame ha preso 28. Qual è la media calcolata sui 4 esami?
...
2.a \textbf{(Punti 14)} Calcola la probabilità che almeno uno dei due eventi sia vero, ovvero la probabilità di \(A \cup B\).
...
2.b (\textbf{Punti 3}) Qual è la probabilità di avere \textbf{esattamente} uno solo due eventi sia vera?
...
2.c (\textbf{Punti 2}) Siano \(X_1\sim N(5,1)\) e \(X_2\sim N(5,1)\), \(X_1\) e \(X_2\) indipendenti. Come si distribuisce
\[
\bar X = \frac 12 (X_1+X_2) ~~~?
\]

2.d (\textbf{Punti 2}) Siano \(A\) e \(B\) due eventi tali che \(P(A)=0.3\) e \(P(B)=0.3\). \(A\) e \(B\) possono essere incompatibili? (scegliere tra \emph{sempre, mai, dipende} e motivare la risposta)

\hypertarget{esercizio-3-33}{%
\subsection{Esercizio 3}\label{esercizio-3-33}}

(\textbf{Punti 14}) Un'urna contiene le seguenti palline numerate \{3,4,5,6,7,12\}. Si vine se esce un numero pari. Si estrae con reintroduzione per \(n=81\) volte.
...
\hypertarget{esercizio-4-33}{%
\subsection{Esercizio 4}\label{esercizio-4-33}}

4.a (\textbf{Punti 3}) Sia \(\hat \beta_0\) lo stimatore dei minimi quadrati di \(\beta_0\) del modello di regressione lineare.
\begin{eqnarray*}
  \hat\beta_0 &=&\bar y-  \hat\beta_1\bar x\\
\end{eqnarray*}
Dimostrare la consistenza di \(\hat\beta_0\).

4.b (\textbf{Punti 3}) Cosa significa che gli stimatori di massima verosimiglianza sono asintoticamente a massima efficienza?

4.c (\textbf{Punti 3}) Definire gli errori di primo e secondo tipo.
...
\(\fbox{B}\) SCELTA E CALCOLO STATISTICA-TEST, \(Z\)\[\hat\pi_\text{ A }=\frac{s_\text{ A }}{n_\text{ A }}=\frac{ 48 }{ 80 }= 0.6 \qquad
   \hat\pi_\text{ B }=\frac{s_\text{ B }}{n_\text{ B }}=\frac{ 40 }{ 80 }= 0.5 \]Calcoliamo la proporzione comune sotto \(H_0\)
\[
     \pi_C=\frac{s_\text{ A }+s_\text{ B }}{n_\text{ A }+n_\text{ B }}=
     \frac{ 88 }{ 160 }= 0.55 
   \]\begin{eqnarray*}
   ...
   \end{eqnarray*}
\(\fbox{C}\) DECISIONE Dalle tavole si ha \(z_{ 0.01 } = 2.326\).
\[z_{\text{obs}} =  1.271   <  z_{ 0.01 } =  2.326 \]CONCLUSIONE: i dati sono coerenti con \(H_{0}\) al LdS del 1 \%

\textbf{Graficamente}

\begin{center}\includegraphics{Esami_passati_con_soluzioni_files/figure-latex/unnamed-chunk-515-1} \end{center}
...
6.a (\textbf{Punti 14}) Si è osservato \(x_3=7\) e \(y_3=22\), stimare il modello di regressione dove \(Y\) viene spiegata da \(X\) e calcolare il residuo per il punto \(i=3\).
...
6.b (\textbf{Punti 3}) Scrivere la scomposizione della varianza e calcolarla per i dati in esame.
...
\begin{eqnarray*}
   ...
   RSS &=& (1-R^2)\cdot TSS\\
      &=& (1- 0.742 )\cdot 4223 \\
      &=&  1089 \\
   TSS &=& RSS+TSS \\ 4223  &=&  3133 + 1089 
  \end{eqnarray*}

6.c (\textbf{Punti 2}) Stimare \(\sigma^2_\varepsilon\).
...
6.d (\textbf{Punti 2}) Definire i punti influenti.

6.e (\textbf{Punti 2}) Se in un modello di regressione \(r=0.35\), \(\hat\sigma_X=1.2\) e \(\hat\beta_1=0.5\), quanto varrà
