...
`r item()` `r punti_p()` Quante famiglie hanno una spesa superiore al 75-esimo percentile?
...
`r item()` `r punti_p()` Si estrae 5 volte da $X\sim N(102,1.5)$, posto $A=\{X>100\}$, quale è la probabilità che $A$ si avveri 3 volte su 5?
...
`r item()` `r punti_p()` Se $F$ è la funzione di ripartizione della VA $X$, posto $a < b$ due numeri qualunque, a cosa equivale
$$
F(b)-F(a) \qquad?
$$
...
`r item()` `r punti_p()` Definire lo _Standard Error_ di uno stimatore.

`r item()` `r punti_p()` Definire gli errori di primo e di secondo tipo di un test statistico.

`r item()` `r punti_p()` In un test statistico, per quali valori di $p_\text{value}$ si tende a rifiutare $H_0$?
...
`r item()` `r punti_p()` Interpretare i parametri di regressione $\hat\beta_0$ e $\hat\beta_1$.

`r item()` `r punti_p()` Se $W=-10\times Y$, quanto varrà $r_{XW}$, coefficiente di correlazione tra $X$ e $W$?
...
`r item()` `r punti_p()` Quante famiglie hanno una spesa inferiore al 25-esimo percentile?
...
`r item()` `r punti_p()` La spesa media è pari a $\bar x=`r mean(samp)`$, che forma ci dobbiamo aspettare dell'istogramma di densità?

`r item()` `r punti_p()` La varianza della spesa è pari a $Var=`r var(samp)`$.
...
`r item()` `r punti_p()` Si estrae 6 volte da $X\sim \text{Pois}(1.5)$, posto $A=\{X<2\}$, quale è la probabilità che $A$ si avveri 3 volte su 6?
...
`r item()` `r punti_p()` Se $X$ è una VC con valore atteso $E(X)=0.5$ e $V(X)=1.2$, posto $Y=X^2$ è vero che $E(Y)=E^2(X)$?
...
`r item()` `r punti_p()` Siano $h_1$ e $h_2$ due stimatori per $\theta$, cosa significa dire che $h_1$ è più efficiente di $h_2$?

`r item()` `r punti_p()` Definire la significatività e la potenza di un test statistico.

`r item()` `r punti_p()` Se in un test statistico il $p_\text{value}>0.1$ possiamo rifiutare $H_0$?
...
`r item()` `r punti_p()` Cosa sono i _punti di leva_ e cosa gli _outliers_?

`r item()` `r punti_p()` Se $W=10\times Y$, posto
...
`r item()` `r punti_p()` Qual è la percentuale di famiglie con spesa superiore al 25-esimo percentile?
...
`r item()` `r punti_p()` Posto $V=2+5\cdot W$, ricavare valore atteso e varianza di $V$.
...
`r item()` `r punti_p()` Se $A$ e $B$ sono due eventi diversi dal vuoto, è possibile che $P(A)+P(B)>1$?
...
`r item()` `r punti_p()` Siano $h$ uno stimatore per $\theta$, cosa significa dire che $h$ è asintoticamente corretto?

`r item()` `r punti_p()` Definire la probabilità di significatività.

`r item()` `r punti_p()` Se in un test statistico $0.01 < p_\text{value} <0.05$ cosa possiamo concludere?
...
`r item()` `r punti_p()` Calcolare e discutere $R^2$.
...
`r item()` `r punti_p()` Cos'è un _punto influente_?

`r item()` `r punti_p()` Se $W=10+ Y$, posto
$$w_i=\beta_0'+\beta_1'x_ì +\epsilon_i'$$
...
`r item()` `r punti_p()` Quante imprese hanno una spesa compresa tra 15 mila euro e il 75-esimo percentile?
...
`r item()` `r punti_p()`  La spesa media è pari a $\bar x=`r mean(samp)`$ mila euro, 
...
`r item()` `r punti_p()` La spesa media in investimenti green su `r n` aziende modenesi è pari a $\bar x=`r mean(samp)`$ mila euro, mentre la spesa media  in investimenti green della provincia di Reggio, su un campione di 150 aziende è pari a $12.22$ mila euro.
...
`r item()` `r punti_p()` Sia $X\sim N(10,1)$, posto $A=\{X>8\}$. Si estrae ripetutamente da $X$  e si finisce quando $A$ si avvera **2** volte. Calcolare la probabilità di finire dopo 6 estrazioni.
...
`r item()` `r punti_p()` Siano $A$ e $B$ due eventi diversi dal vuoto. È vero che se $A$ e $B$ sono 
...
`r item()` `r punti_p()` Se $F$ è la funzione di ripartizione della VA $X$, 
...
`r item()` `r punti_p()` Se $h$ è uno stimatore per $\theta$ tale che
$E(h)\ne \theta$ e che $\lim_ {n\to +\infty}E(h)=\theta$ di quale proprietà gode $h$?

`r item()` `r punti_p()` Definire gli errori di primo e di secondo tipo di un test statistico e le relative probabilità.

`r item()` `r punti_p()` In un confronto tra due campioni viene messo a test
$$
\begin{cases}
H_0:\sigma_A=\sigma_B\\
H_1:\sigma_A\ne \sigma_B
...
`r item(); i2 <- i2+1 ``r punti_p()` La stessa domanda è stata posta ad un secondo campione di $n_S=38$ consumatori privati,scelti a caso tra i cittadini del comune di Sassuolo. 
...
`r item()` `r punti_p()` Scrivere la scomposizione della varianza del modello di regressione 
...
`r item()` `r punti_p()` Il parametro di regressione $\hat\beta_0$, in questo caso, è interpretabile?

`r item()` `r punti_p()` Una previsione per $x=40$ è attendibile? Perché?

`r item()` `r punti_p()` Se $W=5+ Y$, posto $w_i=\beta_0'+\beta_1'x_ì +\epsilon_i'$
il modello in cui $W$ viene spiegata da $X$, quanto varranno $\beta_0'$ e $\beta_1'$?
...
`r item()` `r punti_p()` Quante aziende hanno una spesa compresa tra il 25-esimo percentile e 15 mila euro?
...
`r item()` `r punti_p()` Che relazione dobbiamo attenderci tra media, mediana e moda?
...
`r item()` `r punti_p()` Si osserva la rotatoria per $n=6$ minuti. Qual è
...
`r item()` `r punti_p()` Se $X\sim N(0,2)$ e $X\sim N(1,1.2)$, è vero che
$$
X-Y\sim N(-1,0.8) \qquad ?
$$


`r item()` `r punti_p()` Se $X$ è una VC con supporto {0,1,2} e $Y$ è una VC con supporto {2,3,4,5}.
...
`r item()` `r punti_p()` Siano $T_1$ e $T_2$ due test statistici per la stessa $H_0$ e con la stessa significatività $\alpha$. Cosa significa dire che $T_1$ e più potente di $T_2$?

`r item()` `r punti_p()` Definire la probabilità di significatività osservata.

`r item()` `r punti_p()` Se in un test statistico che utilizza la statistica test _t_ con 10 gradi di libertà $t_\text{obs}=1.4$, il $p_\text{value}$ sarà maggiore o minore di 0.05? Perché?
...
`r item()` `r punti_p()` Un'indagine molto più ampia condotta su tutta la città ha mostrato che
...
`r item()` `r punti_p()` Cosa sono i _punti influenti_?

`r item()` `r punti_p()` Se $W=2\times Y$ e $V=X+3$, posto $w_i=\beta_0'+\beta_1'v_ì +\epsilon_i'$
...
`r item()` `r punti_p()` Qual è la percentuale di aziende che hanno una spesa 
...
`r item()` `r punti_p()` Si ripete il gioco di sopra finché non si vince due volte. Qual è la probabilità di finire alla quarta giocata?
...
`r item()` `r punti_p()` Se $X\sim \text{Pois}(2)$ e $Y\sim\text{Pois}(1)$, è vero che
$$
X-Y\sim\text{Pois}(1)\qquad ?
$$


`r item()` `r punti_p()` Se $X$ è una VC con supporto {0,1,2} e $X$ è una VC con supporto {-2,-1,0}.
...
`r item()` `r punti_p()` Se uno stimatore $h$ per $\theta$ è tale che $\lim_{n\to\infty}V(h)=0$, di quali proprietà gode $h$?

`r item()` `r punti_p()` Un intervallo di confidenza per $\theta$ al 95% è più ampio o meno ampio di uno al 99%? Perché?

`r item()` `r punti_p()` Se in un test statistico che utilizza la statistica test _t_ con 10 gradi di libertà $t_\text{obs}=14$, il $p_\text{value}$ sarà maggiore o minore di 0.05? Perché?
...
`r item()` `r punti_p()` Ricavare numericamente la scomposizione della varianza del modello di regressione sopra stimato.
...
`r item()` `r punti_p()` Che differenza c'è tra _interpolazione_ e _estrapolazione_?

`r item()` `r punti_p()` Cosa significa che $r$ è un numero puro?
...
`r item()` `r punti_p()` Quante imprese hanno un bilancio compreso tra $-4$ mila 
euro e zero.
...
`r item()` `r punti_p()`  La media è risultata essere $\bar x=`r mean(samp)`$; che
...
`r item()` `r punti_p()` Siano $x_1,...,x_n$, $n$ numeri, $n$ dispari. 
...
`r item()` `r punti_p()` Posto $Z=X-Y$, Calcolare la probabilità che $P(Z>1|Z\le 2)$. 
...
`r item()` `r punti_p()` Siano $A$ e $B$ due eventi diversi dal vuoto.
...
`r item()` `r punti_p()` Sia $X\sim\text{Pois}(\lambda=1)$, disegnare la
...
`r item()` `r punti_p()` Definire gli errori di primo e di secondo 
...
`r item()` `r punti_p()` Interpretare il parametro di regressione $\hat\beta_1$.

`r item()` `r punti_p()` Descrivere la differenza tra punti di leva e punti influenti.

`r item()` `r punti_p()` Gli stimatori $\hat\beta_0$ e $\hat\beta_1$ dei minimi 
...
`r item()` `r punti_p()` Quante aziende hanno un bilancio compreso tra il 25-esimo 
...
`r item()` `r punti_p()` Siano $x_1,...,x_n$, $n$ numeri. Si consideri la funzione:
$$g(x)=(x_1-x)^2+...+(x_n-x)^2.$$
Per quale valore di $x$, $g(x)$ è minima?
...
`r item()` `r punti_p()` Siano $X\sim N(0,1)$ e $Y\sim N(0,1)$, $X$ e $Y$ indipendenti.
Come si distribuisce $X^2+Y^2$?


`r item()` `r punti_p()` Se $X$ è una VC con supporto {2,3,4,5} e $Y$ è una VC con supporto 
{1,2,6}.
Qual è il supporto di $X-Y$?
...
`r item()` `r punti_p()` Sia $\hat\sigma^2$ lo stimatore di massima verosimiglianza
di $\sigma^2$. In virtù di quale proprietà $\hat \sigma$ è lo stimatore di
massima verosimiglianza per $\sigma$?

`r item()` `r punti_p()` Siano $T_1$ e $T_2$ due test per lo stesso sistema di ipotesi,
...
`r item()` `r punti_p()` Se in un test statistico bilaterale che utilizza 
...
`r item(); i2 <- i2+1 ``r punti_p()` La stessa domanda è stata posta ad un secondo campione di $n_F=38$ consumatori privati,scelti a caso tra i cittadini del comune di Ferrara. 
...
`r item()` `r punti_p()` Calcolare numericamente $RSS$:
$$
...
`r item()` `r punti_p()` Gli stimatori $\hat\beta_0$ e $\hat\beta_1$ sono consistenti? 
Perché?

`r item()` `r punti_p()` Se in un modello di regressione con 11 dati, 
il residuo studentizzato del dato $i$ è $\tilde \epsilon_i=1.23$, cosa possiamo concludere?

`r item()` `r punti_p()` Sia $\hat\beta_1$ lo stimatore dei minimi quadrati per $\beta_1$.
...
`r item()` `r punti_p()` Quante aziende hanno un bilancio compreso tra $-1$ e il 75-esimo 
percentile?
...
`r item()` `r punti_p()` Sapendo che $\sigma_X=`r sc (samp)`$ la Standard Deviation di $X$
...
`r item()` `r punti_p()` Si ripete il gioco di sopra finché non si vince tre volte. Qual è la probabilità di finire alla quinta giocata?
...
`r item()` `r punti_p()` Se $X$ è una VC con supporto {1,2,3} e $Y$ è una VC con 
supporto {1,3,5}.
Qual è il supporto di $X\times Y$?
...
`r item()` `r punti_p()` Sia $\hat\theta$ lo stimatore di massima verosimiglianza 
per $\theta$. Qual è la distribuzione asintotica di $\hat\theta$?

`r item()` `r punti_p()` Un intervallo di confidenza per $\theta$ al 90% è più ampio o meno ampio di uno al 95%? Perché?

`r item()` `r punti_p()` Se in un test statistico bilaterale che utilizza la statistica test _z_ , risulta $z_\text{obs}=14$, il $p_\text{value}$ sarà maggiore o minore di 0.01? Perché?
...
`r item()` `r punti_p()` La stessa domanda è stata posta ad un secondo 
...
`r item()` `r punti_p()` Calcolare la percentuale di varianza di $Y$ spiegata dal modello.
...
`r item()` `r punti_p()` Se in un modello di regressione con 15 dati, 
il residuo studentizzato del dato $i$ è $\tilde \epsilon_i=12.3$, cosa possiamo concludere?


`r item()` `r punti_p()` Sia $\hat\beta_0$ lo stimatore dei minimi quadrati per $\beta_0$.
