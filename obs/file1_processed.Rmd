...
`r item()` `r punti_p()` Qual è il numero di imprese con utile negativo?
...
`r item()` `r punti_p()` `r punti_p()` L'utile medio è pari a $\bar x=`r mean(samp)`$, e la sua standard deviation
...
`r item()` `r punti_p()` Qual è la probabilità di avere esattamente _due_ casse su _quattro_ senza fila?
...
`r item()` `r punti_p()` Quando due eventi $A$ e $B$ si dicono _indipendenti_ e quando _incompatibili_?
...
`r item()` `r punti_p()` Se $X_1\sim N(2,1)$, $X_2\sim N(1,1)$ e $X_3\sim N(1,1)$, 
...
`r item()` `r punti_p()` Siano $h_1$ e $h_2$ due stimatori per $\theta$, tali che:
\begin{eqnarray*}
MSE(h_1) &=&   \frac\theta n\\
...
`r item()` `r punti_p()` Si sono osservati due gruppi di dati quantitativi e si è osservato, $\hat\mu_1=10.2$ e $\hat\mu_2=15.6$. Posto a test
$$
\begin{cases}
H_0:\mu_1=\mu_2\\
H_1:\mu_1\ne \mu_2
...
:::
  ...
  `r item()` `r punti_p()` Calcolare la percentuale di varianza spiegata dal modello.
...
:::
  ...
  `r item()` `r punti_p()` Discutere il qq-plot dei residui
```{r,fig.asp=1}
...
:::
 ...
 `r item()` `r punti_p()` Cosa vuol dire che $r$ è un numero puro?
...
`r item()` `r punti_p()` Analizzando l'istogramma, individuare il valore della media aritmetica e della mediana.
...
`r item()` `r punti_p()` Qual è la percentuale di dati compresa tra il 25-esimo e il 75-esimo percentile?
...
`r item()` `r punti_p()` Nell'ipotesi di indipendenza tra i giorni, calcolare la probabilità di avere esattamente _due_ giorni su _cinque_ con livello negativo.  `r pp <- pnorm(0,mm,sqrt(ss))`
...
`r item()` `r punti_p()` Se $P(A)=0.6$ e $P(B)=0.8$, $A$ e $B$ possono essere incompatibili?
...
`r item()` `r punti_p()` Se $X\sim N(\mu_X,\sigma^2_X)$ come si distribuisce 
...
`r item()` `r punti_p()` Siano $X_1,...,X_n$ $n$ VC IID, replicazioni di $X\sim \mathscr{L}(\theta)$ e sia $\hat\theta$ lo stimatore di massima verosimiglianza per $\theta$, 
$\hat\theta$ è corretto?
...
:::
  ...
  `r item()` `r punti_p()` Discutere il diagramma dei residui
```{r}
...
`r item()` `r punti_p()` Quando in un modello di regressione lineare un punto è considerato **influente**?
...
`r item()` `r punti_p()` Calcolare la percentuale approssimata di aziende 
...
`r item()` `r punti_p()` Calcolare la probabilità che la somma sia maggiore uguale a 1 dato che dall'urna $A$ è uscito $\fbox{0}$.
...
`r item()` `r punti_p()` Sia $X\sim \text{Pois}(10)$ e $Y\sim\text{Binom}(n,0.5)$.
Sia
$$
W=X-Y
$$
...
`r item()` `r punti_p()` Siano $X_1,...,X_6$, $6$ variabili casuali IID,
tali che
$$
...
`r item()` `r punti_p()` Definire la funzione di verosimiglianza.
...
`r item()` `r punti_p()` Si sono osservati due gruppi di dati quantitativi e si è osservato, $\hat\mu_1=10.2$, $\hat\sigma_1=1.12$ e $\hat\mu_2=15.6$, $\hat\sigma_2=3.72$. Posto a test
$$
\begin{cases}
H_0:\sigma_1=\sigma_2\\
H_1:\sigma_1\ne \sigma_2
...
:::
  ...
  `r item()` `r punti_p()` Quale delle due previsioni, per $x=`r mean(x)`$ e per $x=12.3$,
...
:::
  ...
  `r item()` `r punti_p()` Calcolare le quantità $TSS$, $RSS$ e $ESS$.
...
:::
 ...
 `r item()` `r punti_p()` Cosa vuol dire che $r$ è invariante ai cambiamenti di scala?
...
`r item()` `r punti_p()` La somma dei dati è `r sum(samp)`. Come dobbiamo aspettarci l'istogramma di densità?
...
`r item()` `r punti_p()` Calcolare la probabilità di vincere la prima volta alla quarta estrazione.
...
`r item()` `r punti_p()` Se $A$ e $B$ sono due eventi tali che, $P(A)>0$, $P(B)>0$, $P(A\cap B)=0$. 
...
`r item()` `r punti_p()` Sia $X\sim N(3.2,(1.1)^2)$ e sia $Y\sim\chi_{n-1}^2$, posto
$$
...
`r item()` `r punti_p()` Scrivere la funzione di verosimiglianza di una Poisson.
...
`r item()` `r punti_p()` Definire il $p_{\text{value}}$, la probabilità di significatività osservata.
...
`r item()` `r punti_p()` Ricavare le quantità $TSS$, $RSS$ e $ESS$.
...
`r item()` `r punti_p()` Cosa significa che gli stimatori di massima dei minimi quadrati $\hat\beta_0$ e $\hat\beta_1$ sono _BLUE_?
...
`r item()` `r punti_p()` La varianza dei dati è pari a $\sigma^2=`r s2c(samp)`$. Nell'ipotesi che i costi per metano aumentassero del 10%, quanto varrebbe la varianza?
...
`r item()` `r punti_p()` Calcolare la probabilità di osservare una persona su un binario, una persona su un altro binario e nessuna persona sul rimanetene.
...
`r item()` `r punti_p()` Siano $X_1,...,X_n$, $n$ VC IID, tali che
$$
X_i\sim N(0,1).
$$
Posto
...
`r item()` `r punti_p()` Siano $X_1\sim \text{Binom}(5,0.3)$ e $X_2\sim \text{Binom}(3,0.3)$.
Come si distribuisce,
$$X_1+X_2\sim ~~?$$
...
`r item()` `r punti_p()` Definire la funzione di verosimiglianza della Bernoulli.
...
`r item()` `r punti_p()` Un dado, che non sappiamo se è perfetto oppure no, viene lanciato 40 volte.
...
`r item()` `r punti_p()` È ben noto, da studi pregressi, che quella tipologia di farmaci ha un tempo medio 
...
:::
  ...
  `r item()` `r punti_p()` I due modelli si adattano bene ai dati?
...
:::
  ...
  `r item()` `r punti_p()` Discutere il diagramma dei residui del modello di regressione dove $Y$ viene spiegata da $X$.
```{r}
...
:::
 ...
 `r item()` `r punti_p()` Se ogni individuo risparmiasse 10€ in più al mese, quanto varrebbe $r$?
...
`r item()`  `r punti_p()` Calcolare il numero approssimato di aziende con consumo di gas inferiore al 25-esimo percentile $x_{0.25}$. 
...
`r item()` `r punti_p()` È vero che la media aritmetica minimizza la somma dei valori assoluti degli scarti? Perché?
...
`r item()` `r punti_p()` Per giocare si devono puntare due euro ad ogni giocata. Se vinciamo ci vengono dati 9€, i 2€ giocati più 7€ di vincita. Quindi:
...
`r item()` `r punti_p()` Se $A$ e $B$ sono due eventi tali che, $P(A)=0.5$, $P(B)=0.4$ e $P(A\cup B)=0.58$. $A$ e $B$ sono indipendenti? Perché?
...
`r item()` `r punti_p()` Siano $Y_1,...,Y_n$, $n$ VA IID, tali che, $Y_i\sim\chi_{1}^2$, posto
$$
X=\sum_{i=1}^n Y_i
$$
...
`r item()` `r punti_p()` Descrivere la tavola della verità di un test.

`r item()` `r punti_p()` In uno studio clinico si è osservato l'effeto sul numero di anticorpi su due gruppi trattati con un farmaco differente. Si è osservato $\hat\mu_1=10.2$, $\hat\sigma_1=1.12$ e $\hat\mu_2=15.6$, $\hat\sigma_2=3.72$. Posto a test
$$
\begin{cases}
H_0:\sigma_1=\sigma_2\\
H_1:\sigma_1\ne \sigma_2
...
`r item()` `r punti_p()` È noto che quella fascia di motori ha un consuo medio pari $23.9~\text{Km/litro}$. Testare che il nuovo motore consumi come gli altri contro l'alternativa che abbia consumi _diversi_.
...
`r item()` `r punti_p()` Quale dei due modelli è più affidabile?
...
`r item()` `r punti_p()` Considerata la scomposizine della TSS di $X$ rispetto ad $Y$.
$$
TSS = ESS + RSS
$$
...
`r item()` `r punti_p()` Sotto ipotesi di normalità dei residui, come sono distribuiti gli stimatori dei mini quadrati $\hat\beta_0$ e $\hat\beta_1$?
...
`r item()` `r punti_p()` Quante aziende, approssimativamente, hanno un bilancio inferiore a 1.5?
...
`r item()` `r punti_p()` La varianza dei dati è pari a $\sigma^2=`r s2c(samp)`$. Nell'ipotesi che tutte le aziende aumentassero il loro bilancio di 10 mila ero, quanto varrebbe la varianza dei così trasformati?
...
`r item()` `r punti_p()` Posto $Y=X_1-X_2$, calcolare $P(Y<-1)$.
...
`r item()` `r punti_p()` Siano $X_1,...,X_n$, $n$ VC IID, tali che $X_i\sim \text{Ber}(\pi)$, posto
\[
 X = X_1+...+X_n
\]
come si distribuisce $X$? 

`r item()` `r punti_p()` Sia $X\sim \text{Pois}(1.5)$, per quali valori $x$, $F(x)$, la funzione di ripartizione di $X$ è minore o uguale a 0.5? Ovvero, per quali $x$ la seguente è rispettata $F(x)\le 0.5$?
...
`r item()` `r punti_p()` Il numero di incidenti giornalieri sul lavoro del comparto $A$ è distribuito come una
...
`r item()` `r punti_p()` Definire la funzione di verosimiglianza della Poisson. 

`r item()` `r punti_p()` Una moneta, che non sappiamo se è perfetta oppure no, viene lanciata 40 volte.
...
`r item()` `r punti_p()` In caso di campionamento casuale semplice **senza reintroduzione**
...
`r item()` `r punti_p()` Una indagine più vasta dell'anno precedente
...
`r item()` `r punti_p()` Se in un modello di regressione si conoscono $\hat\beta_1$, $r^2$ e $\hat\sigma_X$ è possibile ricavare $\hat\sigma_Y$? In che modo?
...
`r item()` `r punti_p()` Cosa significa che il coefficiente di correlazione è invariante alle trasformazioni lineari?
