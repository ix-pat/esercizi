...
`r item()` `r punti_p()` Che percentuale di famiglie investe più di 4.5 mila euro all'anno?
...
`r item()` `r punti_p()` Che relazione dobbiamo aspettarci tra media e mediana? 

`r item()` `r punti_p()` La spesa media è pari a $\bar x=`r mean(samp)`$, mentre la SD è pari a $SD=`r sd(samp)`$.
...
`r item()` `r punti_p()` Un'urna contiene due palline rosse, due bianche e una nera. Si estrae due volte senza reinserimento. Qual è la probabilità di avere due colori diversi?  
...
`r item()` `r punti_p()` Se $X\sim\text{Pois}(3.2)$ e $Y\sim\text{Pois}(1.2)$, $X$ e $Y$ indipendenti, quali sono valore atteso e varianza di $X+Y$ e di $X-Y$? 

`r item()` `r punti_p()` Sia $X$ una VC e sia $F$ la sua funzione di ripartizione. Cosa significa dire che $F$ è continua a destra?
...
`r item()` `r punti_p()` Si $h$ uno stimatore per $\theta$ tale che $E(h)=\theta$. Di quale proprietà gode $h$? 

`r item()` `r punti_p()` Definire la potenza di un test. 

`r item()` `r punti_p()` In uno studio sull'efficacia degli integratori alimentari, su un gruppo di 238 atleti è stato misurato il rendimento atletico (ottimo, buono e scarso) e l'assunzione di integratori (alto, medio e basso). Qui di seguito i dati:
...
`r item()` `r punti_p()` Qual è la percentuale di varianza spiegata dal modello? 

:::{.sol data-latex=""}
$$r^2\times 100=`r r^2*100`$$
:::

`r item()` `r punti_p()` Interpretare il diagramma dei residui. 
...
`r item()` `r punti_p()` Se $W=- Y$, quanto varrà $r_{XW}$, coefficiente di correlazione tra $X$ e $W$?
...
`r item()` `r punti_p()` Quante famiglie investono più di 8 mila euro all'anno? 
...
`r item()` `r punti_p()` Un'urna contiene 5 palline: 2 rosse e 3 bianche. Si estrae con reintroduzione finché non escono 2 bianche consecutive. Calcolare la probabilità di finire in __esattamente__ 3 estrazioni. 
...
`r item()` `r punti_p()` Se $X\sim N(3.2,1.5)$ e $Y\sim N(1.2,1.1)$, $X$ e $Y$ indipendenti, come si distribuiscono $X+Y$ e  $X-Y$? 

`r item()` `r punti_p()` Sia $X$ una VC e sia $F$ la sua funzione di ripartizione. Cosa significa dire che $F$ è crescente?
...
`r item()` `r punti_p()` Il modello si adatta bene ai dati? 
...
`r item()` `r punti_p()` Se $W= 1 - Y$, quanto varrà $r_{XW}$, coefficiente di correlazione tra $X$ e $W$?
...
`r item()` `r punti_p()` Quante famiglie investono meno di 4.5 mila euro all'anno? 
...
`r item()` `r punti_p()` Dalla stessa urna di prima, si estrae con reintroduzione finché non escono due palline bianche consecutive. Qual è la probabilità di finire in __al massimo__ 3 tentativi? 
...
`r item()` `r punti_p()` Se $X\sim N(3.2,1.1)$ e $Y\sim\text{Binom}(12,0.3)$, $X$ e $Y$ indipendenti, quali sono valore atteso e varianza di $X+Y$ e di $X-Y$? 

`r item()` `r punti_p()` Sia $X$ una VC e sia $F$ la sua funzione di ripartizione. Quanto valgono $\lim_{x\to-\infty}F(x)$ e $\lim_{x\to+\infty}F(x)$?
...
`r item()` `r punti_p()` Si $h$ uno stimatore per $\theta$ tale che $E(h)\ne\theta$, ma che $\lim_{n\to\infty}E(h)=\theta$. Di quale proprietà gode $h$? 

`r item()` `r punti_p()` Quali sono gli errori di primo e secondo tipo? 
...
`r item()` `r punti_p()` Un'indagine analoga, svolta sull'intera regione, ha mostrato un gradimento medio pari a $\mu_0=`r mu0`$. Testare l'ipotesi che nel comune A il livello di gradimento sia uguale a quello regionale contro l'alternativa che sia __minore__.
...
`r item()` `r punti_p()` Calcolare e interpretare $R^2$. 
...
`r item()` `r punti_p()` Quale è il numero di imprese che hanno un utile maggiore di zero?
...
`r item()` `r punti_p()` L'utile medio è pari a $\bar x=`r mean(samp)`$ che forma avrà l'istogramma?

`r item()` `r punti_p()` Qual è la proprietà di associatività della media aritmetica?
...
`r item()` `r punti_p()` Un'urna contiene 10 palline rosse, 10 bianche e 10 nere. Si estrae tre volte senza reinserimento. Qual è la probabilità di avere tre colori uguali? 
...
`r item()` `r punti_p()` Se $X\sim\text{Ber}(0.2)$ e $Y\sim\text{Ber}(0.2)$, $X$ e $Y$ indipendenti, com'è distribuita $X+Y$?

`r item()` `r punti_p()` Se $X$ è $Y$ sono due VC, tali che $V(X)=\sigma_X^2$ e $Y=\sigma_Y^2$, quanto vale $V(X-Y)$?
...
`r item()` `r punti_p()` Sia $h$ uno stimatore per $\theta$. Cosa significa dire che $h$ è consistente?

`r item()` `r punti_p()` Siano $T_1$ e $T_2$ due test per lo stesso sistema d'ipotesi e con probabilità di errore di secondo tipo pari a, $\beta_1=0.2$ e $\beta_2=0.3$, rispettivamente. A parità di significatività, quale dei due è più potente? Perché?
...
`r item()` `r punti_p()` Qual è la proprietà di linearità della media aritmetica?
...
`r item()` `r punti_p()` Se $X\sim N(\mu_X,\sigma_X^2)$ e $Y\sim N(\mu_Y,\sigma_Y^2)$, $X$ e $Y$ indipendenti, come si distribuiscono $X+Y$ e  $X-Y$?

`r item()` `r punti_p()` Siano $A$ e $B$ due eventi diversi dal vuoto tali che $A\cap B=\emptyset$, motivare perché $A$ e $B$ non possono essere indipendenti.
...
`r item()` `r punti_p()` Definire la significatività e la potenza di un test.
...
`r item()` `r punti_p()` Se $V= 1 - Y$ e $W= 1 - X$ quanto varrà $r_{VW}$, coefficiente di correlazione tra $V$ e $W$?
...
`r item()` `r punti_p()` Quante imprese hanno un utile negativo?
...
`r item()` `r punti_p()` La varianza dei dati è pari a $\hat\sigma^2=`r s2c(samp)`$, se gli utili di ogni impresa aumentassero del 3%, quanto varrebbe la varianza dei dati così trasformati?
...
`r item()` `r punti_p()` Sia $Z\sim N(0,1)$ e siano $A=\{Z<0\}$ e $B=\{Z<1\}$. Calcolare $P(A|B)$
...
`r item()` `r punti_p()` Se $X\sim\text{Pois} (n)$, a cosa tende $X$ se $n$ diverge?
...
`r item()` `r punti_p()` Sia $X\sim N(10,1)$  e sia $F$ la sua funzione di ripartizione. Disegnare approssimativamente $F(x)$ per $x$ che varia tra 6 e 14.
...
`r item()` `r punti_p()` Se $h_1$ e $h_2$ sono due stimatori per $\theta$ cosa significa dire che $h_1$ è più efficiente di $h_2$?

`r item()` `r punti_p()` Definire gli errori di primo e secondo tipo e le relativa probabilità.

`r item()` `r punti_p()`  Una moneta, che non sappiamo se è perfetta oppure no, viene lanciata 40 volte. Abbiamo osservato 14 volte testa su 40 lanci. Posto $\pi$ la probabilità che la moneta mostri testa, si è testato
\[
\begin{cases}
H_0:\pi=\frac 12\\
H_1:\pi\ne\frac 12
...
`r item()` `r punti_p()` Un'indagine analoga, svolta sull'intera regione, ha mostrato un reddito medio pari a $\mu_0=`r mu0`$. Testare l'ipotesi che nel comune $A$ il reddito medio sia uguale a quello regionale contro l'alternativa che sia __diverso__.
...
`r item()` `r punti_p()` Interpretare il qq-plot dei residui.
```{r,fig.height=2.5,fig.width=3.5}
par(cex=.35)
...
`r item()` `r punti_p()` Se $V=1+ Y$ e $W=1-X$, quanto varrà $r_{VW}$, il coefficiente di correlazione tra $V$ e $W$?
...
`r item()` `r punti_p()` Qual è la percentuale di famiglie con spesa superiore a 4 mila euro?
...
`r item()` `r punti_p()` Che relazione dobbiamo attenderci tra moda, mediana e media?

`r item()` `r punti_p()` Cosa significa che la media aritmetica rende zero la somma degli scarti?
...
`r item()` `r punti_p()` Calcolare la probabilità che **solo** uno dei due eventi $A$ oppure $B$ sia vero.

`r item()` `r punti_p()` Calcolare valore atteso e varianza di $X_A-X_B$.

`r item()` `r punti_p()` Sia $X\sim\text{Ber}(\pi=0.4)$. Disegnare la sua funzione di ripartizione.
...
`r item()` `r punti_p()` Se $h_1$ e $h_2$ sono due stimatori per $\theta$ tale per cui $MSE(h_1)=1.2$, e $MSE(h_2)=2.3$, quale dei due è più efficiente e perché?

`r item()` `r punti_p()` Definire gli errori di primo e secondo tipo e le relative probabilità.

`r item()` `r punti_p()`  Una moneta, che non sappiamo se è perfetta oppure no, viene lanciata 80 volte. Abbiamo osservato 28 volte testa su 80 lanci. Posto $\pi$ la probabilità che la moneta mostri testa, si è testato
\[
\begin{cases}
H_0:\pi=\frac 12\\
H_1:\pi\ne\frac 12
...
`r item()` `r punti_p()` Testare l'ipotesi che l'intercetta sia uguale a zero contro l'alternativa che sia diversa da zero.
...
`r item()` `r punti_p()` Perché una previsione per $x=15$ è più affidabile di una per $x=50$?
...
`r item()` `r punti_p()` Qual è la percentuale di famiglie con spesa compresa tra a 4 mila e 8 mila euro?
...
`r item()` `r punti_p()` Cosa significa che la media aritmetica gode della proprietà di linearità?
...
`r item()` `r punti_p()` Si continua ad estrarre da $X_A\sim N(1.5,1.5)$ e si interrompe quando $X_A\le 1$. Calcolare la probabilità di interrompere alla quinta estrazione.
...
`r item()` `r punti_p()` Come si distribuisce $X_A-X_B$?

`r item()` `r punti_p()` Se $A$ e $B$ sono due eventi tali che $A\cap B=\emptyset$, $A$ e $B$ possono essere indipendenti? Perché?
...
`r item()` `r punti_p()` Siano $T_1$ e $T_2$ due test per lo stesso sistema d'ipotesi e con probabilità di errore di secondo tipo pari a, $\beta_1=0.02$ e $\beta_2=0.3$, rispettivamente. A parità di significatività, quale dei due è più potente? Perché?
...
`r item()` `r punti_p()` Posto $W=-2\cdot Y$ calcolare $\beta_0'$ e $\beta_1'$ i coefficienti di regressione del modello
\[
...
`r item()` `r punti_p()` Qual è il numero di famiglie con spesa compresa tra a 4 mila e 8 mila euro?
...
`r item()` `r punti_p()` Cosa significa che la media aritmetica goe della proprietà di linearità?
...
`r item()` `r punti_p()` Si estrae ripetutamente da $X\sim\text{Binom}(5,0.5)$ e ci si ferma quando $X\le 1$. Calcolare la probabilità di finire entro due estrazioni.
...
`r item()` `r punti_p()` Calcolare valore atteso e varianza di $X-Y$.

`r item()` `r punti_p()` Siano $A$ e $B$ due eventi tali che $P(A)=0.6$ e $P(B)=0.5$. $A$ e $B$ possono essere incompatibili? Perché?
...
`r item()` `r punti_p()` Sia $h$ uno stimatore per $\theta$. Cosa significa dire che $h$ è corretto asintoticamente?

`r item()` `r punti_p()` Definire gli errori di primo e di secondo tipo e le relative probabilità.

`r item()` `r punti_p()` Un dado viene lanciato 60 volte e si ottiene 
```{r}
set.seed(1)
...
`r item()` `r punti_p()` Un'indagine analoga, svolta sull'intera regione, ha mostrato un gradimento medio pari a $\mu_0=`r mu0`$. Testare l'ipotesi che nel comune A il livello di gradimento sia uguale a quello regionale contro l'alternativa che sia minore.
...
`r item()` `r punti_p()` Posto $W=-2\cdot X$ calcolare $\beta_0'$ e $\beta_1'$ i coefficienti di regressione del modello
\[
...
`r item()` `r punti_p()` Qual è la percentuale di famiglie con spesa superiore a 60 euro?
...
`r item()` `r punti_p()` La spesa media è pari a $\bar x=`r mean(samp)`$ e la standard deviation $\hat\sigma=`r sd(samp)`$, se tutte le famiglie risparmiassero
...
`r item()` `r punti_p()` Sapendo che $S=X+Y\ge 3$, calcolare la probabilità che $S=X+Y= 5$ ($P(S=5|S\ge 3$)
...
`r item()` `r punti_p()` Se $X$ ed $Y$ sono due variabili casuali, è sempre vero che
\[
V(X-Y)=V(X)+V(Y)~~~?
\]
motivare la risposta.

`r item()` `r punti_p()` Sia $X\sim\text{Binom}(2,\pi=0.5)$. Disegnare la sua funzione di ripartizione.
...
`r item()` `r punti_p()` Sia $h$ uno stimatore per $\theta$ tale che 
\[V(h)=\frac\theta {\sqrt{n}};~~~~E(h)=\theta\frac{n+2}{n}\]
di quali proprietà gode $h$?

`r item()` `r punti_p()` Definire il p-value e descrivere la sua interpretazione.

`r item()` `r punti_p()`  Se un test è significativo al 5% può essere significativo all'1%? (scegliere la risposta tra: _mai_, _non sempre_ oppure _sempre_ e motivare la risposta)
...
`r item()` `r punti_p()` Nel comune $B$ si è condotta un'intervista analoga. 
...
`r item()` `r punti_p()` Scrivere la scomposizione della varianza e 
...
`r item()` `r punti_p()` Perché una previsione per $x=0.15$ è più affidabile di una per $x=50$?
...
`r item()` `r punti_p()` Cosa significa dire che $r$ è un numero puro?
...
`r item()` `r punti_p()` Qual è il numero di imprese con spesa superiore ai 100 euro?
...
`r item()` `r punti_p()` La spesa media è pari a $\bar x=`r mean(samp)`$ e varianza $\hat\sigma^2=`r s2c(samp)`$, se tutte le famiglie aumentassero 
...
`r item()` `r punti_p()` Si lancia una moneta, se esce testa si estrae da una normale $X\sim N(0,1)$, se esce croce si estrae
...
`r item()` `r punti_p()` Se $X\sim\text{Pois}(\lambda_X)$ e $Y\sim\text{Pois}(\lambda_Y)$, quali sono valore atteso e varianza 
di $X-Y$?

`r item()` `r punti_p()` Se $A$ e $B$ sono due eventi *non* indipendenti come si può scrivere $P(A\cap B)$?
...
`r item()` `r punti_p()` Definire la significatività di un test statistico.

`r item()` `r punti_p()` Se un test è significativo all'1% può essere significativo all'5%? (scegliere la risposta tra: _mai_, _non sempre_ oppure _sempre_ e motivare la risposta)
...
`r item()` `r punti_p()` Perché una previsione per $x=0.8$ è più affidabile di una per $x=0$?
...
`r item()` `r punti_p()` Cosa significa dire che $r$ è invariante ai cambiamenti di scala?
...
`r item()` `r punti_p()` Qual è il numero di imprese con spesa superiore ai 60 euro?
...
`r item()` `r punti_p()` Scrivere la proprietà di associatività della media aritmetica.
...
`r item()` `r punti_p()` Sia $X\sim N(2,1)$ e sia $A=\{X>2.5\}$.
...
`r item()` `r punti_p()` `r item()` `r punti_p()` Sia $h$ uno stimatore per $\theta$ tale che 
\[V(h)=2\frac\theta {\sqrt{n}};~~~~E(h)=\theta\frac{n+2}{n-1}\]
scrive il Mean Squared Error di $h$ ($MSE(h)$).

`r item()` `r punti_p()` Definire la potenza di un test statistico.

`r item()` `r punti_p()` Se un t-test bilaterale con 13 gradi libertà presenta una
...
`r item()` `r punti_p()` Nella regione è stata condotta un'indagine analoga
...
`r item()` `r punti_p()` Che differenza c'è tra interpolazione ed estrapolazione?

`r item()` `r punti_p()` Definire il diagramma dei residui.

`r item()` `r punti_p()` Cosa significa che $\hat\beta_0$ e $\hat\beta_1$ sono BLUE?
...
`r item()` `r punti_p()` Qual è la percentuale di imprese con bilancio inferiore a 3 (centomila) euro?
...
`r item()` `r punti_p()` la media aritmetica è pari a `r mean(samp)`, in base al punto 1a che relazione dobbiamo attenderci tra media, mediana e moda?

`r item()` `r punti_p()` Si considerino i seguenti dati
...
`r item()` `r punti_p()` Per andare al lavoro Michele prende la strada che 
...
`r item()` `r punti_p()` Sia $Z\sim N(0,1)$ e $Y\sim \chi^2_2$, $Z$ e $Y$ indipendenti.
Come si distribuisce
\[
\frac{Z}{\sqrt{Y/2}} ~~~?
\]

`r item()` `r punti_p()` Se $P(A)=0.3$, $P(B|A)=0.6$, e $P(B|\bar A)=0.4$, quanto valgono $P(B)$ e $P(A|B)$?
...
`r item()` `r punti_p()` Cosa significa che gli stimatori di massima verosimiglianza sono invarianti alle trasformazioni monotone invertibili?
...
`r item()` `r punti_p()`  L'azienda afferma che la media delle spese mensili dei dipendenti è pari a $1100€$. Effettuare un test di ipotesi per verificare se la media delle spese mensili sia superiore a $1100€$.
...
`r item()` `r punti_p()` Stimare lo Standard Error di $\hat\beta_1$.
...
`r item()` `r punti_p()` Definire i punti di leva.

`r item()` `r punti_p()` Se in un modello di regressione sappiamo che $\hat\sigma_X=0.5$, $\hat\sigma_Y=1.2$ e $r=0.8$, quanto varrà $\hat\beta_1$, il coefficiente angolare del modello di regressione dove $Y$ viene spiegata da $X$?
...
`r item()` `r punti_p()` Qual è il numero di famiglie che consuma più di 30 ore di streaming?
...
`r item()` `r punti_p()` Quando il tempo di manutenzione del macchinario $X$ è inferiore a 14 ore ($A=\{X< 14\}$), la perdita economica dell'azienda è di €100. Mentre quando è superiore a 14 la perdita è di 500€.
...
`r item()` `r punti_p()`  Siano $Z_1\sim N(0,1)$, $Z_2\sim N(0,1)$, $Z_3\sim N(0,1)$, tre VC normali standard indipendenti.
Come si distribuisce
\[
Y=Z_1^2+Z_2^2+Z_3^3~~~?
\]

`r item()` `r punti_p()` Se $A$ e $B$ sono due eventi tali che $A\cap B\ne\emptyset$, $A$ e $B$ sono indipendenti? (scegliere tra _sempre, mai, dipende_ e motivare la risposta)
...
`r item()` `r punti_p()` Come si distribuiscono gli stimatori di massima verosimiglianze asintoticamente?
...
`r item()` `r punti_p()` Stimare lo Standard Error di $\hat\beta_0$.
...
`r item()` `r punti_p()` Definire il qq-plot.

`r item()` `r punti_p()` Se in un modello di regressione $r=0.35$, $\hat\sigma_X=1.2$ e $\hat\sigma_X=0.5$, quanto varrà
...
`r item()` `r punti_p()` Qual è il numero di individui che spende più di 150€ al mese?
...
`r item()` `r punti_p()` Individuare la classe modale, metterla in relazione con la mediana e indicare la loro relazione con la media.

`r item()` `r punti_p()` Michele ha sostenuto tre esami e ha la media del 26, al quarto esame ha preso 28. Qual è la media calcolata sui 4 esami?
...
`r item()` `r punti_p()` Siano $X_1\sim N(5,1)$ e $X_2\sim N(5,1)$, $X_1$ e $X_2$ indipendenti. Come si distribuisce
\[
\bar X = \frac 12 (X_1+X_2) ~~~?
\]

`r item()` `r punti_p()` Siano $A$ e $B$ due eventi tali che $P(A)=0.3$ e $P(B)=0.3$. $A$ e $B$ possono essere incompatibili? (scegliere tra _sempre, mai, dipende_ e motivare la risposta)
...
`r item()` `r punti_p()` Cosa significa che gli stimatori di massima verosimiglianza sono asintoticamente a massima efficienza?

`r item()` `r punti_p()` Definire gli errori di primo e secondo tipo.
...
`r item()` `r punti_p()` Scrivere la scomposizione della varianza e calcolarla per i dati in esame.
...
`r item()` `r punti_p()` Stimare $\sigma^2_\varepsilon$.
...
`r item()` `r punti_p()` Definire i punti influenti.

`r item()` `r punti_p()` Se in un modello di regressione $r=0.35$, $\hat\sigma_X=1.2$ e $\hat\beta_1=0.5$, quanto varrà
